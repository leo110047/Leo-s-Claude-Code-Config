# 2026-07 架構審查全文與 Goldband Loop 整合計畫
...
2. **方向決策**——審查後維護者拍板的 full absorption 決定,以及支持它的補充事實。
3. **執行 TODO**——分 Phase 的行動清單。

注意:審查全文寫於決策之前,其中部分建議(如「讓 goldband-* 真正隱藏」「切掉 goldband 綁售」)已被第二部分的決策**取代**,取代關係在決策章節註明。
...
**它是什麼**:guardrail + policy distribution layer(或「本機 agent control plane」)。它不是 harness——loop 的所有權在 Claude Code/Codex 手上;不是 workflow runtime——那是 vendored goldband 的;「agent OS」是它應該避免的說法。準確的一句話:「把跨 host 的工程政策編譯成各 host 的 enforcement 機制,並負責安裝與同步的那一層」。

但要誠實指出:repo 的體積結構暴露了真相——第一方 tracked 檔案 233 個,vendored goldband 924 個檔案、1.1 GB。goldband 自己的獨特貢獻是 policy 層 + adapter 層 + installer,workflow 價值大半是轉售 goldband 的。
...
- `goldband-review`/`goldband-investigate`/`goldband-qa` 等重流程 loop 是實的,但那是 goldband 的 loop,goldband 只貢獻了名字和語言包裝。

**缺的東西,而且缺得關鍵**:

- **沒有 eval**。goldband 對自己的 policy 層零 evals——「claim-verification hook 有沒有真的降低虛假完成宣稱?」「deny 規則的誤殺率多少?」完全沒量測。諷刺的是 vendored goldband 自己有 `test:evals`(LLM eval、routing e2e),goldband 這個 meta 層反而沒有。
...
**邊界文件是模範生,實際安裝結果打了自己的臉。** Phase 3 前的 ARCHITECTURE.md、ADR-001 與 vendoring runbook 把 shared policy / adapter / external runtime 的邊界講得很清楚,wrapper manifest 單一事實來源也是對的設計。但——

**最嚴重的實際問題:entrypoint 大爆炸。** Phase 3 前的實際 session skill 清單裡同時出現舊 runtime 入口、`goldband-*` 入口、portable skills 與 legacy commands。當時 runbook 明文要求舊 runtime 入口不得成為主要入口,但 hide-root-skill 邏輯沒有擋住它們出現在 session 裡。後果是三重的:每個 session 燒掉大量 context tokens 在 skill descriptions 上、model 面對多個 review 入口會選錯、curation 的價值主張直接失效。**這是目前單一最高優先的 bug 級架構問題。**
...
1. 外部 workflow runtime 更新:1.1 GB snapshot + patch 重放 + wrapper 重生成,每次 upstream 大改都是一次手術。
...
1. **收斂入口面**(最高優先):讓 `goldband-*` 真正隱藏、刪 legacy commands、砍重複 review 入口到一個。先量測:一個乾淨 session 的 skill 清單 token 成本,before/after。
...
**最可能的死法:不是被取代,是被上游拖死。** 三個移動中的 upstream(Claude Code、Codex、goldband)乘上三套 installer 實作,維護成本是乘法不是加法。commit history 已經顯示時間花在讓東西不壞,而不是花在只有 goldband 能提供的價值(可驗證的 policy)上。一年後最可能的狀態是:adapter 們勉強活著,eval 和 telemetry loop 永遠停在「有種子沒長大」。
...
**該刪或收斂**:通用知識 skills(砍一半以上)、legacy commands、goldband wrappers 裡從沒用過的類別(ios-*、landing-report 那些可以做成 opt-in profile 而不是全裝)、三套 installer 收成兩套。
...
4. **切掉 goldband 綁售**:vendored 1.1 GB runtime 是採用的最大阻力——別人要 guardrails,不一定要整套 workflow。讓 workflow 變成 optional dependency 而不是同一個包。
...
### 決策:full absorption,自己當 Goldband Loop 上游

審查後,維護者決定**不再追原 workflow runtime 上游**,把原 runtime
完全整合進 goldband,並以 **Goldband Loop** 作為新的一方名稱。Goldband
Loop 是 goldband 的 first-party workflow runtime。正式決策記錄見
[ADR-002](DECISIONS.md)。

這個決策**取代**審查中的兩條建議:

- 第 5 節優先級 1 的「讓舊 runtime 入口真正隱藏」→ 改為**原始碼層級改名**
  到 `goldband-*`,從根源解決,不再靠藏。
- 第 6 節的「切掉 goldband 綁售、讓 workflow 變 optional dependency」→ 改為
  **全部吸收**:先把整套 workflow runtime 變成第一方程式碼,後續再用
  telemetry、eval 與維護成本做產品裁剪。

### 支持決策的補充事實(決策當天驗證)

- 原 workflow runtime 的重量集中在少數元件:tracked 924 檔中 `test/` 佔 226、`browse/`
...
- 本機 30 天 usage telemetry(134 筆、13 sessions)幾乎全是 verify-config
  與 skill 建議事件,**沒有任何 workflow runtime 的實際呼叫紀錄**。但
...
- 整套 vendoring 機器可刪:vendoring runbook、patches、rsync 更新手術、
  wrapper manifest 別名產生、hide-root-skill 藏名邏輯。這是維護跑步機裡最大的一台。
- 命名問題從根源解決,入口爆炸一次修掉。
- 繼承原 runtime 現成的 eval harness(`test:evals`、routing e2e)——正好補上
  審查點名的最大缺口。
- 專案身份變清楚:goldband 就是產品本身,不再是「別人產品的包裝層」。

### 付出什麼

- 原上游未來的修復和新功能與本 repo 無關;browse 引擎壞了要自己修。
- bun 建置鏈、Chrome extension、iOS bridge 變成第一方責任。
- eval 要跑要花 API 錢。

### 核心原則

> 先停止把 workflow runtime 當外部上游,把 Goldband Loop 全部變成 goldband 第一方責任。
> 後續裁剪由 telemetry、eval、維護成本與產品判斷驅動,不再阻塞整合。

### 授權

原上游為 MIT。整合後保留原 LICENSE 聲明與出處(attribution)。
...
Phase 0/1 對應審查優先級 1、2;Phase 3 實作 full absorption 決策;
...
- [x] 維護者決定跳過 keep / freeze / delete ADR,改為 Phase 3 全部整合。
- [ ] 保留 telemetry 報表作為後續產品裁剪與維護優先序依據。

完成條件:Phase 3 prompt 與本計畫文件都改成 full absorption contract。

### Phase 3 — 合併重構(破壞性最大)

- [x] 原 workflow runtime 全部升格為第一方 `goldband-loop/` 目錄。
- [x] 原始碼層級改名:舊 runtime 前綴收斂為 `goldband-*`。
- [x] 刪除:vendoring runbook、patches、wrapper manifest / generator、hide root skills 邏輯、`shell/install/workflow-wrapper-aliases.sh`。
- [x] 重寫 `ARCHITECTURE.md`(vendor 邊界章節作廢)、更新 README 兩份、`AGENTS.md` / `codex/AGENTS.md` 邊界說明。
- [x] 保留原上游 MIT LICENSE 聲明與 attribution。
- [x] 建「安裝後 inventory 實測」腳本並跑過:在乾淨環境(temp `HOME`)跑 installer,列出安裝結果實際暴露的全部入口(skills、commands、runtime bins),與預期清單逐項比對。

完成條件(全部要有本次執行的實測證據,不接受「應該沒問題」):

1. CI 綠。
2. **安裝後 inventory 實測通過**:乾淨環境安裝後,實際 session 可見的入口清單中——
   - 零個舊 runtime 前綴入口(含 skill 名、wrapper、底層入口);
   - 零個 legacy commands 殘留(`code-review`、`checkpoint`、`map-codebase` 等 Phase 1 已刪項目不得因 installer 路徑或快取復活);
   - 每個出現的入口都能對應到整合後的 Goldband Loop inventory,沒有舊 vendor/wrapper 產物意外復活。
3. **文件與現實一致性核對**:README.md、README.en.md、ARCHITECTURE.md、ADR、installer 的 help/status 輸出、`goldband-loop/inventory.json` 交叉比對,講的是同一套入口清單與同一個架構描述;任何一處把已刪除的元件、vendor 邊界或 wrapper 機制描述成現役機制即算不通過。
...
未排 Phase)、Goldband Loop 與 policy 的解耦(整合後以 installer profile
與 inventory gate 體現)。
...
- **browse 引擎腐爛**:Chrome 改版後 browse/QA 類 workflow 全掛。緩解:full absorption 後排定期 smoke test,再用 telemetry 與維護成本決定是否裁剪。
- **砍錯東西**:telemetry 視窗太短誤刪有用元件。緩解:Phase 0 先補量測;git 歷史永遠可復原。
- **合併後測試債**:繼承的 test/ 有 226 檔,全部吸收後會拖高 CI 維護成本。緩解:Phase 3 先保證可跑與可分類,Phase 4 再接 eval/CI 並逐步裁剪不再承擔的測試。
...
| 文件 | 處置 |
|---|---|
| vendoring runbook | 刪除 |
| `patches/workflow/` | 刪除 |
| `ARCHITECTURE.md` vendor 邊界章節 | 重寫 |
| `docs/DECISIONS.md` ADR-001 的「external runtime 獨立生命週期」假設 | 由 ADR-002 取代該假設,ADR-001 其餘(thin skills)仍有效 |
