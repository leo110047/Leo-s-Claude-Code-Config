...
| 組件 | 數量 | 說明 |
|------|------|------|
| **Skills** | 18 全域 + 10 Unity | 涵蓋除錯、安全、架構、測試、效能、API、資料庫、CI/CD、規劃、Subagent、按需安全模式等 |
...
./install.sh skills-full       # 全量 skills（18 個）
...
- `dev`: `core` + `api-design`、`backend-patterns`、`careful-mode`、`claude-config-verification`、`code-review-skill`、`database-patterns`、`testing-strategy`
...
| Phase | 模式 | 行為 |
|------|------|------|
| `PreToolUse` | blocking + non-blocking | `careful-mode` 啟用時會 block 高風險 Bash；`dev-server/doc-file/secret` 會 block；`git push` 只提醒 |
