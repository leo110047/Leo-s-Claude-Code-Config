...
│   ├── hooks.json
│   └── scripts/
│       ├── hooks/skill-activation-suggestions.js
...
除了 `UserPromptSubmit` 與 `SubagentStop` 之外，其餘 phase 使用單一入口 `hooks/scripts/hooks/hook-router.js` 分派策略：

| Phase | 模式 | 行為 |
|------|------|------|
| `UserPromptSubmit` | non-blocking | 根據 prompt keyword/pattern 建議相關 skill，並記錄 prompt-trigger telemetry |
