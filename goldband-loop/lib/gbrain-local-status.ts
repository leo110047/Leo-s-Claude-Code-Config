 ...
 *
 * Shared between bin/goldband-gbrain-detect (preamble probe on every skill start)
 * and bin/goldband-gbrain-sync.ts (orchestrator SKIP-when-not-ok semantics).
 ...
 *   - Cache: 60s TTL at ~/.goldband/.gbrain-local-status-cache.json, keyed on
...
interface CacheEntry {
  // Local-cache schema version, controlled by goldband. Not to be confused
  // with `gbrain doctor --json` output schema_version (gbrain v0.25+ emits
  // schema_version: 2). Doctor-output parsing lives in
  // lib/goldband-memory-helpers.ts:freshDetectEngineTier and accepts both
...
/** Cache path computed fresh on each call so tests can mutate GOLDBAND_HOME per case. */
export function cacheFilePath(): string {
  return join(
    process.env.GOLDBAND_HOME || join(userHome(), ".goldband"),
