...
function copyDirSync(src: string, dest: string): void {
  ...
  for (const entry of fs.readdirSync(src, { withFileTypes: true })) {
    // Skip symlinks to avoid infinite recursion (e.g., .claude/skills/goldband → repo root)
...
function saveDedupIndex(harvestRoot: string, index: DedupIndex): void {
...
export class WorktreeManager {
  ...
  constructor(repoRoot?: string, stateRoot?: string) {
    ...
    this.harvestRoot = path.join(resolveGoldbandStateRoot(stateRoot), 'dev', 'harvests');
  ...
  create(testName: string): string {
    ...
    const worktreeBase = path.join(this.repoRoot, '.goldband-worktrees', this.runId);
  ...
  harvest(testName: string): HarvestResult | null {
    ...
    try {
      ...
      if (!isDuplicate) {
        ...
        saveDedupIndex(this.harvestRoot, dedupIndex);
      }
  ...
  cleanupAll(): void {
    ...
    const runDir = path.join(this.repoRoot, '.goldband-worktrees', this.runId);
    try {
  ...
  pruneStale(): void {
    try {
      ...
      const worktreeBase = path.join(this.repoRoot, '.goldband-worktrees');
