 ...
 *
 * Every `gbrain ...` spawn from `bin/goldband-gbrain-sync.ts` and
 * `bin/goldband-memory-ingest.ts` MUST go through `spawnGbrain` (or
 ...
 *      regressing on.
 *
 *   3. **`GBRAIN_HOME` honored consistently.** Other goldband helpers
 *      (`detectEngineTier`) already honor `GBRAIN_HOME`. `buildGbrainEnv`
 *      reads from `${GBRAIN_HOME:-$HOME/.gbrain}/config.json` so all
 *      goldband-side gbrain calls agree on which config file matters.
 *
 * **Escape hatch:** `GOLDBAND_RESPECT_ENV_DATABASE_URL=1` returns the
 ...
 * unchanged when:
 *   - `GOLDBAND_RESPECT_ENV_DATABASE_URL=1` (intentional opt-out),
...
export function buildGbrainEnv(opts: BuildGbrainEnvOptions = {}): NodeJS.ProcessEnv {
  ...
  if (baseEnv.GOLDBAND_RESPECT_ENV_DATABASE_URL === "1") return out;
 ...
 * listeners (e.g., `gbrain import` in `goldband-memory-ingest.ts`). Always
