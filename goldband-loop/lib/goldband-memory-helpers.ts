/**
 * goldband-memory-helpers — shared helpers for the V1 memory ingest + retrieval pipeline.
 *
 * Imported by:
 ...
 *   ED1 — engine-tier cache lives in ~/.goldband/.gbrain-engine-cache.json (60s TTL)
 *
 ...
 *
 * Examples:
 *   https://github.com/leo110047/goldband.git → github.com/leo110047/goldband
 *   git@github.com:leo110047/goldband.git    → github.com/leo110047/goldband
...
function gitleaksAvailable(): boolean {
  ...
  try {
    ...
    process.stderr.write(
      "[goldband-memory-helpers] gitleaks not in PATH; secret scanning disabled. " +
 ...
 * Replaces the inadequate regex scanner in bin/goldband-brain-sync (which only
...
function engineCachePath(): string {
  return join(goldbandHome(), ".gbrain-engine-cache.json");
}

function errorLogPath(): string {
  return join(goldbandHome(), ".gbrain-errors.jsonl");
}

/**
 * Detect which gbrain engine is active (PGLite vs Supabase) and cache the
 * answer for 60s in ~/.goldband/.gbrain-engine-cache.json. Caching avoids
...
export function detectEngineTier(): EngineDetect {
  ...
  try {
    ...
    writeFileSync(
      engineCachePath(),
      JSON.stringify({ ...fresh, last_writer: "goldband-memory-helpers.detectEngineTier" }, null, 2),
      "utf-8"
    );
  } catch {
...
// Best-effort JSONL append to ~/.goldband/.gbrain-errors.jsonl. Never throws.
 ...
 * to ~/.goldband/.gbrain-errors.jsonl for forensic debugging. Replaces ad-hoc
