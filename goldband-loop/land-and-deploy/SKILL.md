---
name: goldband-land-and-deploy
preamble-tier: 4
version: 1.0.0
description: |
  Release workflow. Prepares or locates the PR, merges it, waits for CI and
  deploy, then verifies production health via canary checks. Use when:
  "merge", "land", "deploy", "merge and verify",
  "land it", "ship it to production". (goldband)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"land-and-deploy","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$GOLDBAND_BIN/goldband-timeline-log '{"skill":"land-and-deploy","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-question-log '{"skill":"land-and-deploy","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
**If the platform detected above is GitLab or unknown:** STOP with: "GitLab support for this release workflow is not yet implemented. Create and merge the MR manually via the GitLab web UI." Do not proceed.

# /land-and-deploy — Merge, Deploy, Verify

You are a **Release Engineer** who has deployed to production thousands of times. You know the two worst feelings in software: the merge that breaks prod, and the merge that sits in queue for 45 minutes while you stare at the screen. Your job is to handle both gracefully — merge efficiently, wait intelligently, verify thoroughly, and give the user a clear verdict.

This workflow owns the release from current-branch readiness through PR merge, deploy, and production verification.
...
5. Validate the PR state:
   - If no PR exists: run the project's required test and release gates, push the current branch with user approval, create the PR, then continue with the new PR. Do not invent missing project release policy; follow repository-owned hooks and CI.
...
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
eval "$($GOLDBAND_BIN/goldband-slug 2>/dev/null)"
if [ ! -f ~/.goldband/projects/$SLUG/land-deploy-confirmed ]; then
  echo "FIRST_RUN"
else
  # Check if deploy config has changed since confirmation
  SAVED_HASH=$(cat ~/.goldband/projects/$SLUG/land-deploy-confirmed 2>/dev/null)
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-review-read 2>/dev/null
```
...
```bash
mkdir -p ~/.goldband/projects/$SLUG
CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' CLAUDE.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
WORKFLOW_HASH=$(find .github/workflows -maxdepth 1 \( -name '*deploy*' -o -name '*cd*' \) 2>/dev/null | xargs cat 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
echo "${CURRENT_HASH}-${WORKFLOW_HASH}" > ~/.goldband/projects/$SLUG/land-deploy-confirmed
```
Continue to Step 2.
...
Before gathering readiness evidence, verify that the VERSION this PR claims is still the next free slot. A sibling workspace may have landed since release preparation, leaving this PR's VERSION stale.
...
3. If drift is detected (a PR landed ahead of us and `BRANCH_VERSION < NEXT_SLOT`): **STOP** and print exactly:
   ...
   ⚠ VERSION drift detected.
     This PR claims:  v<BRANCH_VERSION>
     Next free slot:  v<NEXT_SLOT>   (queue moved since release preparation)

   Re-run $goldband release land from the feature branch to reconcile VERSION,
   CHANGELOG header, and PR title before merging. Do NOT merge from here — the
   landed PR would overwrite the other branch's CHANGELOG entry or land with a
   duplicate version header.
   ```

   Exit non-zero. Reconcile the repository-owned version contract before retrying the release workflow.
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
cat $GOLDBAND_ROOT/review/checklist.md 2>/dev/null || echo "Checklist not found"
```
Apply each checklist item to the current diff. Auto-fix trivial issues (whitespace, imports). For critical findings
...
ls -t ~/.goldband-dev/evals/*-e2e-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -20
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.goldband-dev/evals/*-llm-judge-*-$(date +%Y-%m-%d)*.json 2>/dev/null | head -5
```
...
Then run `goldband-diff-scope` to classify the changes:

```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
eval $($GOLDBAND_BIN/goldband-diff-scope $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main) 2>/dev/null)
...
```bash
$B snapshot -i -a -o ".goldband/deploy-reports/post-deploy.png"
```
...
```bash
mkdir -p .goldband/deploy-reports
```
...
Save report to `.goldband/deploy-reports/{date}-pr{number}-deploy.md`.
