---
name: goldband-plan-tune
preamble-tier: 2
version: 1.0.0
description: |
  Self-tuning question sensitivity + developer psychographic for goldband (v1: observational).
  Review which AskUserQuestion prompts fire across goldband skills, set per-question preferences
  ...
  "developer profile", or "turn off question tuning". (goldband)

  Proactively suggest when the user says the same goldband question has come up before,
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-tune","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/goldband/bin/goldband-timeline-log '{"skill":"plan-tune","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/goldband/bin/goldband-question-log '{"skill":"plan-tune","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
8. **"Turn it on" / "enable"** → `~/.claude/skills/goldband/bin/goldband-config set question_tuning true`
...
1. Read the current state:
   ```bash
   _QT=$(~/.claude/skills/goldband/bin/goldband-config get question_tuning 2>/dev/null || echo "false")
   echo "QUESTION_TUNING: $_QT"
   ```

2. If `false`, use AskUserQuestion:

   > Question tuning is off. goldband can learn which of its prompts you find
   > valuable vs noisy — so over time, goldband stops asking questions you've
   > already answered the same way. It takes about 2 minutes to set up your
   > initial profile. v1 is observational: goldband tracks your preferences
...
3. If A or B: enable:
   ```bash
   ~/.claude/skills/goldband/bin/goldband-config set question_tuning true
   ```
...
4. If A (full setup), ask FIVE one-per-dimension declaration questions via
   ...
   **Q5 — architecture_care:** "When there's a tradeoff between 'ship now'
   ...
   `~/.goldband/developer-profile.json` under `declared.{dimension}`:

   ```bash
   # Ensure profile exists
   ~/.claude/skills/goldband/bin/goldband-developer-profile --read >/dev/null
   # Update declared dimensions atomically
   eval "$(~/.claude/skills/goldband/bin/goldband-paths)"
   _PROFILE="$GOLDBAND_STATE_ROOT/developer-profile.json"
   bun -e "
     const fs = require('fs');
...
```bash
~/.claude/skills/goldband/bin/goldband-developer-profile --profile
```
...
- Show the vibe (archetype) from `goldband-developer-profile --vibe` — the
...
eval "$(~/.claude/skills/goldband/bin/goldband-slug 2>/dev/null)"
eval "$(~/.claude/skills/goldband/bin/goldband-paths)"
_LOG="$GOLDBAND_STATE_ROOT/projects/$SLUG/question-log.jsonl"
if [ ! -f "$_LOG" ]; then
  echo "NO_LOG"
else
  bun -e "
...
If `NO_LOG`, tell the user: "No questions logged yet. As you use goldband skills,
goldband will log them here."
...
4. Write:
   ```bash
   ~/.claude/skills/goldband/bin/goldband-question-preference --write '{"question_id":"<id>","preference":"<never-ask|always-ask|ask-only-for-one-way>","source":"plan-tune","free_text":"<original phrase>"}'
   ```
...
```bash
~/.claude/skills/goldband/bin/goldband-developer-profile --gap
```
...
```bash
~/.claude/skills/goldband/bin/goldband-question-preference --stats
eval "$(~/.claude/skills/goldband/bin/goldband-slug 2>/dev/null)"
eval "$(~/.claude/skills/goldband/bin/goldband-paths)"
_LOG="$GOLDBAND_STATE_ROOT/projects/$SLUG/question-log.jsonl"
[ -f "$_LOG" ] && echo "TOTAL_LOGGED: $(wc -l < "$_LOG" | tr -d ' ')" || echo "TOTAL_LOGGED: 0"
~/.claude/skills/goldband/bin/goldband-developer-profile --profile | bun -e "
