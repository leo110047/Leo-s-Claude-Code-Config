...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"goldband","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/goldband/bin/goldband-timeline-log '{"skill":"goldband","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
document and follow it as executable workflow instructions.

Internal workflow resolution:

1. Resolve the active runtime root:
   - If this skill defines `GOLDBAND_ROOT` in its setup block, use
     `$GOLDBAND_ROOT`.
   - Otherwise use the directory that contains this root `SKILL.md`.
2. Strip the leading slash from the workflow name.
3. Try these files under the active runtime root, in order:
   - `workflows/<name>.workflow.md`
   - `workflows/goldband-<name>.workflow.md`
   - `workflows/<name without goldband->.workflow.md`
4. Read the first existing file completely, then execute its workflow exactly as
...
- User asks to upgrade goldband → invoke `/goldband-upgrade`
...
- User asks to launch a real browser for QA, "open the browser" → invoke `/open-goldband-browser`
...
- User asks what goldband has learned, "show learnings" → invoke `/learn`
...
If the user opts out of suggestions, run `goldband-config set proactive false`.
If they opt back in, run `goldband-config set proactive true`.

# goldband browse: QA Testing & Dogfooding
