...
description: |
  ...
  Use before /plan-ceo-review or /plan-eng-review. (goldband)
allowed-tools:
  - Bash
  - Read
  - Grep
...
gbrain:
  ...
  context_queries:
    ...
    - id: builder-profile
      kind: filesystem
      glob: "~/.goldband/builder-profile.jsonl"
      tail: 1
      render_as: "## Your builder profile snapshot"
    - id: design-doc-history
      kind: filesystem
      glob: "~/.goldband/projects/{repo_slug}/*-design-*.md"
      sort: mtime_desc
      limit: 3
      render_as: "## Recent design docs for this project"
    - id: prior-eureka
      kind: filesystem
      glob: "~/.goldband/analytics/eureka.jsonl"
      tail: 5
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/goldband/bin/goldband-timeline-log '{"skill":"office-hours","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/goldband/bin/goldband-question-log '{"skill":"office-hours","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
If `NEEDS_SETUP`:
1. Tell the user: "goldband browse needs a one-time build (~10 seconds). OK to proceed?" Then STOP and wait.
...
4. **List existing design docs for this project:**
   ...
   ls -t ~/.goldband/projects/$SLUG/*-design-*.md 2>/dev/null
   ```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
grep -li "<keyword1>\|<keyword2>\|<keyword3>" ~/.goldband/projects/$SLUG/*-design-*.md 2>/dev/null
```
...
This enables cross-team discovery — multiple users exploring the same project will see each other's design docs in `~/.goldband/projects/`.
...
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
D=""
[ -n "$_ROOT" ] && [ -x "$_ROOT/.claude/skills/goldband/design/dist/design" ] && D="$_ROOT/.claude/skills/goldband/design/dist/design"
[ -z "$D" ] && D="$HOME/.claude/skills/goldband/design/dist/design"
...
```bash
SKETCH_FILE="/tmp/goldband-sketch-$(date +%s).html"
```
...
$B screenshot /tmp/goldband-sketch.png
```
...
The screenshot file at `/tmp/goldband-sketch.png` can be referenced by downstream skills
...
`goldband-developer-profile --log-session` binary handles its own directory creation
and writes via atomic mktemp+mv to `~/.goldband/developer-profile.json`.
...
```bash
~/.claude/skills/goldband/bin/goldband-developer-profile --log-session '{"date":"TIMESTAMP","mode":"MODE","project_slug":"SLUG","signal_count":N,"signals":SIGNALS_ARRAY,"design_doc":"DOC_PATH","assignment":"ASSIGNMENT_TEXT","resources_shown":[],"topics":TOPICS_ARRAY}' 2>/dev/null || true
```
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
PRIOR=$(ls -t ~/.goldband/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `~/.goldband/projects/{slug}/{user}-{branch}-design-{datetime}.md`.
...
```bash
mkdir -p ~/.goldband/analytics
echo '{"skill":"office-hours","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.goldband/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```bash
PROFILE=$(~/.claude/skills/goldband/bin/goldband-builder-profile 2>/dev/null) || PROFILE="SESSION_COUNT: 0
TIER: introduction"
...
**Builder Journey Summary** (session 5+): Auto-generate `~/.goldband/builder-journey.md`
...
open "$GOLDBAND_STATE_ROOT/builder-journey.md"
```
...
Auto-generate updated `~/.goldband/builder-journey.md` with narrative arc. Open it.
...
```bash
eval "$(~/.claude/skills/goldband/bin/goldband-slug 2>/dev/null || true)"
~/.claude/skills/goldband/bin/goldband-developer-profile --log-session '{"date":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","mode":"resources","project_slug":"'"${SLUG:-unknown}"'","signal_count":0,"signals":[],"design_doc":"","assignment":"","resources_shown":["URL1","URL2","URL3"],"topics":[]}' 2>/dev/null || true
```

2. Log the selection to analytics:
```bash
mkdir -p ~/.goldband/analytics
echo '{"skill":"office-hours","event":"resources_shown","count":NUM_RESOURCES,"categories":"CAT1,CAT2","ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'"}' >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
```
...
The design doc at `~/.goldband/projects/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
...
```bash
~/.claude/skills/goldband/bin/goldband-learnings-log '{"skill":"office-hours","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
