...
description: |
  ...
  fix evidence, and a ship-readiness summary. For report-only mode, use /qa-only. (goldband)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$GOLDBAND_BIN/goldband-timeline-log '{"skill":"qa","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-question-log '{"skill":"qa","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
if [ "$_CROSS_PROJ" = "true" ]; then
  $GOLDBAND_BIN/goldband-learnings-search --limit 10 --query "qa testing bug regression flake fixture" --cross-project 2>/dev/null || true
else
  $GOLDBAND_BIN/goldband-learnings-search --limit 10 --query "qa testing bug regression flake fixture" 2>/dev/null || true
fi
echo ""
echo "KNOWLEDGE:"
$GOLDBAND_BIN/goldband-knowledge search --domain "qa" --status active --query "qa testing bug regression flake fixture" --limit 5 2>/dev/null || echo "KNOWLEDGE: no matching entries"
```
...
```bash
bun run workflows/run.ts qa app --loop --mode mock
```

The typed mock adapter runs a fixed check list, records schema-validated
pass/fail results, and reruns only failed checks on the next loop iteration.
Step evidence is written to:

```bash
${GOLDBAND_HOME:-$HOME/.goldband}/workflow-runs/goldband-qa.jsonl
```

Live browser QA still uses the markdown flow below until browser actions and
screenshot artifacts are migrated into typed runtime steps.
...
| Mode | full | `--regression .goldband/qa-reports/baseline.json` |
| Output dir | `.goldband/qa-reports/` | `Output to /tmp/qa` |
...
```bash
mkdir -p .goldband/qa-reports/screenshots
```
...
1. **Project-scoped test plans:** Check `~/.goldband/projects/` for recent `*-test-plan-*.md` files for this repo
   ```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
   setopt +o nomatch 2>/dev/null || true  # zsh compat
   eval "$($GOLDBAND_BIN/goldband-slug 2>/dev/null)"
   ls -t ~/.goldband/projects/$SLUG/*-test-plan-*.md 2>/dev/null | head -1
   ```
...
$GOLDBAND_BIN/goldband-knowledge search --domain qa --query "<your-keyword>" --limit 5 2>/dev/null || true
```
...
- Make the **healthiest complete fix** that resolves the issue without unrelated refactors
...
- Include full attribution comment:
  ...
  // Report: .goldband/qa-reports/qa-report-{domain}-{date}.md
  ```
...
**Local:** `.goldband/qa-reports/qa-report-{domain}-{YYYY-MM-DD}.md`
...
```
Write to `~/.goldband/projects/{slug}/{user}-{branch}-test-outcome-{datetime}.md`
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-learnings-log '{"skill":"qa","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
