 ...
 *   1. $GOLDBAND_BROWSE_BIN env override (preferred, matches v1.24 GOLDBAND_*_BIN pattern)
 *   2. $BROWSE_BIN env override (back-compat alias)
 *   3. sibling dir: dirname(argv[0])/../browse/dist/browse[.exe]
 *   4. ~/.claude/skills/goldband/browse/dist/browse[.exe]
...
export function resolveBrowseBin(env: NodeJS.ProcessEnv = process.env): string {
  // 1 + 2: env overrides (GOLDBAND_BROWSE_BIN preferred, BROWSE_BIN back-compat).
  const overrideRaw = env.GOLDBAND_BROWSE_BIN ?? env.BROWSE_BIN;
  ...
  const globalPath = path.join(home, ".claude/skills/goldband/browse/dist/browse");
  ...
  throw new BrowseClientError(
    ...
    [
      ...
      "",
      "make-pdf needs browse (the goldband Chromium daemon) to render PDFs.",
      "Tried:",
      `  - $GOLDBAND_BROWSE_BIN (${env.GOLDBAND_BROWSE_BIN || "unset"})`,
      `  - $BROWSE_BIN (${env.BROWSE_BIN || "unset"})`,
      `  - sibling: ${siblingCandidates.join(", ")}`,
      `  - global: ${globalPath}`,
      "  - PATH: `browse`",
      "",
      "To fix: run goldband setup from the goldband repo:",
      "  cd ~/.claude/skills/goldband && ./setup",
      "",
      "Or set GOLDBAND_BROWSE_BIN explicitly:",
      process.platform === "win32"
        ? '  setx GOLDBAND_BROWSE_BIN "C:\\path\\to\\browse.exe"'
        : "  export GOLDBAND_BROWSE_BIN=/path/to/browse",
    ].join("\n"),
  );
}
