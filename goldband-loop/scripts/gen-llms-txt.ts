...
export async function generateLlmsTxt(opts: GenerateOptions = {}): Promise<GenerateResult> {
  ...
  lines.push('## More');
  lines.push('');
  lines.push('- Repository: https://github.com/garrytan/goldband');
  ...
  void (async () => {
    ...
    if (dryRun) {
      ...
      if (existing !== result.content) {
        console.error('[gen-llms-txt] OUT OF DATE — run `bun run gen:skill-docs` to regenerate goldband/llms.txt');
        process.exit(1);
      }
