#!/usr/bin/env bun
/**
 * analytics — CLI for viewing goldband skill usage statistics.
 *
 * Reads ~/.goldband/analytics/skill-usage.jsonl and displays:
...
const ANALYTICS_FILE = path.join(os.homedir(), '.goldband', 'analytics', 'skill-usage.jsonl');
...
export function formatReport(events: AnalyticsEvent[], period: string = 'all'): string {
  ...
  const lines: string[] = [];
  lines.push('goldband skill usage analytics');
