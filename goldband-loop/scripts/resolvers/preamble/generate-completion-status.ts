...
- summary: <one-line reusable lesson>
  ...
  capture: ${ctx.paths.binDir}/goldband-knowledge capture-candidate --source-type workflow-evidence --source-evidence "<evidence>" --title "<one-line title>" --type <problem-solution|decision|practice> --domains <domain> --summary "<one-line recall summary>" --confidence N --body-file path/to/sanitized-entry.md
\`\`\`
...
\`\`\`bash
${ctx.paths.binDir}/goldband-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
\`\`\`
...
${ctx.paths.binDir}/goldband-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
...
if [ "$_TEL" != "off" ] && [ -x ${ctx.paths.binDir}/goldband-telemetry-log ]; then
  ${ctx.paths.binDir}/goldband-telemetry-log \\
