...
\`\`\`bash
source <(${ctx.paths.binDir}/goldband-diff-scope <base> 2>/dev/null)
\`\`\`
...
**Log the result:**
\`\`\`bash
${ctx.paths.binDir}/goldband-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
\`\`\`
Replace STATUS with "clean" or "issues_found", SOURCE with "codex+subagent", "codex-only", "subagent-only", or "unavailable".`;
}

// ─── Design Hard Rules (OpenAI framework + goldband slop blacklist) ───
...
the legacy approved.json aggregate — \`${ctx.paths.binDir}/goldband-taste-update\`
