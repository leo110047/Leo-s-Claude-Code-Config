#!/bin/bash
# Build Goldband Loop Browser.app — macOS application bundle
#
...
#
# Output: dist/Goldband Loop Browser.app and dist/Goldband Loop-Browser.dmg
#
# Usage:
...
APP_NAME="Goldband Loop Browser"
BUNDLE_ID="com.goldband.browser"
...
# Launcher script
cp "$ROOT/scripts/app/goldband-browser" "$APP_DIR/Contents/MacOS/goldband-browser"
chmod +x "$APP_DIR/Contents/MacOS/goldband-browser"
...
# Patch the bundled Chromium's Info.plist so macOS shows "Goldband Loop Browser"
...
if [ -f "$CHROMIUM_PLIST" ]; then
  ...
  # Replace Chromium's icon with ours so the Dock shows the Goldband Loop icon
...
<dict>
  ...
  <string>goldband-browser</string>
...
echo "    Contents/MacOS/goldband-browser     (launcher)"
...
DMG_PATH="$BUILD_DIR/Goldband Loop-Browser.dmg"
echo ""
echo "  Creating DMG..."
rm -f "$DMG_PATH"
