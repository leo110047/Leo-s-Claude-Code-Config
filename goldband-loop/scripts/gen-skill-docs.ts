...
function externalSkillName(skillDir: string, frontmatterName?: string): string {
  // Root skill (skillDir === '' or '.') always maps to 'goldband' regardless of frontmatter
  if (skillDir === '.' || skillDir === '') return 'goldband';
  ...
  // Don't double-prefix: goldband-upgrade → goldband-upgrade (not goldband-goldband-upgrade)
  if (baseName.startsWith('goldband-')) return baseName;
  return `goldband-${baseName}`;
}
...
for (const currentHost of hostsToRun) {
  ...
  try {
    ...
    // Generate goldband-lite and goldband-full for OpenClaw host
    if (currentHost === 'openclaw' && !DRY_RUN) {
      const openclawDir = path.join(ROOT, 'openclaw');
      if (!fs.existsSync(openclawDir)) fs.mkdirSync(openclawDir, { recursive: true });

      const goldbandLite = `# goldband-lite Planning Discipline
...
`;
      fs.writeFileSync(path.join(openclawDir, 'goldband-lite-CLAUDE.md'), goldbandLite);
      console.log('GENERATED: openclaw/goldband-lite-CLAUDE.md');

      const goldbandFull = `# goldband-full Pipeline
...
2. Run $goldband plan auto to review your approach (CEO + eng + design review pipeline).
...
`;
      fs.writeFileSync(path.join(openclawDir, 'goldband-full-CLAUDE.md'), goldbandFull);
      console.log('GENERATED: openclaw/goldband-full-CLAUDE.md');

      const goldbandPlan = `# goldband-plan: Full Review Gauntlet
...
## Planning Pipeline
1. Read CLAUDE.md and understand the project context.
2. Run $goldband plan strategy to produce a design doc (problem statement, premises, alternatives).
3. Run $goldband plan auto to review the design (CEO + eng + design + DX reviews + codex adversarial).
...
`;
      fs.writeFileSync(path.join(openclawDir, 'goldband-plan-CLAUDE.md'), goldbandPlan);
      console.log('GENERATED: openclaw/goldband-plan-CLAUDE.md');
    }
...
if (!DRY_RUN) {
  try {
    const configPath = path.join(process.env.HOME || '', '.goldband', 'config.yaml');
    if (fs.existsSync(configPath)) {
      const config = fs.readFileSync(configPath, 'utf-8');
      if (/^skill_prefix:\s*true/m.test(config)) {
        console.log('\nNote: skill_prefix is true. Run goldband-relink to re-apply name: patches.');
      }
    }
  } catch { /* non-fatal */ }
}

// Regenerate goldband/llms.txt — single-file capability index for AI agents.
  ...
  void (async () => {
    try {
      ...
      if (result.warnings.length > 0) {
        ...
        console.log(`[gen-llms-txt] goldband/llms.txt: ${result.skills.length} skills, ${result.browseCommands.length} browse commands`);
      }
    } catch (err) {
