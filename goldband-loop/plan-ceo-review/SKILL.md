...
description: |
  ...
  or when the plan feels like it could be thinking bigger. (goldband)
...
gbrain:
  ...
  context_queries:
    - id: prior-ceo-plans
      kind: filesystem
      glob: "~/.goldband/projects/{repo_slug}/ceo-plans/*.md"
      sort: mtime_desc
      limit: 5
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$GOLDBAND_BIN/goldband-timeline-log '{"skill":"plan-ceo-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-question-log '{"skill":"plan-ceo-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
* Healthiest maintainable path: favor the smallest diff that cleanly expresses a sound design, but do not compress a necessary rewrite into a minimal patch. If the existing foundation is broken, invoke permission #9 and say "scrap it and do this instead."
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t ~/.goldband/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
eval "$($GOLDBAND_BIN/goldband-slug 2>/dev/null)" && mkdir -p ~/.goldband/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.goldband/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.goldband/projects/$SLUG/ceo-plans/{old-plan}.md ~/.goldband/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.goldband/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
```bash
mkdir -p ~/.goldband/analytics
echo '{"skill":"plan-ceo-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","iterations":ITERATIONS,"issues_found":FOUND,"issues_fixed":FIXED,"remaining":REMAINING,"quality_score":SCORE}' >> ~/.goldband/analytics/spec-review.jsonl 2>/dev/null || true
```
...
```
NOTE: These represent human-team implementation hours. With CC + goldband,
...
* **Effort estimate:** S/M/L/XL (human team) → with CC+goldband: S→S, M→S, L→M, XL→L
* **Priority:** P1/P2/P3
...
: > "$TASKS_FILE"
"$TASK_EMISSION_BIN" append   --file "$TASKS_FILE"   --phase 'ceo-review'   --run-id "$RUN_ID"   --branch "$BRANCH"   --commit "$COMMIT"   --id "$TASK_ID"   --priority "$PRIORITY"   --component "$COMPONENT"   --files-json "$FILES_JSON"   --effort-human "$EFFORT_HUMAN"   --effort-cc "$EFFORT_CC"   --title "$TITLE"   --source-finding "$SOURCE_FINDING"
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
eval "$($GOLDBAND_BIN/goldband-slug 2>/dev/null)"
rm -f ~/.goldband/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null || true
```
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-review-log '{"skill":"plan-ceo-review","timestamp":"TIMESTAMP","status":"STATUS","unresolved":N,"critical_gaps":N,"mode":"MODE","scope_proposed":N,"scope_accepted":N,"scope_deferred":N,"commit":"COMMIT"}'
```
...
- **B)** Keep in `~/.goldband/projects/` only (local, personal reference)
- **C)** Skip
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-learnings-log '{"skill":"plan-ceo-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
