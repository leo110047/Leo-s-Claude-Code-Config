...
const BIN = path.join(ROOT, 'bin', 'goldband-paths');
...
// helper from inside a bash block (`eval "$(~/.claude/skills/goldband/bin/goldband-paths)"`)
...
function run(env: Record<string, string | undefined>): Record<string, string> {
  ...
  if (result.status !== 0) {
    throw new Error(`goldband-paths failed (status ${result.status}): ${result.stderr}`);
  }
...
describe('goldband-paths', () => {
  test('GOLDBAND_HOME wins over CLAUDE_PLUGIN_DATA and HOME', () => {
    const got = run({
      GOLDBAND_HOME: '/tmp/explicit-state',
      CLAUDE_PLUGIN_DATA: '/tmp/plugin-data',
      HOME: '/tmp/home',
    });
    expect(got.GOLDBAND_STATE_ROOT).toBe('/tmp/explicit-state');
  });

  test('CLAUDE_PLUGIN_DATA ignored when CLAUDE_PLUGIN_ROOT is absent or non-goldband', () => {
    // Without CLAUDE_PLUGIN_ROOT, falls through to HOME path.
    const noRoot = run({ CLAUDE_PLUGIN_DATA: '/tmp/plugin-data', HOME: '/tmp/home' });
    expect(noRoot.GOLDBAND_STATE_ROOT).toBe('/tmp/home/.goldband');

    // With a CLAUDE_PLUGIN_ROOT that doesn't contain "goldband" (e.g. the codex plugin),
    ...
    expect(wrongRoot.GOLDBAND_STATE_ROOT).toBe('/tmp/home/.goldband');
  });

  test('CLAUDE_PLUGIN_DATA respected when CLAUDE_PLUGIN_ROOT identifies goldband', () => {
    const got = run({
      CLAUDE_PLUGIN_DATA: '/tmp/goldband-plugin-data',
      CLAUDE_PLUGIN_ROOT: '/tmp/goldband-garrytan',
      HOME: '/tmp/home',
    });
    expect(got.GOLDBAND_STATE_ROOT).toBe('/tmp/goldband-plugin-data');
  });

  test('HOME-derived state root when GOLDBAND_HOME and CLAUDE_PLUGIN_DATA unset', () => {
    const got = run({ HOME: '/tmp/myhome' });
    expect(got.GOLDBAND_STATE_ROOT).toBe('/tmp/myhome/.goldband');
  });
  ...
  test('CWD fallback when HOME also unset (container env)', () => {
    ...
    const got = run({ HOME: '' });
    expect(got.GOLDBAND_STATE_ROOT).toBe('.goldband');
  });

  test('PLAN_ROOT chain: GOLDBAND_PLAN_DIR > CLAUDE_PLANS_DIR > HOME > CWD', () => {
    expect(run({ GOLDBAND_PLAN_DIR: '/tmp/explicit', HOME: '/h' }).PLAN_ROOT).toBe('/tmp/explicit');
