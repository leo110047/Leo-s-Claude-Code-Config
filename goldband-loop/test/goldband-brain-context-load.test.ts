/**
 * Unit tests for bin/goldband-brain-context-load.ts (Lane C).
 *
...
const SCRIPT = join(import.meta.dir, "..", "bin", "goldband-brain-context-load.ts");
...
describe("goldband-brain-context-load CLI", () => {
  it("--help exits 0 with usage", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: goldband-brain-context-load");
...
describe("goldband-brain-context-load — manifest dispatch", () => {
  ...
  it("uses skill manifest when --skill-file points at a valid SKILL.md", () => {
    const dir = mkdtempSync(join(tmpdir(), "goldband-bcl-"));
...
describe("goldband-brain-context-load — graceful gbrain absence", () => {
