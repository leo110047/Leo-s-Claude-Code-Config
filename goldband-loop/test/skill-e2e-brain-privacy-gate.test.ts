 ...
 * on PATH), runs a cheap goldband skill through the Agent SDK, intercepts
...
describeE2E('gbrain-sync privacy gate fires once via preamble', () => {
  test('goldband skill preamble fires the 3-option AskUserQuestion when gbrain is detected', async () => {
    // Stage a fresh GOLDBAND_HOME with artifacts_sync_mode_prompted=false.
    const goldbandHome = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-goldband-'));
    const fakeBinDir = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-bin-'));

    // Seed the config so the gate's condition passes.
    fs.writeFileSync(
      path.join(goldbandHome, 'config.yaml'),
    ...
    // don't inherit them.
    const origGoldbandHome = process.env.GOLDBAND_HOME;
    const origPath = process.env.PATH;
    process.env.GOLDBAND_HOME = goldbandHome;
    ...
    try {
      ...
      if (origGoldbandHome === undefined) delete process.env.GOLDBAND_HOME;
      else process.env.GOLDBAND_HOME = origGoldbandHome;
      if (origPath === undefined) delete process.env.PATH;
      else process.env.PATH = origPath;
      fs.rmSync(goldbandHome, { recursive: true, force: true });
  ...
  test('privacy gate does NOT fire when artifacts_sync_mode_prompted is already true', async () => {
    ...
    const goldbandHome = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-gate-off-'));
