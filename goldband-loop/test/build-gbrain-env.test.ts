 ...
 * gbrain see when spawned from goldband." The bug it prevents: gbrain's
...
describe("buildGbrainEnv", () => {
  ...
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), "goldband-build-env-"));
  ...
  it("leaves DATABASE_URL untouched when GOLDBAND_RESPECT_ENV_DATABASE_URL=1", () => {
    writeFileSync(join(gbrainHome, "config.json"), JSON.stringify({ database_url: "postgresql://gbrain/db" }));
    const baseEnv = {
      HOME: home,
      DATABASE_URL: "postgresql://intentional/app-db",
      GOLDBAND_RESPECT_ENV_DATABASE_URL: "1",
    };
    const result = buildGbrainEnv({ baseEnv });
