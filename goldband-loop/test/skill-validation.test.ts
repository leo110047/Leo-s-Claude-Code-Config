...
describe('Cross-skill path consistency', () => {
  ...
  test('all greptile-history write references specify both per-project and global paths', () => {
    ...
    for (const file of filesToCheck) {
      ...
      const hasBoth = (content.includes('per-project') && content.includes('global')) ||
        (content.includes('$REMOTE_SLUG/greptile-history') && content.includes('~/.goldband/greptile-history'));
  ...
  test('greptile-triage.md contains both project and global history paths', () => {
    ...
    expect(content).toContain('~/.goldband/greptile-history.md');
  });
...
describe('QA skill structure validation', () => {
  ...
  test('output structure references report directory layout', () => {
    ...
    expect(qaContent).toContain('.goldband/qa-reports/');
  });
});
...
describe('office-hours skill structure', () => {
  ...
  test('contains YC apply CTA with ref tracking', () => {
    expect(content).toContain('ycombinator.com/apply?ref=goldband');
  });
...
// --- goldband-slug helper ---

describe('goldband-slug', () => {
  const SLUG_BIN = path.join(ROOT, 'bin', 'goldband-slug');
  ...
  test('eval sets variables under bash with set -euo pipefail', () => {
    const result = Bun.spawnSync(
      ['bash', '-c', 'set -euo pipefail; eval "$(./bin/goldband-slug 2>/dev/null)"; echo "SLUG=$SLUG"; echo "BRANCH=$BRANCH"'],
  ...
  test('no templates or bin scripts use source process substitution for goldband-slug', () => {
    const result = Bun.spawnSync(
      ['grep', '-r', 'source <(.*goldband-slug', '--include=*.tmpl', '--include=goldband-review-*', '.'],
      { cwd: ROOT, stdout: 'pipe', stderr: 'pipe' }
    );
...
describe('Phase 8e.5 regression test generation', () => {
  ...
  test('regression test includes full attribution comment format', () => {
    ...
    expect(content).toContain('// Report: .goldband/qa-reports/');
  });
...
describe('Codex skill', () => {
  test('codex/SKILL.md exists and has correct frontmatter', () => {
    ...
    expect(content).toContain('name: goldband-codex');
  ...
  test('codex/SKILL.md contains review log persistence', () => {
    ...
    expect(content).toContain('goldband-review-log');
  });
  ...
  test('codex-host ship/review do NOT contain adversarial review step', () => {
    ...
    const shipContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'goldband-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'goldband-review', 'SKILL.md'), 'utf-8');
...
describe('Skill trigger phrases', () => {
  ...
  // Excluded: root goldband (browser tool), goldband-upgrade (goldband-specific),
  // humanizer (text tool)
...
describe('Codex skill validation', () => {
  ...
  test('all skills (except /codex) have both Claude and Codex variants', () => {
    for (const skillDir of CLAUDE_SKILLS_WITH_TEMPLATES) {
      ...
      // Codex variant
      const codexName = skillDir.startsWith('goldband-') ? skillDir : `goldband-${skillDir}`;
  ...
  test('/claude skill is external-host-only — no Claude-host variant', () => {
    ...
    expect(fs.existsSync(path.join(AGENTS_DIR, 'goldband-claude', 'SKILL.md'))).toBe(true);
  });

  test('Codex skill names follow goldband-{name} convention', () => {
    const codexDirs = fs.readdirSync(AGENTS_DIR);
    for (const dir of codexDirs) {
      // Every directory should start with goldband
      expect(dir.startsWith('goldband')).toBe(true);
      // Root is just 'goldband', others are 'goldband-{name}'
      if (dir !== 'goldband') {
        expect(dir.startsWith('goldband-')).toBe(true);
      }
    }
...
describe('Repo mode preamble validation', () => {
  test('generated SKILL.md preamble contains REPO_MODE output', () => {
    ...
    expect(content).toContain('goldband-repo-mode');
  });
...
//
// Browser-skills are bundled in <goldband-root>/browser-skills/<name>/. Each
