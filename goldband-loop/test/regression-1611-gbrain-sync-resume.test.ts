 ...
 *   - resolveStageTimeoutMs (goldband-gbrain-sync.ts) — env parsing + bounds
 *   - decideResume          (goldband-gbrain-sync.ts) — checkpoint+staging detection
 *   - SIGTERM staging preservation invariants in goldband-memory-ingest.ts
 *
...
describe("#1611 resolveStageTimeoutMs — env parsing + bounds", () => {
  test("undefined env → default 2_100_000ms (unchanged from prior behavior)", () => {
    expect(resolveStageTimeoutMs(undefined, "GOLDBAND_SYNC_MEMORY_TIMEOUT_MS")).toBe(DEFAULT_MS);
  });

  test("empty string env → default", () => {
    expect(resolveStageTimeoutMs("", "GOLDBAND_SYNC_MEMORY_TIMEOUT_MS")).toBe(DEFAULT_MS);
  });

  test("non-numeric env → warn + default", () => {
    expect(resolveStageTimeoutMs("not-a-number", "GOLDBAND_SYNC_CODE_TIMEOUT_MS")).toBe(DEFAULT_MS);
  });

  test("zero env → warn + default (not positive)", () => {
    expect(resolveStageTimeoutMs("0", "GOLDBAND_SYNC_MEMORY_TIMEOUT_MS")).toBe(DEFAULT_MS);
  });

  test("negative env → warn + default", () => {
    expect(resolveStageTimeoutMs("-1000", "GOLDBAND_SYNC_MEMORY_TIMEOUT_MS")).toBe(DEFAULT_MS);
  });

  test("below 60_000ms floor (1min) → warn + default", () => {
    expect(resolveStageTimeoutMs("30000", "GOLDBAND_SYNC_MEMORY_TIMEOUT_MS")).toBe(DEFAULT_MS);
    expect(resolveStageTimeoutMs(`${MIN_MS - 1}`, "GOLDBAND_SYNC_MEMORY_TIMEOUT_MS")).toBe(DEFAULT_MS);
  });

  test("above 86_400_000ms ceiling (24h) → warn + default", () => {
    expect(resolveStageTimeoutMs(`${MAX_MS + 1}`, "GOLDBAND_SYNC_MEMORY_TIMEOUT_MS")).toBe(DEFAULT_MS);
    expect(resolveStageTimeoutMs("999999999999", "GOLDBAND_SYNC_CODE_TIMEOUT_MS")).toBe(DEFAULT_MS);
  });

  test("at floor (60_000ms exactly) → accepted", () => {
    expect(resolveStageTimeoutMs(`${MIN_MS}`, "GOLDBAND_SYNC_MEMORY_TIMEOUT_MS")).toBe(MIN_MS);
  });

  test("at ceiling (86_400_000ms exactly) → accepted", () => {
    expect(resolveStageTimeoutMs(`${MAX_MS}`, "GOLDBAND_SYNC_MEMORY_TIMEOUT_MS")).toBe(MAX_MS);
  });

  test("valid mid-range (2h = 7_200_000ms) → returns value", () => {
    expect(resolveStageTimeoutMs("7200000", "GOLDBAND_SYNC_MEMORY_TIMEOUT_MS")).toBe(7_200_000);
  });
});
...
describe("#1611 decideResume — checkpoint + staging detection", () => {
  ...
  beforeEach(() => {
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "goldband-1611-"));
...
describe("#1611 SIGTERM staging preservation — static invariants", () => {
  test("memory-ingest signal handler checks stagingDirIsCheckpointed before cleanup", () => {
    const body = fs.readFileSync(
      path.join(ROOT, "bin", "goldband-memory-ingest.ts"),
      "utf-8",
    );
  ...
  test("memory-ingest reads GOLDBAND_INGEST_RESUME_DIR env to reuse staging dir", () => {
    const body = fs.readFileSync(
      path.join(ROOT, "bin", "goldband-memory-ingest.ts"),
      "utf-8",
    );
    expect(body).toMatch(/process\.env\.GOLDBAND_INGEST_RESUME_DIR/);
    expect(body).toMatch(/skipping prepare phase/);
  });

  test("gbrain-sync orchestrator passes GOLDBAND_INGEST_RESUME_DIR to grandchild on resume", () => {
    const body = fs.readFileSync(
      path.join(ROOT, "bin", "goldband-gbrain-sync.ts"),
      "utf-8",
    );
    expect(body).toMatch(/GOLDBAND_INGEST_RESUME_DIR/);
