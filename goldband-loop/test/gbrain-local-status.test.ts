...
/**
 * Build a tmp HOME + GOLDBAND_HOME + optional fake `gbrain` on PATH.
 *
...
function applyEnv(env: FakeEnv): () => void {
  ...
  process.env.GOLDBAND_HOME = env.goldbandHome;
  return () => {
    ...
    if (prev.GOLDBAND_HOME === undefined) delete process.env.GOLDBAND_HOME;
    else process.env.GOLDBAND_HOME = prev.GOLDBAND_HOME;
  };
}
...
describe("lib/gbrain-local-status — cache behavior", () => {
  ...
  it("invalidates cache when HOME changes (key invariant)", () => {
    ...
    // Switch to a new HOME (different user). Same goldband home (shared cache file).
    const env2 = makeEnv({
      withGbrain: true,
    ...
    // GOLDBAND_HOME stays pointing at env.goldbandHome (the original cache file).
