/**
 * Shape regression test for bin/goldband-gbrain-detect.
 *
 ...
 * callers across most goldband skill preambles shell out to this script and
...
const DETECT_BIN = join(import.meta.dir, "..", "bin", "goldband-gbrain-detect");
...
interface DetectShape {
  ...
  goldband_brain_sync_mode: string;
  goldband_brain_git: boolean;
  goldband_artifacts_remote: string;
  gbrain_local_status: string;
}

describe("bin/goldband-gbrain-detect — shape regression", () => {
  it("emits valid JSON", () => {
  ...
  it("contains all 9 pre-existing keys + the new gbrain_local_status key", () => {
    ...
    try {
      ...
      expect(parsed).toHaveProperty("goldband_brain_sync_mode");
      expect(parsed).toHaveProperty("goldband_brain_git");
      expect(parsed).toHaveProperty("goldband_artifacts_remote");
  ...
  it("preserves field types from the bash version", () => {
    ...
    try {
      ...
      expect(typeof parsed.goldband_brain_git).toBe("boolean");
      ...
      expect(typeof parsed.goldband_brain_sync_mode).toBe("string");
      expect(typeof parsed.goldband_artifacts_remote).toBe("string");
  ...
  it("goldband_brain_sync_mode is one of the three documented values", () => {
    const tmp = mkdtempSync(join(tmpdir(), "detect-shape-"));
    try {
      const out = runDetect({
        HOME: tmp,
      ...
      const parsed = JSON.parse(out) as DetectShape;
      expect(["off", "artifacts-only", "full"]).toContain(parsed.goldband_brain_sync_mode);
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  });
