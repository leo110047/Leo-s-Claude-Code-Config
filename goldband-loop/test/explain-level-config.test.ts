/**
 * goldband-config explain_level round-trip + validation tests.
 *
 * Coverage:
...
const BIN_CONFIG = path.join(ROOT, 'bin', 'goldband-config');

let tmpHome: string;

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'goldband-cfg-test-'));
});
...
function run(...args: string[]): { stdout: string; stderr: string; status: number } {
  // goldband-config precedence is `${GOLDBAND_HOME:-${GOLDBAND_STATE_DIR:-$HOME/.goldband}}`,
  // so GOLDBAND_HOME from the developer's parent env wins over the test's
  // GOLDBAND_STATE_DIR. Override both to isolate from the real ~/.goldband.
  const res = spawnSync(BIN_CONFIG, args, {
    env: { ...process.env, GOLDBAND_STATE_DIR: tmpHome, GOLDBAND_HOME: tmpHome },
    encoding: 'utf-8',
    cwd: ROOT,
  });
  return {
...
describe('goldband-config explain_level', () => {
  ...
  test('get with unset explain_level returns the documented default', () => {
    // goldband-config returns the documented default ("default") when the
    // key is absent from config.yaml — see bin/goldband-config:103. Earlier
