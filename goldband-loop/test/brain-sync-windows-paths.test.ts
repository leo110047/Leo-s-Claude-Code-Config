...
// Static invariants guarding Windows artifact-sync (bin/goldband-brain-sync).
//
...
const SRC = fs.readFileSync(path.join(ROOT, 'bin', 'goldband-brain-sync'), 'utf-8');

describe('goldband-brain-sync — Windows path/exec invariants', () => {
  test('discover-new normalizes relpath separators before fnmatch (bug 1)', () => {
    expect(SRC).toContain('os.path.relpath(full, goldband_home).replace(os.sep, "/")');
  });
