 ...
 * auto-deciding a specific question via `goldband-question-preference --write
 ...
 *
 * Set up:
 *   - tmpDir as GOLDBAND_HOME (isolated state, doesn't touch the user's
 *     real ~/.goldband)
...
describeE2E('AUTO_DECIDE opt-in preserved under Conductor flags (periodic)', () => {
  test('user-opted-in question still auto-decides when AskUserQuestion is --disallowedTools', async () => {
    const tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'goldband-auto-decide-'));
    try {
      // 1. Bootstrap the tmp GOLDBAND_HOME with question_tuning=true.
      const configBin = path.join(ROOT, 'bin', 'goldband-config');
      const setRes = spawnSync(configBin, ['set', 'question_tuning', 'true'], {
        env: { ...process.env, GOLDBAND_HOME: tmpHome },
        encoding: 'utf-8',
      });
      if (setRes.status !== 0) {
        throw new Error(`goldband-config set failed: ${setRes.stderr || setRes.stdout}`);
      }
      ...
      const slugBin = path.join(ROOT, 'bin', 'goldband-slug');
      ...
      const prefBin = path.join(ROOT, 'bin', 'goldband-question-preference');
      ...
      if (writeRes.status !== 0) {
        throw new Error(`goldband-question-preference --write failed: ${writeRes.stderr || writeRes.stdout}`);
      }
