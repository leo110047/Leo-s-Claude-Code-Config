...
function setupWorkdir(suffix: string): { workDir: string; goldbandHome: string; slug: string } {
  const workDir = fs.mkdtempSync(path.join(os.tmpdir(), `skill-e2e-ctx-${suffix}-`));
  const goldbandHome = path.join(workDir, '.goldband-home');
  ...
  for (const script of [
    'goldband-timeline-log', 'goldband-timeline-read', 'goldband-slug',
    'goldband-learnings-log', 'goldband-learnings-search',
    'goldband-update-check', 'goldband-config', 'goldband-repo-mode',
  ]) {
...
Environment:
- Use GOLDBAND_HOME="${goldbandHome}" for all goldband bin scripts.
...
`);
  ...
  return { workDir, goldbandHome, slug };
}

// Helper: seed a saved-context file into the storage dir.
function seedSave(goldbandHome: string, slug: string, filename: string, frontmatter: Record<string, string>, body: string) {
  const dir = path.join(goldbandHome, 'projects', slug, 'checkpoints');
...
describeIfSelected('Context Skills E2E (live-fire)', [
  ...
  testConcurrentIfSelected('context-save-routing', async () => {
    const { workDir, goldbandHome, slug } = setupWorkdir('routing');

    // Prompt pattern: the slash command + explicit "invoke via Skill tool"
    // instruction. The GOLDBAND_HOME / ./bin bash setup that used to be in
    ...
    const result = await runSkillTest({
      ...
      env: { GOLDBAND_HOME: goldbandHome },
      maxTurns: 12,
    ...
    const checkpointDir = path.join(goldbandHome, 'projects', slug, 'checkpoints');
  ...
  testConcurrentIfSelected('context-save-then-restore-roundtrip', async () => {
    const { workDir, goldbandHome, slug } = setupWorkdir('roundtrip');
...
Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('context-restore-fragment-match', async () => {
    const { workDir, goldbandHome, slug } = setupWorkdir('fragment');

    // Seed three saves with distinct titles.
    seedSave(goldbandHome, slug, '20260101-120000-alpha-feature.md',
    ...
    seedSave(goldbandHome, slug, '20260202-120000-middle-payments.md',
    ...
    seedSave(goldbandHome, slug, '20260303-120000-omega-release.md',
  ...
  testConcurrentIfSelected('context-restore-empty-state', async () => {
    const { workDir, goldbandHome, slug } = setupWorkdir('empty');
  ...
  testConcurrentIfSelected('context-restore-list-delegates', async () => {
    const { workDir, goldbandHome, slug } = setupWorkdir('delegates');
    seedSave(goldbandHome, slug, '20260101-120000-seed.md',
  ...
  testConcurrentIfSelected('context-restore-legacy-compat', async () => {
    const { workDir, goldbandHome, slug } = setupWorkdir('legacy');
    ...
    // exactly this reason.
    seedSave(goldbandHome, slug, '20260301-120000-legacy-pre-rename-work.md',
      {
        status: 'in-progress',
  ...
  testConcurrentIfSelected('context-save-list-current-branch', async () => {
    const { workDir, goldbandHome, slug } = setupWorkdir('list-current');

    // Seed 3 files on 3 different branches. Current branch is "main".
    seedSave(goldbandHome, slug, '20260101-120000-main-work.md',
      { status: 'in-progress', branch: 'main', timestamp: '2026-01-01T12:00:00Z' },
      '## Working on: main work LISTCURR_MAIN_TOKEN\n');
    seedSave(goldbandHome, slug, '20260202-120000-feat-alpha.md',
      { status: 'in-progress', branch: 'feat/alpha', timestamp: '2026-02-02T12:00:00Z' },
      '## Working on: alpha LISTCURR_ALPHA_TOKEN\n');
    seedSave(goldbandHome, slug, '20260303-120000-feat-beta.md',
  ...
  testConcurrentIfSelected('context-save-list-all-branches', async () => {
    const { workDir, goldbandHome, slug } = setupWorkdir('list-all');
