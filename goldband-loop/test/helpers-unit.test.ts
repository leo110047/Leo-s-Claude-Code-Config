...
describe('findBudgetRegressions', () => {
  ...
  test('respects GOLDBAND_BUDGET_RATIO env override', () => {
    ...
    const prev = process.env.GOLDBAND_BUDGET_RATIO;
    try {
      ...
      if (prev === undefined) delete process.env.GOLDBAND_BUDGET_RATIO;
      else process.env.GOLDBAND_BUDGET_RATIO = prev;
    }
  });
...
describe('assertNoBudgetRegression', () => {
  ...
  test('throws with all violations and the cap value in the message', () => {
    ...
    expect(err!.message).toContain('GOLDBAND_BUDGET_RATIO');
  });
});
