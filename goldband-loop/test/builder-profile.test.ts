...
function runProfile(): Record<string, string> {
  ...
  const stdout = execSync(`${BIN}/goldband-builder-profile`, execOpts).trim();
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'goldband-profile-'));
});
...
describe('goldband-builder-profile', () => {
