...
const SCRIPT = path.join(import.meta.dir, "..", "bin", "goldband-learnings-search");

describe("goldband-learnings-search injection prevention", () => {
  ...
  test("uses process.env for all user-controlled values", () => {
    ...
    expect(bunBlock).toContain("process.env.GOLDBAND_SEARCH_TYPE");
    expect(bunBlock).toContain("process.env.GOLDBAND_SEARCH_QUERY");
    expect(bunBlock).toContain("process.env.GOLDBAND_SEARCH_LIMIT");
  });
  ...
  test("env vars are set on the bun command line", () => {
    ...
    expect(script).toContain("GOLDBAND_SEARCH_TYPE=");
    expect(script).toContain("GOLDBAND_SEARCH_QUERY=");
    expect(script).toContain("GOLDBAND_SEARCH_LIMIT=");
    expect(script).toContain("GOLDBAND_SEARCH_CROSS=");
  });
