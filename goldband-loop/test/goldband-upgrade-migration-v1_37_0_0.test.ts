/**
 * Unit tests for goldband-upgrade/migrations/v1.37.0.0.sh — split-engine notice.
 *
...
function makeEnv(opts: {
  ...
  const claudeSkillsBin = join(home, ".claude", "skills", "goldband", "bin");
  const claudeJson = join(home, ".claude.json");
  const gbrainConfig = join(gbrainDir, "config.json");
  const configBin = join(claudeSkillsBin, "goldband-config");
  ...
  // Fake goldband-config: returns "true" iff opted-out (matches the real bin's
...
`,
  ...
  return {
    ...
    home,
    goldbandHome,
    doneTouch: join(goldbandHome, ".migrations", "v1.37.0.0.done"),
    claudeJson,
    gbrainConfig,
    configBin,
...
function runMigration(env: MigEnv): { stdout: string; stderr: string; exitCode: number } {
  const result = spawnSync("bash", [MIGRATION], {
    ...
    env: {
      ...process.env,
      HOME: env.home,
      GOLDBAND_HOME: env.goldbandHome,
      // The script looks for goldband-config at $HOME/.claude/skills/goldband/bin
...
describe("goldband-upgrade/migrations/v1.37.0.0.sh", () => {
