...
describe('setup: _link_or_copy invariant (D7)', () => {
  ...
  test('workflow materialization calls the Windows note printer', () => {
    const fnStart = SETUP_SRC.indexOf('create_internal_workflow_docs() {');
...
describe.skipIf(process.platform === 'win32')('setup: _link_or_copy helper — behavior matrix', () => {
  ...
  function runHelper(
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'goldband-helper-'));
    try {
