/**
 * goldband-gbrain-source-wireup — unit tests with mocked gbrain CLI.
 *
 * The helper registers the goldband brain repo as a gbrain federated source
...
const WIREUP_BIN = path.join(BIN_DIR, 'goldband-gbrain-source-wireup');

let tmpHome: string;
let goldbandHome: string;
...
function run(
  ...
  const env = {
    ...
    GOLDBAND_BRAIN_WORKTREE: worktreeDir,
    GOLDBAND_BRAIN_NO_SYNC: '0',
    ...(opts.env || {}),
  };
...
function setupGoldbandRepo(remoteUrl: string) {
  // Real git repo at goldbandHome with at least one commit + an origin remote.
  fs.mkdirSync(goldbandHome, { recursive: true });
  spawnSync('git', ['-C', goldbandHome, 'init', '-q', '-b', 'main'], { stdio: 'pipe' });
  spawnSync('git', ['-C', goldbandHome, 'config', 'user.email', 'test@example.com'], { stdio: 'pipe' });
  spawnSync('git', ['-C', goldbandHome, 'config', 'user.name', 'test'], { stdio: 'pipe' });
  fs.writeFileSync(path.join(goldbandHome, '.brain-allowlist'), '# allowlist\n');
  spawnSync('git', ['-C', goldbandHome, 'add', '.'], { stdio: 'pipe' });
  spawnSync('git', ['-C', goldbandHome, 'commit', '-q', '-m', 'init'], { stdio: 'pipe' });
  spawnSync('git', ['-C', goldbandHome, 'remote', 'add', 'origin', remoteUrl], { stdio: 'pipe' });
}

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'goldband-wireup-test-'));
  goldbandHome = path.join(tmpHome, '.goldband');
  worktreeDir = path.join(tmpHome, '.goldband-brain-worktree');
...
describe('goldband-gbrain-source-wireup — wireup mode', () => {
  test('fresh state: registers source + creates worktree + syncs', () => {
    setupGoldbandRepo('git@github.com:user/goldband-brain-user.git');
    makeFakeGbrain({});
    const r = run([], { env: { GOLDBAND_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    expect(fs.existsSync(worktreeDir)).toBe(true);
    const state = readState();
    expect(state.sources).toHaveLength(1);
    expect(state.sources[0].id).toBe('goldband-brain-user');
  ...
  test('idempotent re-run after success: no new sources add call', () => {
    setupGoldbandRepo('git@github.com:user/goldband-brain-user.git');
    makeFakeGbrain({});
    run([], { env: { GOLDBAND_BRAIN_NO_SYNC: '1' } });
  ...
  test('drift recovery: existing source with different path triggers remove + add', () => {
    ...
    fs.writeFileSync(
      gbrainStateFile,
      JSON.stringify({
        sources: [{ id: 'goldband-brain-user', local_path: '/old/stale/path', federated: true }],
      })
    );
    const r = run([], { env: { GOLDBAND_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
    expect(calls.some((c) => c.startsWith('gbrain sources remove goldband-brain-user'))).toBe(true);
    expect(calls.some((c) => c.includes(`gbrain sources add goldband-brain-user --path ${worktreeDir}`))).toBe(true);
    const state = readState();
    expect(state.sources[0].local_path).toBe(worktreeDir);
  });
  ...
  test('source-id derived from origin URL', () => {
    setupGoldbandRepo('git@github.com:user/goldband-brain-alice.git');
    makeFakeGbrain({});
    const r = run([], { env: { GOLDBAND_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    expect(readState().sources[0].id).toBe('goldband-brain-alice');
  });

  test('source-id fallback to ~/.goldband-brain-remote.txt when .git is gone', () => {
    // No git repo at goldbandHome; just the remote-file
    fs.mkdirSync(tmpHome, { recursive: true });
    fs.writeFileSync(
      path.join(tmpHome, '.goldband-brain-remote.txt'),
      'git@github.com:user/goldband-brain-bob.git\n'
    );
    makeFakeGbrain({});
    // No --strict: helper should benign-skip because .goldband/.git is missing
    const r = run([]);
  ...
  test('source-id from --source-id flag overrides everything', () => {
    setupGoldbandRepo('git@github.com:user/goldband-brain-different.git');
    makeFakeGbrain({});
    run(['--source-id', 'custom-id'], { env: { GOLDBAND_BRAIN_NO_SYNC: '1' } });
    const state = readState();
  ...
  test('--probe: read-only, prints state without mutating', () => {
    ...
    const r = run(['--probe']);
    expect(r.status).toBe(0);
    expect(r.stdout).toContain('source_id=goldband-brain-user');
...
describe('goldband-gbrain-source-wireup — --database-url lock (defends against external config rewrites)', () => {
  test('--database-url flag is exported as GBRAIN_DATABASE_URL to child gbrain calls', () => {
    ...
    const r = run(['--database-url', TARGET], { env: { GOLDBAND_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
    const calls = gbrainCalls();
...
describe('goldband-gbrain-source-wireup — uninstall mode', () => {
...
describe('goldband-gbrain-source-wireup — defensive paths', () => {
  test('--no-pull skips HEAD advance on existing worktree', () => {
    ...
    fs.writeFileSync(path.join(goldbandHome, 'newfile.md'), 'new');
    spawnSync('git', ['-C', goldbandHome, 'add', '.'], { stdio: 'pipe' });
    spawnSync('git', ['-C', goldbandHome, 'commit', '-q', '-m', 'second commit'], { stdio: 'pipe' });
    const parentHeadAfter = spawnSync('git', ['-C', goldbandHome, 'rev-parse', 'HEAD'], {
      encoding: 'utf-8',
    }).stdout.trim();
    ...
    const r = run(['--no-pull'], { env: { GOLDBAND_BRAIN_NO_SYNC: '1' } });
    expect(r.status).toBe(0);
