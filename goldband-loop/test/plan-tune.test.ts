...
describe('end-to-end pipeline (binaries working together)', () => {
  test('log many expand choices → derive pushes scope_appetite up', () => {
    const tmpHome = fs.mkdtempSync(path.join(require('os').tmpdir(), 'goldband-e2e-'));
    try {
      const env = { ...process.env, GOLDBAND_HOME: tmpHome };
      const { spawnSync } = require('child_process');
      const logBin = path.join(ROOT, 'bin', 'goldband-question-log');
      const devBin = path.join(ROOT, 'bin', 'goldband-developer-profile');
  ...
  test('migration preserves sessions, builder-profile shim still works', () => {
    ...
    try {
      ...
      const { spawnSync } = require('child_process');
      const devBin = path.join(ROOT, 'bin', 'goldband-developer-profile');
      const shimBin = path.join(ROOT, 'bin', 'goldband-builder-profile');
