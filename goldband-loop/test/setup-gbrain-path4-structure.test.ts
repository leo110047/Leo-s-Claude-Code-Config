...
//     by unit tests for goldband-gbrain-mcp-verify and goldband-artifacts-init.
//   - The structural prose is the source of regressions for AUQ pacing
//     (the failure mode the goldband repo has tracked since v1.26.x:
...
describe('setup-gbrain Path 4 (Remote MCP) — structural contract', () => {
  ...
  test('Step 4c invokes goldband-gbrain-mcp-verify and STOPs on failure', () => {
    expect(tmpl).toContain('goldband-gbrain-mcp-verify');
  ...
  test('Step 7 calls goldband-artifacts-init with --url-form-supported flag', () => {
    expect(tmpl).toMatch(/goldband-artifacts-init.*--url-form-supported/);
  });
