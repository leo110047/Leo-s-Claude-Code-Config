 ...
 *      Skill written to ~/.goldband/browser-skills/<name>/ with full
 ...
 *   - A scoped GOLDBAND_HOME under the workdir so on-disk artifacts are
...
function setupSkillifyWorkdir(suffix: string, installSkills: string[] = ['scrape', 'skillify']): Workdir {
  ...
  for (const script of [
    'goldband-timeline-log', 'goldband-slug', 'goldband-config',
    'goldband-update-check', 'goldband-repo-mode',
 ...
 * has to live under <workdir>/.goldband/browser-skills/ for the project-tier
 * lookup to find it (goldband's bundled tier resolves from the install dir,
...
function installBundledHackernewsSkill(workDir: string) {
  ...
  const dst = path.join(workDir, '.goldband', 'browser-skills', 'hackernews-frontpage');
  copyDirSync(src, dst);
}
...
describeIfSelected('Browser-skills Phase 2a E2E (/scrape + /skillify)', [
  ...
  testConcurrentIfSelected('scrape-match-path', async () => {
    const { workDir, goldbandHome } = setupSkillifyWorkdir('match', ['scrape']);
...
Do NOT enter the prototype phase. Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('scrape-prototype-path', async () => {
    const { workDir, goldbandHome } = setupSkillifyWorkdir('prototype', ['scrape']);
...
Do NOT use AskUserQuestion.`,
  ...
  testConcurrentIfSelected('skillify-happy-path', async () => {
    const { workDir, goldbandHome } = setupSkillifyWorkdir('happy', ['scrape', 'skillify']);
    const fixturePath = path.join(workDir, 'fixture.html');
    fs.writeFileSync(fixturePath, PROTOTYPE_FIXTURE_HTML);
    const fileUrl = `file://${fixturePath}`;
...
(translates to ~/.goldband/browser-skills/<name>/ via $HOME).

Do NOT halt for clarification.`,
      workingDirectory: workDir,
      env: {
        GOLDBAND_HOME: goldbandHome,
        HOME: workDir,  // /skillify writes to $HOME/.goldband/browser-skills/
      },
      maxTurns: 40,
    ...
    // The skill should land in $HOME/.goldband/browser-skills/<name>/
    const skillsRoot = path.join(workDir, '.goldband', 'browser-skills');
  ...
  testConcurrentIfSelected('skillify-provenance-refusal', async () => {
    const { workDir, goldbandHome } = setupSkillifyWorkdir('refusal', ['skillify']);
...
write any files.`,
    ...
    const skillsRoot = path.join(workDir, '.goldband', 'browser-skills');
    const stagingRoot = path.join(workDir, '.goldband', '.tmp');
  ...
  testConcurrentIfSelected('skillify-approval-reject', async () => {
    const { workDir, goldbandHome } = setupSkillifyWorkdir('reject', ['scrape', 'skillify']);
    const fixturePath = path.join(workDir, 'fixture.html');
    fs.writeFileSync(fixturePath, PROTOTYPE_FIXTURE_HTML);
    const fileUrl = `file://${fixturePath}`;
