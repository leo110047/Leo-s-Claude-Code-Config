// Pure-function tests for bin/goldband-next-version.
...
import {
  ...
  markActiveSiblings,
  resolveVersionPath,
} from "../bin/goldband-next-version";
...
describe("resolveVersionPath (monorepo VERSION-path support)", () => {
  test("CLI flag wins over everything", () => {
    ...
    try {
      mkdirSync(join(dir, ".goldband"));
      writeFileSync(join(dir, ".goldband", "version-path"), "config/VERSION\n");
  ...
  test(".goldband/version-path config is picked up", () => {
    const dir = mkdtempSync(join(tmpdir(), "nextver-"));
    try {
      mkdirSync(join(dir, ".goldband"));
      writeFileSync(join(dir, ".goldband", "version-path"), "Tinas Second Brain/health-tracker/VERSION\n");
  ...
  test("trims whitespace and ignores blank lines after the first", () => {
    ...
    try {
      mkdirSync(join(dir, ".goldband"));
      writeFileSync(join(dir, ".goldband", "version-path"), "  apps/web/VERSION  \n\n# comment-ish line\n");
  ...
  test("empty config file falls back to default VERSION", () => {
    ...
    try {
      mkdirSync(join(dir, ".goldband"));
      writeFileSync(join(dir, ".goldband", "version-path"), "\n");
  ...
  test("empty override string falls back to config/default", () => {
    ...
    try {
      mkdirSync(join(dir, ".goldband"));
      writeFileSync(join(dir, ".goldband", "version-path"), "subproj/VERSION\n");
