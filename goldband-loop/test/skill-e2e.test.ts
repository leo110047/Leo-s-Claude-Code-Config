...
// Eval result collector — accumulates test results, writes to ~/.goldband-dev/evals/ on finalize
...
function dumpOutcomeDiagnostic(dir: string, label: string, report: string, judgeResult: any) {
  try {
    const transcriptDir = path.join(dir, '.goldband', 'test-transcripts');
...
Report whether it worked.`,
  ...
  testIfSelected('skillmd-no-local-binary', async () => {
    // Create a tmpdir with no browse binary — no local .claude/skills/goldband/browse/dist/browse
...
Report the exact output. Do NOT try to fix or install anything — just report what you see.`,
    ...
    // On dev machines with goldband installed globally, the fallback path
    // ~/.claude/skills/goldband/browse/dist/browse exists, so we get READY.
...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testIfSelected('session-awareness', async () => {
    ...
    const result = await runSkillTest({
      prompt: `You are running a goldband skill. The session preamble detected _SESSIONS=4 (the user has 4 goldband windows open).
...
Write your report to ${qaOnlyDir}/qa-reports/qa-only-report.md`,
    ...
    const statusLines = gitStatus.stdout.toString().trim().split('\n').filter(
      (l: string) => l.trim() && !l.includes('.prompt-tmp') && !l.includes('.goldband/') && !l.includes('qa-reports/'),
    );
...
`);
    ...
    projectDir = path.join(os.homedir(), '.goldband', 'projects', 'test-project');
  ...
  test('/plan-eng-review writes test-plan artifact to ~/.goldband/projects/', async () => {
...
describeIfSelected('goldband-upgrade E2E', ['goldband-upgrade-happy-path'], () => {
  ...
  testIfSelected('goldband-upgrade-happy-path', async () => {
    const mockGoldband = path.join(upgradeDir, '.claude', 'skills', 'goldband');
    const result = await runSkillTest({
...
IMPORTANT: The install directory is at ./.claude/skills/goldband — use that exact path.`,
    ...
    recordE2E('/goldband-upgrade happy path', 'goldband-upgrade E2E', result, {
