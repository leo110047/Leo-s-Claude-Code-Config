 ...
 *   - telemetry JSONL under $HOME/.goldband/analytics/ (same walk, but called
...
export interface SinkResult {
  ...
  /** Subset of filesWritten matching .goldband/analytics/*.jsonl. */
...
export async function runWithSecretSink(opts: SecretSinkOptions): Promise<SinkResult> {
  ...
  // Make sure .goldband exists so bins that append to analytics have somewhere to write.
  fs.mkdirSync(path.join(tmpHome, '.goldband', 'analytics'), { recursive: true });

  const env = {
    // Minimal PATH that still finds jq/git/curl/sed so our bins work.
    PATH: '/usr/bin:/bin:/usr/sbin:/sbin:/opt/homebrew/bin:/usr/local/bin',
    HOME: tmpHome,
    GOLDBAND_HOME: path.join(tmpHome, '.goldband'),
    ...(opts.env || {}),
  };
  ...
  for (const [rel, content] of Object.entries(filesWritten)) {
    if (rel.startsWith('.goldband/analytics/') && rel.endsWith('.jsonl')) {
  ...
  for (const seed of opts.seeds) {
    ...
    for (const { rule, matchType } of rules) {
      ...
      for (const [rel, content] of Object.entries(filesWritten)) {
        ...
        if (hit !== null) {
          const channel = rel.startsWith('.goldband/analytics/') ? 'telemetry' : 'file';
