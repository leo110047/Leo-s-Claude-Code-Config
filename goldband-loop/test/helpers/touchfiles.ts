...
export const E2E_TOUCHFILES: Record<string, string[]> = {
  ...
  'operational-learning':     ['scripts/resolvers/preamble.ts', 'bin/goldband-learnings-log'],
  ...
  'auto-decide-preserved':        ['scripts/resolvers/question-tuning.ts', 'scripts/resolvers/preamble/generate-ask-user-format.ts', 'scripts/resolvers/preamble/generate-completion-status.ts', 'plan-ceo-review/**', 'bin/goldband-question-preference', 'bin/goldband-config', 'bin/goldband-slug', 'test/helpers/claude-pty-runner.ts'],
  ...
  'brain-privacy-gate':           ['scripts/resolvers/preamble/generate-brain-sync-block.ts', 'scripts/resolvers/preamble.ts', 'bin/goldband-brain-sync', 'bin/goldband-artifacts-init', 'bin/goldband-config', 'test/helpers/agent-sdk-runner.ts'],
  ...
  'plan-tune-inspect':         ['plan-tune/**', 'scripts/question-registry.ts', 'scripts/psychographic-signals.ts', 'scripts/one-way-doors.ts', 'bin/goldband-question-log', 'bin/goldband-question-preference', 'bin/goldband-developer-profile'],
  ...
  // Global discover
  'global-discover':   ['bin/goldband-global-discover.ts', 'test/global-discover.test.ts'],
  ...
  // Learnings
  'learnings-show': ['learn/**', 'bin/goldband-learnings-search', 'bin/goldband-learnings-log', 'scripts/resolvers/learnings.ts'],

  // Session Intelligence (timeline, context recovery, /context-save + /context-restore)
  'timeline-event-flow':            ['bin/goldband-timeline-log', 'bin/goldband-timeline-read'],
  'context-recovery-artifacts':     ['scripts/resolvers/preamble.ts', 'bin/goldband-timeline-log', 'bin/goldband-slug', 'learn/**'],
  'context-save-writes-file':       ['context-save/**', 'bin/goldband-slug'],
  'context-restore-loads-latest':   ['context-restore/**', 'bin/goldband-slug'],
  ...
  'context-save-then-restore-roundtrip':   ['context-save/**', 'context-restore/**', 'bin/goldband-slug'],
  ...
  'codex-review-findings': ['review/**', '.agents/skills/goldband-review/**', 'codex/**', 'test/helpers/codex-session-runner.ts', 'lib/worktree.ts'],
  ...
  // goldband-upgrade
  'goldband-upgrade-happy-path': ['goldband-upgrade/**'],

  // Deploy skills
  'land-and-deploy-workflow':      ['land-and-deploy/**', 'scripts/gen-skill-docs.ts'],
  'land-and-deploy-first-run':     ['land-and-deploy/**', 'scripts/gen-skill-docs.ts', 'bin/goldband-slug'],
  'land-and-deploy-review-gate':   ['land-and-deploy/**', 'bin/goldband-review-read'],
  ...
  'autoplan-dual-voice': ['autoplan/**', 'codex/**', 'bin/goldband-codex-probe', 'scripts/resolvers/review.ts', 'scripts/resolvers/design.ts'],

  // Multi-provider benchmark adapters — live API smoke against real claude/codex/gemini CLIs
  'benchmark-providers-live': ['bin/goldband-model-benchmark', 'test/helpers/providers/**', 'test/helpers/benchmark-runner.ts', 'test/helpers/pricing.ts'],
  ...
  // path requires GOLDBAND_HAS_IOS_DEVICE=1 and is periodic-tier.
  ...
  // Real-device path — only runs with GOLDBAND_HAS_IOS_DEVICE=1 + a paired
...
export const E2E_TIERS: Record<string, 'gate' | 'periodic'> = {
  ...
  // Privacy gate for goldband-brain-sync — periodic (non-deterministic LLM call,
  ...
  // goldband-upgrade
  'goldband-upgrade-happy-path': 'gate',
  ...
  // dependency, deterministic). with-device path requires GOLDBAND_HAS_IOS_DEVICE.
  'ios-qa-e2e': 'gate',
...
export const LLM_JUDGE_TOUCHFILES: Record<string, string[]> = {
  ...
  'goldband-upgrade/SKILL.md upgrade flow': ['goldband-upgrade/SKILL.md', 'goldband-upgrade/SKILL.md.tmpl'],
