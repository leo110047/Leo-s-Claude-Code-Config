 ...
 * `.gbrain-source` to goldband's own `.gitignore`. Without the consumer-side
...
import { ensureGbrainSourceGitignored } from "../bin/goldband-gbrain-sync";

describe("ensureGbrainSourceGitignored", () => {
  let root: string;

  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "goldband-gbrain-gitignore-"));
  });
