...
describeIfSelected('Learnings E2E', ['learnings-show'], () => {
  ...
  beforeAll(() => {
    ...
    for (const script of ['goldband-learnings-search', 'goldband-learnings-log', 'goldband-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }

    // Seed learnings JSONL — slug must match what goldband-slug computes.
    // With no git remote, goldband-slug falls back to basename(workDir).
...
- Replace any references to ~/.claude/skills/goldband/bin/goldband-slug with ./bin/goldband-slug.
