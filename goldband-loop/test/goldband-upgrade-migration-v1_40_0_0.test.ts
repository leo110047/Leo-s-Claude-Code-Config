/**
 * goldband-upgrade/migrations/v1.40.0.0.sh — migration script unit tests.
 *
...
beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), "goldband-mig-v1400-"));
  goldbandHome = path.join(tmpHome, ".goldband");
  migrationDir = path.join(goldbandHome, ".migrations");
  donePath = path.join(migrationDir, "v1.40.0.0.done");
  allowlistPath = path.join(goldbandHome, ".brain-allowlist");
  privacyPath = path.join(goldbandHome, ".brain-privacy-map.json");
  gitattrsPath = path.join(goldbandHome, ".gitattributes");
  fs.mkdirSync(goldbandHome, { recursive: true });
});

afterEach(() => {
  try {
    fs.chmodSync(goldbandHome, 0o755);
...
function makeCuratedPath(opts: { jq?: "missing" | "shim-fail" | "real" } = {}): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "goldband-mig-path-"));
  const utils = [
    "bash",
    "sh",
    "mkdir",
