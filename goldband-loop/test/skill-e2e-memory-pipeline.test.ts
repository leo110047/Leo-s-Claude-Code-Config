 ...
 *      ~/.goldband/ artifacts (eureka, learning, ceo-plan, design-doc, retro,
 *      builder-profile)
 *   2. Run goldband-memory-ingest --probe → verify counts match disk
 *   3. Run goldband-memory-ingest --bulk → verify state file gets written +
 *      session_id dedup works on re-run (idempotency)
 *   4. Run goldband-gbrain-sync --dry-run → verify all 3 stages preview
 *   5. Run goldband-brain-context-load against a real V1 skill manifest
 ...
 * may or may not be available in CI. We assert on side effects goldband
...
const INGEST = join(REPO_ROOT, "bin", "goldband-memory-ingest.ts");
const SYNC = join(REPO_ROOT, "bin", "goldband-gbrain-sync.ts");
const CONTEXT = join(REPO_ROOT, "bin", "goldband-brain-context-load.ts");

function makeFixtureHome(): string {
  return mkdtempSync(join(tmpdir(), "goldband-e2e-pipeline-"));
}

function setupFixture(home: string): { goldbandHome: string; counts: Record<string, number> } {
  ...
  mkdirSync(join(goldbandHome, "projects", "test-repo", "ceo-plans"), { recursive: true });
  mkdirSync(join(goldbandHome, "projects", "test-repo", "retros"), { recursive: true });
...
describe("V1 memory ingest pipeline E2E", () => {
  it("--probe finds all 9 fixture files across all source types", () => {
    ...
    const { goldbandHome, counts } = setupFixture(home);
    const env = { HOME: home, GOLDBAND_HOME: goldbandHome, GOLDBAND_MEMORY_INGEST_NO_WRITE: "1" };
  ...
  it("--incremental writes a state file with schema_version: 1 + last_writer", () => {
    const home = makeFixtureHome();
    const { goldbandHome } = setupFixture(home);
    const env = { HOME: home, GOLDBAND_HOME: goldbandHome, GOLDBAND_MEMORY_INGEST_NO_WRITE: "1" };
  ...
  it("--incremental is idempotent — re-run reports 0 changes", () => {
    ...
    // First run
    runBun(INGEST, ["--incremental", "--quiet"], env);
    const stateAfterFirst = readFileSync(join(goldbandHome, ".transcript-ingest-state.json"), "utf-8");
