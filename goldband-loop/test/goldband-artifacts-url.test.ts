/**
 * goldband-artifacts-url — URL canonicalization helper.
 *
...
const URL_BIN = path.join(ROOT, 'bin', 'goldband-artifacts-url');
...
describe('goldband-artifacts-url', () => {
  ...
  test('--to https from ssh:// scheme (gitlab self-hosted style)', () => {
    const r = run(['--to', 'https', 'ssh://git@gitlab.example.org/team/goldband-artifacts-team.git']);
    expect(r.stdout).toBe('https://gitlab.example.org/team/goldband-artifacts-team');
  });
