...
Report the exact output — either "READY: <path>" or "NEEDS_SETUP".`,
  ...
  testConcurrentIfSelected('operational-learning', async () => {
    ...
    const goldbandHome = path.join(opDir, '.goldband-home');
    ...
    for (const script of ['goldband-learnings-log', 'goldband-slug']) {
      fs.copyFileSync(path.join(ROOT, 'bin', script), path.join(binDir, script));
      fs.chmodSync(path.join(binDir, script), 0o755);
    }

    // goldband-learnings-log will create the project dir automatically via goldband-slug
...
\`\`\`bash
GOLDBAND_HOME="${goldbandHome}" ${binDir}/goldband-learnings-log '{"skill":"qa","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
\`\`\`
...
Log the operational learning now. Then say what you logged.`,
    ...
    const projectsDir = path.join(goldbandHome, 'projects');
