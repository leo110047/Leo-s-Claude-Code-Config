...
describeIfSelected('PlanTune E2E', ['plan-tune-inspect'], () => {
  ...
  beforeAll(() => {
    ...
    for (const script of [
      'goldband-slug',
      'goldband-config',
      'goldband-question-log',
      'goldband-question-preference',
      'goldband-developer-profile',
      'goldband-builder-profile',
    ]) {
    ...
    // goldband-developer-profile --derive imports from scripts/ — copy those too.
    ...
    const cfgDir = path.join(goldbandHome);
...
IMPORTANT:
- Use GOLDBAND_HOME="${goldbandHome}" as an environment variable for all bin calls.
- Replace any ~/.claude/skills/goldband/bin/ references with ./bin/ (relative path).
- Replace any ~/.claude/skills/goldband/scripts/ references with ./scripts/.
