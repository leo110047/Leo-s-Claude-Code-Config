...
const ALL_SKILLS = (() => {
  ...
  if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
    skills.push({ dir: '.', name: 'root goldband' });
  }
...
describe('gen-skill-docs', () => {
  ...
  test('preamble .pending-* glob is zsh-safe (uses find, not shell glob)', () => {
    for (const skill of CLAUDE_GENERATED_SKILLS) {
      ...
      expect(content).toContain("find ~/.goldband/analytics -maxdepth 1 -name '.pending-*'");
    }
  });
...
describe('PLAN_FILE_REVIEW_REPORT resolver', () => {
  ...
  for (const skill of REVIEW_SKILLS) {
    test(`plan file review report appears in ${skill} generated file`, () => {
      const content = fs.readFileSync(path.join(ROOT, skill, 'SKILL.md'), 'utf-8');
      expect(content).toContain('GOLDBAND REVIEW REPORT');
    });
  }
...
describe('CODEX_SECOND_OPINION resolver', () => {
  const content = fs.readFileSync(path.join(ROOT, 'office-hours', 'SKILL.md'), 'utf-8');
  const codexContent = fs.readFileSync(path.join(ROOT, '.agents', 'skills', 'goldband-office-hours', 'SKILL.md'), 'utf-8');
  ...
  test('Codex host variant does NOT contain the Phase 3.5 resolver output', () => {
    ...
    expect(codexContent).not.toContain('goldband-codex-oh-');
  });
});
...
describe('Codex filesystem boundary', () => {
  ...
  test('codex skill has rabbit-hole detection rule', () => {
    ...
    expect(content).toContain('goldband-update-check');
  ...
  test('autoplan boundary text avoids host-specific paths for cross-host compatibility', () => {
    ...
    // autoplan template uses generic 'skills/goldband' pattern instead of host-specific
    ...
    expect(boundarySection).toContain('skills/goldband');
...
describe('Codex generation (--host codex)', () => {
  ...
  const CODEX_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('goldband')) {
        skills.push({ dir: '.', codexName: 'goldband' });
      }
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      ...
      const codexName = entry.name.startsWith('goldband-') ? entry.name : `goldband-${entry.name}`;
  ...
  test('root goldband bundle has OpenAI metadata for Codex skill browsing', () => {
    ...
    expect(content).toContain('display_name: "goldband"');
    expect(content).toContain('Use $goldband to locate the bundled goldband skills.');
    expect(content).toContain('allow_implicit_invocation: true');
  });

  test('externalSkillName mapping: root is goldband, others are goldband-{dir}', () => {
    // Root → goldband
    expect(fs.existsSync(path.join(AGENTS_DIR, 'goldband', 'SKILL.md'))).toBe(true);
    // Subdirectories → goldband-{dir}
    expect(fs.existsSync(path.join(AGENTS_DIR, 'goldband-review', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'goldband-ship', 'SKILL.md'))).toBe(true);
    // goldband-upgrade doesn't double-prefix
    expect(fs.existsSync(path.join(AGENTS_DIR, 'goldband-upgrade', 'SKILL.md'))).toBe(true);
    // No double-prefix: goldband-goldband-upgrade must NOT exist
    expect(fs.existsSync(path.join(AGENTS_DIR, 'goldband-goldband-upgrade', 'SKILL.md'))).toBe(false);
  });
  ...
  test('/codex skill excluded from Codex output', () => {
    expect(fs.existsSync(path.join(AGENTS_DIR, 'goldband-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(AGENTS_DIR, 'goldband-codex'))).toBe(false);
  });

  test('Codex output includes Claude outside-voice skill with read-only boundary', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'goldband-claude', 'SKILL.md'), 'utf-8');
    expect(content).toContain('claude -p');
    expect(content).toContain('mktemp /tmp/goldband-claude-prompt-');
    expect(content).toContain('mktemp /tmp/goldband-claude-diff-');
    expect(content).not.toContain('/tmp/goldband-claude-diff-$$');
  ...
  test('Codex review step stripped from Codex-host ship and review', () => {
    const shipContent = fs.readFileSync(path.join(AGENTS_DIR, 'goldband-ship', 'SKILL.md'), 'utf-8');
    expect(shipContent).not.toContain('codex review --base');
    expect(shipContent).not.toContain('CODEX_REVIEWS');

    const reviewContent = fs.readFileSync(path.join(AGENTS_DIR, 'goldband-review', 'SKILL.md'), 'utf-8');
  ...
  test('multiline descriptions preserved in Codex output', () => {
    ...
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'goldband-office-hours', 'SKILL.md'), 'utf-8');
    const fmEnd = content.indexOf('\n---', 4);
    const frontmatter = content.slice(4, fmEnd);
  ...
  test('hook skills have safety prose and no hooks: in frontmatter', () => {
    const HOOK_SKILLS = ['goldband-careful', 'goldband-freeze', 'goldband-guard'];
  ...
  test('sidecar paths in ship skill point to goldband/review/ for pre-landing review', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'goldband-ship', 'SKILL.md'), 'utf-8');
    // Ship references the review checklist in its pre-landing review step
    if (content.includes('checklist.md')) {
      expect(content).toContain('.agents/skills/goldband/review/');
      expect(content).not.toContain('.agents/skills/goldband-review/checklist');
    }
  });
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.claude/skills/goldband → $GOLDBAND_ROOT
    expect(content).not.toContain('~/.claude/skills/goldband');
    expect(content).toContain('$GOLDBAND_ROOT');

    // Rule 2: .claude/skills/goldband → .agents/skills/goldband
    expect(content).not.toContain('.claude/skills/goldband');

    // Rule 3: .claude/skills/review → .agents/skills/goldband/review
  ...
  test('path rewrite rules apply to all Codex skills with sidecar references', () => {
    ...
    for (const skill of CODEX_SKILLS) {
      ...
      expect(content).not.toContain('~/.claude/skills');
      expect(content).not.toContain('.claude/skills');
      if (content.includes('goldband-config') || content.includes('goldband-update-check') || content.includes('goldband-telemetry-log')) {
        expect(content).toContain('$GOLDBAND_ROOT');
      }
      ...
      if (content.includes('checklist.md') && !content.includes('design-checklist.md')) {
        expect(content).not.toContain('goldband-review/checklist.md');
      }
    }
  });
  ...
  test('Claude output unchanged: all Claude skills have zero Codex paths', () => {
    for (const skill of CLAUDE_GENERATED_SKILLS) {
      ...
      // not the goldband Codex host install path.
      if (skill.dir !== 'pair-agent' && skill.dir !== 'codex' && skill.dir !== 'autoplan') {
        expect(content).not.toContain('~/.codex/');
      }
      // goldband-upgrade legitimately references .agents/skills for cross-platform detection
      if (skill.dir !== 'goldband-upgrade') {
        expect(content).not.toContain('.agents/skills');
      }
    }
  });
  ...
  test('codex host produces empty outside voices in design-review', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'goldband-design-review', 'SKILL.md'), 'utf-8');
    expect(codexContent).not.toContain('Design Outside Voices');
  });

  test('codex host does not include Codex design block in ship', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'goldband-ship', 'SKILL.md'), 'utf-8');
...
describe('Factory generation (--host factory)', () => {
  ...
  const FACTORY_SKILLS = (() => {
    ...
    if (fs.existsSync(path.join(ROOT, 'SKILL.md.tmpl'))) {
      if (!isSymlinkLoop('goldband')) skills.push({ dir: '.', factoryName: 'goldband' });
    }
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      if (entry.name === 'codex') continue;
      if (!fs.existsSync(path.join(ROOT, entry.name, 'SKILL.md.tmpl'))) continue;
      const factoryName = entry.name.startsWith('goldband-') ? entry.name : `goldband-${entry.name}`;
  ...
  test('sensitive skills have disable-model-invocation', () => {
    const SENSITIVE = ['goldband-ship', 'goldband-land-and-deploy', 'goldband-guard', 'goldband-careful', 'goldband-freeze', 'goldband-unfreeze'];
  ...
  test('non-sensitive skills lack disable-model-invocation', () => {
    const NON_SENSITIVE = ['goldband-qa', 'goldband-review', 'goldband-investigate', 'goldband-browse'];
  ...
  test('/codex skill excluded from Factory output', () => {
    expect(fs.existsSync(path.join(FACTORY_DIR, 'goldband-codex', 'SKILL.md'))).toBe(false);
    expect(fs.existsSync(path.join(FACTORY_DIR, 'goldband-codex'))).toBe(false);
  });

  test('Factory keeps Codex integration blocks', () => {
    // Factory users CAN use Codex second opinions (codex exec is a standalone binary)
    const shipContent = fs.readFileSync(path.join(FACTORY_DIR, 'goldband-ship', 'SKILL.md'), 'utf-8');
    ...
    describe(`${hostConfig.displayName} (--host ${hostConfig.name})`, () => {
      ...
      test('no .claude/skills path leakage outside repo-root sidecar symlinks', () => {
        ...
        for (const skill of skills) {
          // Dev installs may mount the repo root at host/skills/goldband as a runtime
      ...
      test('generates Claude outside-voice skill for external hosts', () => {
        const skillMd = path.join(hostDir, 'goldband-claude', 'SKILL.md');
        expect(fs.existsSync(skillMd)).toBe(true);
        const content = fs.readFileSync(skillMd, 'utf-8');
        expect(content).toContain('claude -p');
        expect(content).toContain('--disable-slash-commands');
      ...
      if (hostConfig.generation.skipSkills?.includes('codex')) {
        test('/codex skill excluded', () => {
          expect(fs.existsSync(path.join(hostDir, 'goldband-codex', 'SKILL.md'))).toBe(false);
        });
      }
    });
  }
...
describe('setup script validation', () => {
  ...
  test('setup separates install path from source path for symlinked repo-local installs', () => {
    expect(setupContent).toContain('INSTALL_GOLDBAND_DIR=');
  ...
  test('generated Codex preambles use dynamic GOLDBAND_ROOT paths', () => {
    const codexSkillDir = path.join(ROOT, '.agents', 'skills', 'goldband-ship');
  ...
  test('setup supports --host kiro with install section and sed rewrites', () => {
    ...
    expect(setupContent).toContain('~/.kiro/skills/goldband');
  });
  ...
  test('setup supports --host opencode with install section and OpenCode skill path vars', () => {
    ...
    expect(setupContent).toContain('OPENCODE_GOLDBAND="$OPENCODE_SKILLS/goldband"');
  });

  test('setup installs OpenCode skills into a nested goldband runtime root', () => {
  ...
  test('create_codex_runtime_root exposes only runtime assets', () => {
    ...
    expect(fnBody).not.toContain('_link_or_copy "$goldband_dir" "$codex_goldband"');
  });

  test('direct Codex installs are migrated out of ~/.codex/skills/goldband', () => {
  ...
  test('setup reads skill_prefix from config', () => {
    ...
    expect(setupContent).toContain('GOLDBAND_CONFIG');
  });
...
describe('discover-skills hidden directory filtering', () => {
  test('discoverTemplates skips dot-prefixed directories', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'goldband-discover-'));
    try {
...
describe('community fixes wave', () => {
  ...
  // #594 — Discoverability: every SKILL.md.tmpl description contains "goldband"
  test('every SKILL.md.tmpl description contains "goldband"', () => {
    for (const skill of ALL_SKILLS) {
      const tmplPath = skill.dir === '.' ? path.join(ROOT, 'SKILL.md.tmpl') : path.join(ROOT, skill.dir, 'SKILL.md.tmpl');
      const content = fs.readFileSync(tmplPath, 'utf-8');
      const desc = extractDescription(content);
      expect(desc.toLowerCase()).toContain('goldband');
    }
  });
...
describe('codex commands must not use inline $(git rev-parse --show-toplevel) for cwd', () => {
  ...
  // Use Bun.Glob to avoid ELOOP from .claude/skills/goldband symlink back to ROOT
...
describe('gen-skill-docs prefix warning (#620/#578)', () => {
  ...
  test('warns about skill_prefix when config has prefix=true', () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'goldband-prefix-warn-'));
    try {
      // Create a fake ~/.goldband/config.yaml with skill_prefix: true
      const fakeHome = tmpDir;
      const fakeGoldband = path.join(fakeHome, '.goldband');
      fs.mkdirSync(fakeGoldband, { recursive: true });
      fs.writeFileSync(path.join(fakeGoldband, 'config.yaml'), 'skill_prefix: true\n');
      ...
      expect(output).toContain('goldband-relink');
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  test('no warning when skill_prefix is false or absent', () => {
    ...
    try {
      ...
      const fakeGoldband = path.join(fakeHome, '.goldband');
      fs.mkdirSync(fakeGoldband, { recursive: true });
      fs.writeFileSync(path.join(fakeGoldband, 'config.yaml'), 'skill_prefix: false\n');
...
// GOLDBAND REVIEW REPORT report-at-bottom contract — verifies the prompt-text
...
describe('GOLDBAND REVIEW REPORT delete-then-append flow', () => {
...
describe('LEARNINGS_SEARCH resolver: query parameter', () => {
  ...
  test('claude host + query=foo bar → both cross-project and project-scoped branches contain --query', () => {
    ...
    const lines = out.split('\n').filter(l => l.includes('goldband-learnings-search'));
  ...
  test('codex host + query=foo bar → codex bash variant contains --query', () => {
    ...
    expect(out).toContain('$GOLDBAND_BIN/goldband-learnings-search');
  });
...
describe('EXIT PLAN MODE GATE placement', () => {
  ...
  // already contains `## GOLDBAND REVIEW REPORT` inside a markdown example fence,
  // and the gate text itself shows `## GOLDBAND REVIEW REPORT` inside a fence too.
