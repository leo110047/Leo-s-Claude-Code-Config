/**
 * goldband-gbrain-detect — gbrain_mcp_mode + goldband_artifacts_remote tests.
 *
...
const DETECT_BIN = path.join(ROOT, 'bin', 'goldband-gbrain-detect');
...
describe('goldband_artifacts_remote', () => {
  ...
  test('artifacts file wins over brain file when both exist', () => {
    ...
    expect(runDetect().json.goldband_artifacts_remote).toBe('https://github.com/x/new');
  });
...
describe('schema regression', () => {
  test('output JSON has all expected keys (sync-gbrain compat)', () => {
    ...
    expect(keys).toEqual([
      ...
      'gbrain_pooler_mode',
      'gbrain_version',
      'goldband_artifacts_remote',
      'goldband_brain_git',
      'goldband_brain_sync_mode',
    ]);
  });
});
