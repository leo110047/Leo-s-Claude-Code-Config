/**
 * Coverage for #1606 — `_goldband_gbrain_validate_varname` LC_ALL=C pin.
 *
 ...
 * failures.
 *
 * Tests exercise the validator by sourcing bin/goldband-gbrain-lib.sh and
 * calling _goldband_gbrain_validate_varname directly. Asserts:
...
function runValidator(name: string): { status: number | null } {
  ...
  const result = spawnSync(
    "bash",
    ["-c", `. "${LIB}"; _goldband_gbrain_validate_varname "$1"`, "bash", name],
    {
      encoding: "utf-8",
      timeout: 5000,
...
describe("#1606 _goldband_gbrain_validate_varname — LC_ALL=C pin", () => {
  ...
  test("LC_ALL=C is local to the validator (does not leak to caller)", () => {
    ...
    const result = spawnSync(
      "bash",
      ["-c", `. "${LIB}"; LC_ALL=fr_FR.UTF-8; _goldband_gbrain_validate_varname FOO; echo "$LC_ALL"`],
      {
        encoding: "utf-8",
        timeout: 5000,
