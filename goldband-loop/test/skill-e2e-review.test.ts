...
describeIfSelected('Review Dashboard Via Attribution', ['review-dashboard-via'], () => {
  ...
  beforeAll(() => {
    ...
    // goldband-review-read reads from ~/.goldband/projects/$SLUG/$BRANCH-reviews.jsonl
    // For the test, we'll write a mock goldband-review-read script that returns our test data
    ...
    // Write a mock goldband-review-read that returns our test data
    const mockBinDir = path.join(dashDir, '.mock-bin');
    fs.mkdirSync(mockBinDir, { recursive: true });
    fs.writeFileSync(path.join(mockBinDir, 'goldband-review-read'), [
      '#!/usr/bin/env bash',
