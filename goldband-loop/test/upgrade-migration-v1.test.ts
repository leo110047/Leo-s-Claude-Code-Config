/**
 * goldband-upgrade/migrations/v1.0.0.0.sh — writing style migration.
 *
 * Coverage:
...
const MIGRATION = path.join(ROOT, 'goldband-upgrade', 'migrations', 'v1.0.0.0.sh');

let tmpHome: string;

beforeEach(() => {
  tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'goldband-mig-test-'));
});
...
function run(): { stdout: string; stderr: string; status: number } {
  // Override HOME too — the migration reads `${HOME}/.claude/skills/goldband/bin/goldband-config`,
  ...
  const res = spawnSync('bash', [MIGRATION], {
    encoding: 'utf-8',
    env: { ...process.env, GOLDBAND_HOME: tmpHome, HOME: tmpHome },
  });
  return {
