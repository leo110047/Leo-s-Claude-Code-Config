...
describeE2E('/setup-gbrain Path 4 (Remote MCP) — happy path', () => {
  test('verifies, registers HTTP MCP, never writes token to CLAUDE.md', async () => {
    ...
    const goldbandHome = fs.mkdtempSync(path.join(os.tmpdir(), 'setup-gbrain-remote-'));
    const fakeBinDir = fs.mkdtempSync(path.join(os.tmpdir(), 'setup-gbrain-remote-bin-'));
    const callLog = makeFakeClaude(fakeBinDir);

    // The skill writes CLAUDE.md in cwd. Use goldbandHome as cwd so we
    // can inspect it after the run.
    fs.writeFileSync(path.join(goldbandHome, 'CLAUDE.md'), '# Test project\n');
    ...
    const orig = {
      goldbandHome: process.env.GOLDBAND_HOME,
    ...
    try {
      ...
      const claudeMd = fs.readFileSync(path.join(goldbandHome, 'CLAUDE.md'), 'utf-8');
      ...
      const wroteBefore = /## GOLDBAND REVIEW REPORT|critical_gaps/i.test(modelTextOutput);
      ...
      if (orig.goldbandHome === undefined) delete process.env.GOLDBAND_HOME; else process.env.GOLDBAND_HOME = orig.goldbandHome;
