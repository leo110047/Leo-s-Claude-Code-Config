...
const INIT_BIN = path.join(ROOT, 'bin', 'goldband-artifacts-init');
...
function makeFakeGh(opts: { authStatus?: 'ok' | 'fail'; repoCreate?: 'success' | 'already-exists' | 'fail'; webUrl?: string } = {}) {
  ...
  const webUrl = opts.webUrl ?? `https://github.com/testuser/goldband-artifacts-testuser`;
...
function makeFakeGlab(opts: { authStatus?: 'ok' | 'fail'; repoCreate?: 'success' | 'fail'; webUrl?: string } = {}) {
  const authStatus = opts.authStatus ?? 'ok';
  const repoCreate = opts.repoCreate ?? 'success';
  const webUrl = opts.webUrl ?? 'https://gitlab.com/testuser/goldband-artifacts-testuser';
  const script = `#!/bin/bash
...
describe('goldband-artifacts-init provider selection', () => {
  test('--host github invokes gh repo create with goldband-artifacts-$USER', () => {
    makeFakeGh({});
    ...
    expect(createCall).toContain('goldband-artifacts-testuser');
  ...
  test('neither authed → falls through to manual URL paste', () => {
    // No gh, no glab fakes.
    const r = run([], { input: 'https://github.com/testuser/goldband-artifacts-testuser\n' });
    expect(r.status).toBe(0);
    expect(r.stderr).toContain('Neither gh nor glab');
  });
});

describe('goldband-artifacts-init canonical URL storage (codex Finding #10)', () => {
  test('stores HTTPS URL canonical in ~/.goldband-artifacts-remote.txt', () => {
    makeFakeGh({ webUrl: 'https://github.com/testuser/goldband-artifacts-testuser' });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
    const remoteFile = path.join(tmpHome, '.goldband-artifacts-remote.txt');
    ...
    expect(stored).toBe('https://github.com/testuser/goldband-artifacts-testuser');
  });

  test('strips trailing .git from gh repo view output', () => {
    makeFakeGh({ webUrl: 'https://github.com/testuser/goldband-artifacts-testuser.git' });
    const r = run(['--host', 'github']);
    expect(r.status).toBe(0);
    const stored = fs.readFileSync(path.join(tmpHome, '.goldband-artifacts-remote.txt'), 'utf-8').trim();
    expect(stored).toBe('https://github.com/testuser/goldband-artifacts-testuser');
  });
...
describe('goldband-artifacts-init idempotency', () => {
  test('--remote <url> bypasses provider selection entirely', () => {
    makeFakeGh({});
    const r = run(['--remote', 'https://github.com/testuser/goldband-artifacts-testuser']);
    expect(r.status).toBe(0);
  ...
  test('re-run with same --remote is safe (no conflict error)', () => {
    makeFakeGh({});
    const url = 'https://github.com/testuser/goldband-artifacts-testuser';
    run(['--remote', url]);
  ...
  test('re-run with DIFFERENT --remote exits 1 with conflict message', () => {
    makeFakeGh({});
    run(['--remote', 'https://github.com/testuser/goldband-artifacts-testuser']);
