/**
 * Static invariant: every goldbandInjectToTerminal call in extension/*.js
 * must be preceded by an await on goldbandScanForPTYInject on the same code
 * path (#1370 / D6).
 *
 ...
 *
 *   Rule 1: every file that calls goldbandInjectToTerminal must also call
 *           goldbandScanForPTYInject.
 *
 *   Rule 2: in any function that calls goldbandInjectToTerminal, an
 *           `await ... goldbandScanForPTYInject` MUST appear before the
...
const INJECT_FN = 'goldbandInjectToTerminal';
const SCAN_FN = 'goldbandScanForPTYInject';
...
function findInjectCallSites(content: string): number[] {
  // Find positions of `goldbandInjectToTerminal(` or `goldbandInjectToTerminal?.(`
  // — but exclude the function DEFINITION (window.goldbandInjectToTerminal = ).
  const sites: number[] = [];
  const callRe = /window\.goldbandInjectToTerminal\s*\??\.?\s*\(/g;
  let match: RegExpExecArray | null;
  while ((match = callRe.exec(content)) !== null) {
    // Look back ~30 chars; if "window.goldbandInjectToTerminal =" appears
    ...
    if (window30.includes('goldbandInjectToTerminal =')) continue;
...
describe('extension/* PTY injection invariant (#1370 / D6)', () => {
  ...
  test('sidepanel-terminal.js defines both goldbandInjectToTerminal and goldbandScanForPTYInject', () => {
    const file = join(EXTENSION_DIR, 'sidepanel-terminal.js');
    const content = readFileSync(file, 'utf-8');
    expect(content).toContain('window.goldbandInjectToTerminal');
    expect(content).toContain('window.goldbandScanForPTYInject');
  });
  ...
  test('inject function stays synchronous (D6 contract preservation)', () => {
    ...
    const match = content.match(/window\.goldbandInjectToTerminal\s*=\s*(async\s+)?function/);
