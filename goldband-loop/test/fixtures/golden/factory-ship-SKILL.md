...
description: |
  ...
  is ready, asks about deploying, wants to push code up, or asks to create a PR. (goldband)
user-invocable: true
...
GOLDBAND_ROOT="$HOME/.factory/skills/goldband"
[ -n "$_ROOT" ] && [ -d "$_ROOT/.factory/skills/goldband" ] && GOLDBAND_ROOT="$_ROOT/.factory/skills/goldband"
GOLDBAND_BIN="$GOLDBAND_ROOT/bin"
GOLDBAND_BROWSE="$GOLDBAND_ROOT/browse/dist"
GOLDBAND_DESIGN="$GOLDBAND_ROOT/design/dist"
_UPD=$($GOLDBAND_BIN/goldband-update-check 2>/dev/null || .factory/skills/goldband/bin/goldband-update-check 2>/dev/null || true)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ship","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
_VENDORED="no"
if [ -d ".factory/skills/goldband" ] && [ ! -L ".factory/skills/goldband" ]; then
  if [ -f ".factory/skills/goldband/VERSION" ] || [ -d ".factory/skills/goldband/.git" ]; then
    _VENDORED="yes"
  fi
fi
...
In plan mode, allowed because they inform the plan: `$B`, `$D`, `codex exec`/`codex review`, writes to `~/.goldband/`, writes to the plan file, and `open` for generated artifacts.
...
```bash
rm -f ~/.goldband/.writing-style-prompt-pending
touch ~/.goldband/.writing-style-prompted
```
...
> Help goldband get better. Share usage data only: skill, duration, crashes, stable device ID. No code, file paths, or repo names.
...
Always run:
```bash
touch ~/.goldband/.telemetry-prompted
```
...
> Let goldband proactively suggest skills, like /qa for "does this work?" or /investigate for bugs?
...
Always run:
```bash
touch ~/.goldband/.proactive-prompted
```
...
> goldband works best when your project's CLAUDE.md includes skill routing rules.
...
> This project has goldband vendored in `.factory/skills/goldband/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .factory/skills/goldband/`
2. Run `echo '.factory/skills/goldband/' >> .gitignore`
...
5. Tell the user: "Done. Each developer now runs: `cd $GOLDBAND_ROOT && ./setup --team`"
...
Effort both-scales: when an option involves effort, label both human-team and CC+goldband time, e.g. `(human: ~2 days / CC: ~15 min)`. Makes AI compression visible at decision time.
...
if [ -f "$HOME/.goldband-artifacts-remote.txt" ]; then
  _BRAIN_REMOTE_FILE="$HOME/.goldband-artifacts-remote.txt"
else
...
if [ -f "$_BRAIN_REMOTE_FILE" ] && [ ! -d "$_GOLDBAND_HOME/.git" ] && [ "$_BRAIN_SYNC_MODE" = "off" ]; then
  _BRAIN_NEW_URL=$(head -1 "$_BRAIN_REMOTE_FILE" 2>/dev/null | tr -d '[:space:]')
  if [ -n "$_BRAIN_NEW_URL" ]; then
    echo "ARTIFACTS_SYNC: artifacts repo detected: $_BRAIN_NEW_URL"
    echo "ARTIFACTS_SYNC: run 'goldband-brain-restore' to pull your cross-machine artifacts (or 'goldband-config set artifacts_sync_mode off' to dismiss forever)"
  fi
fi

if [ -d "$_GOLDBAND_HOME/.git" ] && [ "$_BRAIN_SYNC_MODE" != "off" ]; then
  _BRAIN_LAST_PULL_FILE="$_GOLDBAND_HOME/.brain-last-pull"
  _BRAIN_NOW=$(date +%s)
  _BRAIN_DO_PULL=1
  ...
  if [ "$_BRAIN_DO_PULL" = "1" ]; then
    ( cd "$_GOLDBAND_HOME" && git fetch origin >/dev/null 2>&1 && git merge --ff-only "origin/$(git rev-parse --abbrev-ref HEAD)" >/dev/null 2>&1 ) || true
...
elif [ -d "$_GOLDBAND_HOME/.git" ] && [ "$_BRAIN_SYNC_MODE" != "off" ]; then
  _BRAIN_QUEUE_DEPTH=0
  [ -f "$_GOLDBAND_HOME/.brain-queue.jsonl" ] && _BRAIN_QUEUE_DEPTH=$(wc -l < "$_GOLDBAND_HOME/.brain-queue.jsonl" | tr -d ' ')
  _BRAIN_LAST_PUSH="never"
  [ -f "$_GOLDBAND_HOME/.brain-last-push" ] && _BRAIN_LAST_PUSH=$(cat "$_GOLDBAND_HOME/.brain-last-push" 2>/dev/null || echo never)
...
> goldband can publish your artifacts (CEO plans, designs, reports) to a private GitHub repo that GBrain indexes across machines. How much should sync?
...
If A/B and `~/.goldband/.git` is missing, ask whether to run `goldband-artifacts-init`. Do not block the skill.
...
```bash
jq -n --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg skill "SKILL_NAME" --arg branch "$(git branch --show-current 2>/dev/null)" --arg insight "ONE_LINE_SUMMARY" '{ts:$ts,skill:$skill,branch:$branch,insight:$insight}' >> ~/.goldband/analytics/eureka.jsonl 2>/dev/null || true
```
...
`~/.goldband/analytics/`, matching preamble analytics writes.
...
$GOLDBAND_ROOT/bin/goldband-timeline-log '{"skill":"SKILL_NAME","event":"completed","branch":"'$(git branch --show-current 2>/dev/null || echo unknown)'","outcome":"OUTCOME","duration_s":"'"$_TEL_DUR"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null || true
...
if [ "$_TEL" != "off" ] && [ -x $GOLDBAND_ROOT/bin/goldband-telemetry-log ]; then
  $GOLDBAND_ROOT/bin/goldband-telemetry-log \
...
```bash
$GOLDBAND_ROOT/bin/goldband-review-read
```
...
For Design Review: run `source <($GOLDBAND_ROOT/bin/goldband-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.factory/skills/goldband/review/TODOS-format.md`).
...
- Create an issue assigned to that person (use the platform detected in Step 0):
  - **If GitHub:**
    ...
    gh issue create \
      ...
      --body "Found failing on branch <current-branch>. Failure is pre-existing.\n\n**Error:**\n```\n<first 10 lines>\n```\n\n**Last modified by:** <author>\n**Noticed by:** goldband /ship on <date>" \
...
# Compute project slug for ~/.goldband/projects/ lookup
_PLAN_SLUG=$(git remote get-url origin 2>/dev/null | sed 's|.*[:/]\([^/]*/[^/]*\)\.git$|\1|;s|.*[:/]\([^/]*/[^/]*\)$|\1|' | tr '/' '-' | tr -cd 'a-zA-Z0-9._-') || true
_PLAN_SLUG="${_PLAN_SLUG:-$(basename "$PWD" | tr -cd 'a-zA-Z0-9._-')}"
# Search common plan file locations (project designs first, then personal/local)
for PLAN_DIR in "$HOME/.goldband/projects/$_PLAN_SLUG" "$HOME/.claude/plans" "$HOME/.codex/plans" ".goldband/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
- Review report sections (`## GOLDBAND REVIEW REPORT`)
...
`~/.goldband-dev/plans/1539-framework-aware-review.md` design doc.
...
2. **Read `.factory/skills/goldband/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
For each conditional specialist that passed scope gating, check the `goldband-specialist-stats` output above:
...
```bash
$GOLDBAND_ROOT/bin/goldband-review-log '{"skill":"review","timestamp":"TIMESTAMP","status":"STATUS","issues_found":N,"critical":N,"informational":N,"quality_score":SCORE,"specialists":SPECIALISTS_JSON,"findings":FINDINGS_JSON,"commit":"'"$(git rev-parse --short HEAD)"'","via":"ship"}'
```
...
> You are classifying Greptile review comments for a /ship workflow. Read `.factory/skills/goldband/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
OLD_CFG=$($GOLDBAND_ROOT/bin/goldband-config get codex_reviews 2>/dev/null || true)
...
```bash
$GOLDBAND_ROOT/bin/goldband-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
$GOLDBAND_BIN/goldband-knowledge capture-candidate --source-type workflow-evidence --source-evidence "workflow-runs/<workflow>.jsonl#event" --title "One-line title" --type practice --domains general --summary "One-line recall summary" --confidence N --body-file path/to/entry.md
```
...
```bash
$GOLDBAND_ROOT/bin/goldband-learnings-search --query "<your-keyword>" --limit 5 2>/dev/null || true
```
...
Read `.factory/skills/goldband/review/TODOS-format.md` for the canonical format reference.
...
```bash
# Export [goldband-context] blocks from all WIP commits on this branch.
# This file becomes input to the CHANGELOG entry and may inform PR body context.
mkdir -p "$(git rev-parse --show-toplevel)/.goldband"
git log <base>..HEAD --grep="^WIP:" --format="%H%n%B%n---END---" > \
  "$(git rev-parse --show-toplevel)/.goldband/wip-context-before-squash.md" 2>/dev/null || true
```
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/.factory/skills/goldband/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
...
2. Compute the corrected title: `NEW_TITLE=$($GOLDBAND_ROOT/bin/goldband-pr-title-rewrite.sh "$NEW_VERSION" "$CURRENT")`. The helper handles three cases: title already correct (no-op), title has a different `v<X.Y.Z.W>` prefix (replace it), or title has no version prefix (prepend one).
...
```bash
echo '{"skill":"ship","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","coverage_pct":COVERAGE_PCT,"plan_items_total":PLAN_TOTAL,"plan_items_done":PLAN_DONE,"verification_result":"VERIFY_RESULT","version":"VERSION","branch":"BRANCH"}' >> ~/.goldband/projects/$SLUG/$BRANCH-reviews.jsonl
```
...
- **Review handoff defaults to the healthiest complete fix.** If review or QA finds a confirmed defect, fix the root cause with an intentional blast radius instead of hiding a structural problem behind a cosmetic patch.
