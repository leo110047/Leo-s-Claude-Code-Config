/**
 * Unit tests for bin/goldband-memory-ingest.ts (Lane A).
 *
...
const SCRIPT = join(import.meta.dir, "..", "bin", "goldband-memory-ingest.ts");

// ── Helpers ────────────────────────────────────────────────────────────────

function makeTestHome(): string {
  return mkdtempSync(join(tmpdir(), "goldband-memory-ingest-"));
}
...
describe("goldband-memory-ingest CLI", () => {
  it("prints usage on --help and exits 0", () => {
    const r = runScript(["--help"]);
    expect(r.exitCode).toBe(0);
    expect(r.stderr).toContain("Usage: goldband-memory-ingest");
  ...
  it("--probe on empty home reports 0 files", () => {
    ...
    const r = runScript(["--probe"], { HOME: home, GOLDBAND_HOME: goldbandHome });
    expect(r.exitCode).toBe(0);
  ...
  it("--probe finds goldband artifacts (learnings, eureka, ceo-plan)", () => {
    const home = makeTestHome();
    const goldbandHome = join(home, ".goldband");
    mkdirSync(join(goldbandHome, "analytics"), { recursive: true });
    mkdirSync(join(goldbandHome, "projects", "foo-bar", "ceo-plans"), { recursive: true });

    writeFileSync(join(goldbandHome, "analytics", "eureka.jsonl"), '{"insight":"lake first"}\n');
    writeFileSync(join(goldbandHome, "projects", "foo-bar", "learnings.jsonl"), '{"key":"a","insight":"b"}\n');
    writeFileSync(join(goldbandHome, "projects", "foo-bar", "ceo-plans", "2026-05-01-test.md"), "# Plan\n");
  ...
  it("--sources filter limits the walk to specific types", () => {
    ...
    const goldbandHome = join(home, ".goldband");
    mkdirSync(join(goldbandHome, "analytics"), { recursive: true });
    mkdirSync(join(goldbandHome, "projects", "foo", "ceo-plans"), { recursive: true });

    writeFileSync(join(goldbandHome, "analytics", "eureka.jsonl"), '{"insight":"x"}\n');
    writeFileSync(join(goldbandHome, "projects", "foo", "learnings.jsonl"), '{"key":"a"}\n');

    const r = runScript(["--probe", "--sources", "eureka"], { HOME: home, GOLDBAND_HOME: goldbandHome });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("Total files in window: 1");
    expect(r.stdout).toContain("eureka");
...
describe("goldband-memory-ingest state file", () => {
  it("--incremental on empty home creates state file with schema_version: 1", () => {
    const home = makeTestHome();
    const goldbandHome = join(home, ".goldband");
    mkdirSync(goldbandHome, { recursive: true });
    const r = runScript(["--incremental", "--quiet"], { HOME: home, GOLDBAND_HOME: goldbandHome });
    expect(r.exitCode).toBe(0);
    const statePath = join(goldbandHome, ".transcript-ingest-state.json");
    expect(existsSync(statePath)).toBe(true);
    const state = JSON.parse(readFileSync(statePath, "utf-8"));
    expect(state.schema_version).toBe(1);
    expect(state.last_writer).toBe("goldband-memory-ingest");
    rmSync(home, { recursive: true, force: true });
  });
...
describe("goldband-memory-ingest security: untrusted cwd cannot trigger shell substitution", () => {
  it("does not invoke /bin/sh when a transcript record contains $() in cwd", () => {
    ...
    const markerDir = mkdtempSync(join(tmpdir(), "goldband-mi-cwd-marker-"));
    ...
    const r = runScript(["--incremental", "--quiet"], {
      ...
      GOLDBAND_MEMORY_INGEST_NO_WRITE: "1",
    });
...
describe("internal: parseTranscriptJsonl + buildTranscriptPage shape", () => {
  it("parses a Claude Code JSONL session", async () => {
    const dir = mkdtempSync(join(tmpdir(), "goldband-mi-parse-"));
    ...
    const r = runScript(["--probe"], { HOME: home, GOLDBAND_HOME: join(home, ".goldband") });
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain("Total files in window: 1");
...
describe("goldband-memory-ingest --limit", () => {
...
describe("goldband-memory-ingest writer (gbrain v0.20+ batch `import` interface)", () => {
