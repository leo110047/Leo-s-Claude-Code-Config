...
function runLog(input: string, opts: { expectFail?: boolean } = {}): { stdout: string; exitCode: number } {
  const execOpts: ExecSyncOptionsWithStringEncoding = {
    cwd: ROOT,
    env: { ...process.env, GOLDBAND_HOME: tmpDir },
    encoding: 'utf-8',
    timeout: 15000,
  };
  try {
    const stdout = execSync(`${BIN}/goldband-timeline-log '${input.replace(/'/g, "'\\''")}'`, execOpts).trim();
...
function runRead(args: string = ''): string {
  ...
  try {
    return execSync(`${BIN}/goldband-timeline-read ${args}`, execOpts).trim();
  } catch {
    return '';
  }
}
...
function runReadArgs(args: string[] = []): string {
  ...
  try {
    return execFileSync(path.join(BIN, 'goldband-timeline-read'), args, execOpts).trim();
  } catch {
    return '';
  }
}
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'goldband-timeline-'));
...
describe('goldband-timeline-log', () => {
...
describe('goldband-timeline-read', () => {
