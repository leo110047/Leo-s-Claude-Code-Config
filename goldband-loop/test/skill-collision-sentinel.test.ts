 ...
 * for /rewind, silently shadowing the goldband /checkpoint skill. Users
 ...
 *
 * This file is the "never again" test. It enumerates every goldband skill name
 * from every SKILL.md.tmpl file in the repo and cross-checks against a
 * per-host list of known built-in slash commands. If any goldband skill name
 ...
 * below. If a goldband skill needs to coexist with a built-in anyway (e.g.,
...
const KNOWN_BUILTINS: Record<string, string[]> = {
  ...
  // invoke codex from goldband, we don't install skills INTO codex the same
...
interface GoldbandSkill {
  name: string;
  templatePath: string;
}

function enumerateGoldbandSkills(): GoldbandSkill[] {
  const skills: GoldbandSkill[] = [];
...
describe('skill-collision-sentinel', () => {
  const skills = enumerateGoldbandSkills();
  ...
  test('no duplicate skill names within goldband', () => {
  ...
  // Hard check: no goldband skill name collides with a known host built-in
    ...
    test(`no skill name collides with a ${host} built-in (or has written justification)`, () => {
      ...
      if (collisions.length > 0) {
        const msg = collisions.map(c =>
          ...
          `    Fix: rename the goldband skill (precedent: /checkpoint → /context-save+/context-restore),\n` +
  ...
  test('KNOWN_COLLISIONS_TOLERATED entries are all still active collisions', () => {
    ...
    for (const name of Object.keys(KNOWN_COLLISIONS_TOLERATED)) {
      if (!skillNames.has(name)) {
        stale.push(`  "${name}" is in KNOWN_COLLISIONS_TOLERATED but no goldband skill has that name — remove the exception`);
  ...
  test('the /checkpoint collision that started this file is actually resolved', () => {
    ...
    expect(names.has('goldband-context-save')).toBe(true);
    expect(names.has('goldband-context-restore')).toBe(true);
  });
