/**
 * Unit tests for lib/goldband-memory-helpers.ts (Lane 0 foundation).
 *
...
describe("secretScanFile", () => {
  ...
  it("returns scanner=missing or runs gitleaks (env-dependent)", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "goldband-test-"));
...
`
    ...
    expect(m!.context_queries[1].glob).toBe("~/.goldband/builder-profile.jsonl");
...
describe("withErrorContext", () => {
  ...
  beforeEach(() => {
    savedHome = process.env.GOLDBAND_HOME;
    testHome = mkdtempSync(join(tmpdir(), "goldband-test-home-"));
    process.env.GOLDBAND_HOME = testHome;
  });

  afterAll(() => {
    if (savedHome === undefined) delete process.env.GOLDBAND_HOME;
    else process.env.GOLDBAND_HOME = savedHome;
  });
...
describe("detectEngineTier", () => {
  ...
  beforeEach(() => {
    ...
    testHome = mkdtempSync(join(tmpdir(), "goldband-test-engine-"));
    testGbrainHome = mkdtempSync(join(tmpdir(), "goldband-test-gbrain-"));
    process.env.GOLDBAND_HOME = testHome;
  ...
  it("writes a cache file at ~/.goldband/.gbrain-engine-cache.json", () => {
    detectEngineTier();
    const cachePath = join(testHome, ".gbrain-engine-cache.json");
    expect(existsSync(cachePath)).toBe(true);
    const cached = JSON.parse(readFileSync(cachePath, "utf-8"));
    expect(cached.schema_version).toBe(1);
    expect(cached.last_writer).toBe("goldband-memory-helpers.detectEngineTier");
  });
  ...
  it("parses schema_version:2 doctor JSON via the exec path (regression for #1418)", () => {
    ...
    const binDir = mkdtempSync(join(tmpdir(), "goldband-gbrain-shim-"));
