...
describeIfSelected('Session Intelligence E2E', [
  ...
  beforeAll(() => {
    ...
    goldbandHome = path.join(workDir, '.goldband-home');
    ...
    // Compute slug (same logic as goldband-slug without git remote)
  ...
  // Write a timeline event via goldband-timeline-log, read it back via goldband-timeline-read.
  ...
  testConcurrentIfSelected('timeline-event-flow', async () => {
    const projectDir = path.join(goldbandHome, 'projects', slug);
    fs.mkdirSync(projectDir, { recursive: true });

    // Write two events via the binary
    const logBin = path.join(workDir, 'bin', 'goldband-timeline-log');
    const readBin = path.join(workDir, 'bin', 'goldband-timeline-read');
    const env = { ...process.env, GOLDBAND_HOME: goldbandHome };
    ...
    // Read via goldband-timeline-read
    ...
    // Verify goldband-timeline-read output includes the events
...
IMPORTANT:
- Use GOLDBAND_HOME="${goldbandHome}" as an environment variable when running bin scripts.
- The bin scripts are at ./bin/ (relative to this directory), not at ~/.claude/skills/goldband/bin/.
  Replace any references to ~/.claude/skills/goldband/bin/ with ./bin/ when running commands.
