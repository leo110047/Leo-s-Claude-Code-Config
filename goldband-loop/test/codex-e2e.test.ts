...
const CODEX_E2E_TOUCHFILES: Record<string, string[]> = {
  ...
  'codex-review-findings':   ['review/**', '.agents/skills/goldband-review/**', 'codex/**', 'test/helpers/codex-session-runner.ts'],
};
...
describeCodex('Codex E2E', () => {
  ...
  testIfSelected('codex-discover-skill', async () => {
    // Install goldband-review skill to a temp HOME and ask Codex to list skills
    ...
    expect(
      outputLower.includes('review') || outputLower.includes('goldband') || outputLower.includes('skill'),
    ).toBe(true);
  }, 120_000);
  ...
  testIfSelected('codex-review-findings', async () => {
    ...
    const result = await runCodexSkill({
      skillDir,
      prompt: 'Run the goldband-review skill on this repository. Review the current branch diff and report your findings.',
      timeoutMs: 540_000,
      cwd: testWorktree,
      skillName: 'goldband-review',
    });
