---
name: goldband-plan-design-review
preamble-tier: 3
interactive: true
version: 2.0.0
description: |
  ...
  should be reviewed before implementation. (goldband)
allowed-tools:
  - Read
  - Edit
  - Grep
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"plan-design-review","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$GOLDBAND_BIN/goldband-timeline-log '{"skill":"plan-design-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-question-log '{"skill":"plan-design-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
### The goldband designer — YOUR PRIMARY TOOL

You have the **goldband designer**, an AI mockup generator that creates real visual mockups
...
MUST be saved to `~/.goldband/projects/$SLUG/designs/`, NEVER to `.context/`,
...
goldband designer is available (`DESIGN_READY` was printed during setup), **generate
mockups immediately.** Do not ask permission. This is the default behavior.

Tell the user: "Generating visual mockups with the goldband designer. This is how we
...
`~/.goldband/projects/$SLUG/designs/` (user config directory, not project files).
Mockups are design artifacts that inform the plan, not code changes. The goldband
...
- `mkdir -p ~/.goldband/projects/$SLUG/designs/...`
...
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
eval "$($GOLDBAND_BIN/goldband-slug 2>/dev/null)"
_DESIGN_DIR="$HOME/.goldband/projects/$SLUG/designs/<screen-name>-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
**If `DESIGN_NOT_AVAILABLE`:** Tell the user: "The goldband designer isn't set up yet. Run `$D setup` to enable visual mockups. Proceeding with text-only review, but you're missing the best part." Then proceed to review passes with text-based review.
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-review-log '{"skill":"design-outside-voices","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
$D generate --brief "<description of what 10/10 looks like for this dimension>" --output /tmp/goldband-ideal-<dimension>.png
```
...
Source: [OpenAI "Designing Delightful Frontends with GPT-5.4"](https://developers.openai.com/blog/designing-delightful-frontends-with-gpt-5-4) (Mar 2026) + goldband design methodology.
...
: > "$TASKS_FILE"
"$TASK_EMISSION_BIN" append   --file "$TASKS_FILE"   --phase 'design-review'   --run-id "$RUN_ID"   --branch "$BRANCH"   --commit "$COMMIT"   --id "$TASK_ID"   --priority "$PRIORITY"   --component "$COMPONENT"   --files-json "$FILES_JSON"   --effort-human "$EFFORT_HUMAN"   --effort-cc "$EFFORT_CC"   --title "$TITLE"   --source-finding "$SOURCE_FINDING"
```
...
|----------------|-------------|-----------|-------|
| [screen name]  | ~/.goldband/projects/$SLUG/designs/[folder]/[filename].png | [brief description] | [constraints from review] |
```
...
`~/.goldband/` (user config directory, not project files). The skill preamble
already writes to `~/.goldband/sessions/` and `~/.goldband/analytics/` — this is
the same pattern. The review dashboard depends on this data. Skipping this
command breaks the review readiness dashboard used by release workflows.

```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-review-log '{"skill":"plan-design-review","timestamp":"TIMESTAMP","status":"STATUS","initial_score":N,"overall_score":N,"unresolved":N,"decisions_made":N,"commit":"COMMIT"}'
```
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-learnings-log '{"skill":"plan-design-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
