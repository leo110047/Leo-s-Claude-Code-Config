...
goldband's browser surface in one document. Headless Chromium daemon, ~70+
...
B=./browse/dist/browse           # or ~/.claude/skills/goldband/browse/dist/browse
...
- **D3 — Atomic write.** Stages everything to `~/.goldband/.tmp/skillify-<spawnId>/`,
...
collisions. State at `<project>/.goldband/browse.json`.

| Workspace | State file | Port |
|-----------|-----------|------|
| `/code/project-a` | `/code/project-a/.goldband/browse.json` | random (10000–60000) |
| `/code/project-b` | `/code/project-b/.goldband/browse.json` | random (10000–60000) |
...
| `connect` | Launch headed Goldband Loop Browser with Side Panel extension |
...
| Tier | Path | When |
|------|------|------|
| **Project** | `<project>/.goldband/browser-skills/<name>/` | Project-specific skills (committed or gitignored) |
| **Global** | `~/.goldband/browser-skills/<name>/` | Per-user skills, all projects |
| **Bundled** | `<goldband-install>/browser-skills/<name>/` | Ships with goldband, read-only |
...
| **Process-side env** | `trusted: true` frontmatter passes `process.env` minus `GOLDBAND_TOKEN`. `trusted: false` (default) drops everything except a minimal allowlist (LANG, LC_ALL, TERM, TZ) and pattern-strips secrets (TOKEN/KEY/SECRET/PASSWORD, AWS_*, ANTHROPIC_*, OPENAI_*, GITHUB_*, etc.) | Untrusted (must opt in) |

`GOLDBAND_PORT` and `GOLDBAND_SKILL_TOKEN` are injected last, so a parent process
can't override them.
...
- `stageSkill(opts)` — writes files to `~/.goldband/.tmp/skillify-<spawnId>/<name>/`
...
Storage:
- Per-project: `<project>/.goldband/domain-skills/<host>.md`
- Global: `~/.goldband/domain-skills/<host>.md`
...
`$B connect` launches **Goldband Loop Browser** — a rebranded Chromium controlled by
...
```bash
$B connect              # launches Goldband Loop Browser, headed
...
"goldband" pill in the bottom-right corner so you always know which Chrome
window is being controlled.

### What "Goldband Loop Browser" means
...
work without captchas), a custom user agent, and the goldband extension
...
The Chrome extension that ships baked into Goldband Loop Browser shows a live
...
pipe text into the live Claude PTY via `window.goldbandInjectToTerminal(text)`,
...
that accepts the Bearer token OR the HttpOnly `goldband_sse` session cookie
...
| Content script | `extension/content.js` | Page overlays, `goldband` pill |
...
```bash
bin/goldband-extension    # opens chrome://extensions, copies path to clipboard
```

Or do it manually: `chrome://extensions` → toggle Developer mode → Load
unpacked → navigate to `~/.claude/skills/goldband/extension` → pin the
...
HttpOnly `goldband_sse` cookie (never valid against `/command`).
...
`~/.goldband/security/attempts.jsonl` — append-only, salted SHA-256 of source
+ domain only (no raw IP, no full request body), rotates at 10MB with 5
generations. Per-device salt at `~/.goldband/security/device-salt` (mode 0600).
...
The root token is written to `<project>/.goldband/browse.json` with chmod 600.
...
token OR a 30-minute HttpOnly `goldband_sse` cookie minted via
...
- `GOLDBAND_SECURITY_OFF=1` — emergency kill switch. Classifier stays off
...
- Classifier model cache: `~/.goldband/models/testsavant-small/` (112MB, first
  run only) plus `~/.goldband/models/deberta-v3-injection/` (721MB, only when
  ensemble enabled).
- Attack log: `~/.goldband/security/attempts.jsonl` (salted SHA-256 + domain
  only, rotates at 10MB, 5 generations).
- Per-device salt: `~/.goldband/security/device-salt` (0600).
- Session state: `~/.goldband/security/session-state.json` (cross-process,
  atomic).
...
(override via `GOLDBAND_BROWSE_MAX_HTML_BYTES`).
...
- Console: `.goldband/browse-console.log`
- Network: `.goldband/browse-network.log`
- Dialog: `.goldband/browse-dialog.log`
...
```bash
$B state save my-session         # save cookies + URLs to .goldband/browse-state-my-session.json
...
in `.goldband/browser-scout.jsonl`.
...
| **goldband browse** | **~3s** | **~100-200ms** | **0 tokens** (plain text stdout) |
| **goldband browse + codified skill** | **~3s** | **~200ms** | **0 tokens** (single skill invocation) |

In a 20-command browser session, MCP tools burn 30,000–40,000 tokens on
protocol framing alone. goldband burns zero. The codified-skill path takes a
...
goldband skips all of this. Compiled binary. Plain text in, plain text out.
...
project-tier skill at `/code/project-a/.goldband/browser-skills/foo/` shadows
the global `~/.goldband/browser-skills/foo/` only inside project-a.
...
| `BROWSE_STATE_FILE` | `.goldband/browse.json` | Path to state file |
...
| `GOLDBAND_BROWSE_MAX_HTML_BYTES` | 52428800 (50MB) | `load-html` size cap |
| `GOLDBAND_SECURITY_OFF` | unset | Emergency kill switch — disable ML classifier |
| `GOLDBAND_SECURITY_ENSEMBLE` | unset | Set to `deberta` for 3-classifier ensemble (721MB download) |
...
│   ├── tunnel-denial-log.ts     # ~/.goldband/security/attempts.jsonl writer (salted)
