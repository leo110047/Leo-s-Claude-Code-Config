// CLI tests for goldband-ios-qa-mint. Invokes the bash launcher end-to-end
...
// so the user's real ~/.goldband/ios-qa-allowlist.json is untouched.
...
const MINT_BIN = join(ROOT, 'bin', 'goldband-ios-qa-mint');
const DAEMON_BIN = join(ROOT, 'bin', 'goldband-ios-qa-daemon');

function runMint(args: string[]) {
  return spawnSync(MINT_BIN, args, { stdio: 'pipe', encoding: 'utf-8' });
}

describe('bin/goldband-ios-qa-mint launcher', () => {
  let tmpDir: string;
  let listPath: string;
  ...
  test('--help prints usage without touching allowlist', () => {
    ...
    expect(r.stdout).toContain('goldband-ios-qa-mint');
...
describe('bin/goldband-ios-qa-daemon launcher', () => {
