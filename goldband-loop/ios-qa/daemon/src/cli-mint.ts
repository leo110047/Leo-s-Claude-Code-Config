...
//
// Invoked from bin/goldband-ios-qa-mint.
...
function printHelp() {
  const help = `goldband-ios-qa-mint — manage the tailnet allowlist for remote iOS QA agents

USAGE
  goldband-ios-qa-mint grant  --remote <identity> [--capability <tier>] [--ttl <seconds>] [--note <text>]
  goldband-ios-qa-mint revoke --remote <identity>
...
EXAMPLES
  ...
  goldband-ios-qa-mint grant --remote 'tag:ci' --capability mutate --ttl 86400 --note 'nightly run'
  goldband-ios-qa-mint revoke --remote 'alice@example.com'
  goldband-ios-qa-mint list

The allowlist lives at ~/.goldband/ios-qa-allowlist.json (mode 0600). The daemon's
...
if (import.meta.main) {
  main().catch((err) => {
    process.stderr.write(`goldband-ios-qa-mint: ${(err as Error).message}\n`);
    process.exit(1);
  });
}
