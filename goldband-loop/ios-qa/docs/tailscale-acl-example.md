...
- **Auth:** Tailscale identity validation via the local `tailscaled` socket
  ...
  `~/.goldband/ios-qa-allowlist.json` is the single source of truth for who can
  do what.
...
goldband-ios-qa-mint --remote ci@example.com --capability mutate --ttl 1h
...
row to `~/.goldband/security/ios-qa-audit.jsonl`:
...
allowlisted, rate limit hit) write to `~/.goldband/security/attempts.jsonl`.
...
| `403 identity_not_allowed` | identity missing from allowlist | Owner mint: `goldband-ios-qa-mint --remote <identity>` |
