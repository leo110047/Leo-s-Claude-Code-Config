...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ios-qa","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/goldband/bin/goldband-timeline-log '{"skill":"ios-qa","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/goldband/bin/goldband-question-log '{"skill":"ios-qa","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
If `~/.goldband/ios-qa-session.json` exists and the device is still connected,
...
```bash
SESSION="$HOME/.goldband/ios-qa-session.json"
...
2. Run `swift run --package-path $GOLDBAND_HOME/ios-qa/scripts/gen-accessors-tool gen-accessors --input <source-dir>`.
...
5. Spawn the Mac-side daemon (on-demand) — `goldband-ios-qa-daemon`. Daemon
   acquires an exclusive flock on `~/.goldband/ios-qa-daemon.pid`. If another
...
`~/.goldband/security/ios-qa-audit.jsonl`.
...
Owner mints via `goldband-ios-qa-mint --remote <identity> --capability <tier>`
...
| `403 identity_not_allowed` from `/auth/mint` | identity missing from allowlist | Run `goldband-ios-qa-mint --remote <identity>` on the Mac |
