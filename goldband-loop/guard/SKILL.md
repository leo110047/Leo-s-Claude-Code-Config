...
description: |
  ...
  "full safety", "lock it down", or "maximum safety". (goldband)
triggers:
  - full safety mode
...
by the goldband setup script).

```bash
mkdir -p ~/.goldband/analytics
echo '{"skill":"guard","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
```
