...
description: |
  ...
  Proactively suggest at the end of a work week or sprint. (goldband)
allowed-tools:
  - Bash
  - Read
  - Write
...
gbrain:
  ...
  context_queries:
    - id: prior-retros
      kind: filesystem
      glob: "~/.goldband/projects/{repo_slug}/retros/*.md"
      sort: mtime_desc
      limit: 5
      render_as: "## Prior retros for this project"
    - id: recent-timeline
      kind: filesystem
      glob: "~/.goldband/projects/{repo_slug}/timeline.jsonl"
      tail: 30
      render_as: "## Recent timeline events"
    - id: recent-learnings
      kind: filesystem
      glob: "~/.goldband/projects/{repo_slug}/learnings.jsonl"
      tail: 10
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"retro","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$GOLDBAND_BIN/goldband-timeline-log '{"skill":"retro","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-question-log '{"skill":"retro","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
if [ "$_CROSS_PROJ" = "true" ]; then
  $GOLDBAND_BIN/goldband-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  $GOLDBAND_BIN/goldband-learnings-search --limit 10 2>/dev/null || true
fi
```
...
```bash
[ -f ~/.goldband/retro-context.md ] && echo "RETRO_CONTEXT_FOUND" || echo "NO_RETRO_CONTEXT"
```

If `RETRO_CONTEXT_FOUND`: read `~/.goldband/retro-context.md`. This file is user-authored and may contain meeting notes, calendar events, decisions, and other context that doesn't appear in git history. Incorporate this context into the retro narrative where relevant.
...
cat ~/.goldband/greptile-history.md 2>/dev/null || true
...
# 12. goldband skill usage telemetry (if available)
cat ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
...
**Skill Usage (if analytics exist):** Read `~/.goldband/analytics/skill-usage.jsonl` if it exists. Filter entries within the retro time window by `ts` field. Separate skill activations (no `event` field) from hook fires (`event: "hook_fire"`). Aggregate by skill name. Present as:

```
| Capability Usage | release(12) qa(8) review(5) · 3 safety hook fires |
```

If the JSONL file doesn't exist or has no entries in the window, skip the Skill Usage row.

**Eureka Moments (if logged):** Read `~/.goldband/analytics/eureka.jsonl` if it exists. Filter entries within the retro time window by `ts` field. For each eureka moment, show the skill that flagged it, the branch, and a one-line summary of the insight. Present as:
...
```
  EUREKA /office-hours (branch: project-owner/auth-rethink): "Session tokens don't need server storage — browser crypto API makes client-side JWT validation viable"
  EUREKA /plan-eng-review (branch: project-owner/cache-layer): "Redis isn't needed here — Bun's built-in LRU cache handles this workload"
```
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-learnings-log '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
```json
{
  ...
  "authors": {
    "project maintainer": { "commits": 32, "insertions": 2400, "deletions": 300, "test_ratio": 0.41, "top_area": "browse/" },
...
**Note:** Only include the `greptile` field if `~/.goldband/greptile-history.md` exists and has entries within the time window. Only include the `backlog` field if `TODOS.md` exists. Only include the `test_health` field if test files were found (command 10 returns > 0). If any has no data, omit the field entirely.
...
### Plan Completion
Check review JSONL logs for plan completion data from release runs this period:

```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
setopt +o nomatch 2>/dev/null || true  # zsh compat
eval "$($GOLDBAND_BIN/goldband-slug 2>/dev/null)"
cat ~/.goldband/projects/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
DISCOVER_BIN=""
[ -x "$GOLDBAND_BIN/goldband-global-discover" ] && DISCOVER_BIN="$GOLDBAND_BIN/goldband-global-discover"
[ -z "$DISCOVER_BIN" ] && which goldband-global-discover >/dev/null 2>&1 && DISCOVER_BIN=$(which goldband-global-discover)
[ -z "$DISCOVER_BIN" ] && [ -f bin/goldband-global-discover.ts ] && DISCOVER_BIN="bun run bin/goldband-global-discover.ts"
echo "DISCOVER_BIN: $DISCOVER_BIN"
```

If no binary is found, tell the user: "Discovery script not found. Run `bun run build` in the goldband directory to compile it." and stop.

Run the discovery:
```bash
$DISCOVER_BIN --since "<window>" --format json 2>/tmp/goldband-discover-stderr
```

Read the stderr output from `/tmp/goldband-discover-stderr` for diagnostic info. Parse the JSON output from stdout.
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  not "feat: goldband-global-discover" + "feat: /retro global template".
...
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
ls -t ~/.goldband/retros/global-*.json 2>/dev/null | head -5
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
today=$(date +%Y-%m-%d)
existing=$(ls ~/.goldband/retros/global-${today}-*.json 2>/dev/null | wc -l | tr -d ' ')
next=$((existing + 1))
```

Use the Write tool to save JSON to `~/.goldband/retros/global-${today}-${next}.json`:
...
```json
{
  ...
  "tweetable": "Week of Mar 14: 5 projects, 182 commits, 15.3k LOC | CC: 48, Codex: 8, Gemini: 3 | Focus: goldband (58%) | Streak: 52d"
}
```
...
- **Global mode:** Does NOT require being inside a git repo. Saves snapshots to `~/.goldband/retros/` (not `.context/retros/`). Gracefully skip AI tools that aren't installed. Only compare against prior global retros with the same window value. If streak hits 365d cap, display as "365+ days".
