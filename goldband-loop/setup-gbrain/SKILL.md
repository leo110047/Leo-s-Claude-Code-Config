...
description: |
  ...
  gbrain", "install gbrain", "configure gbrain for this machine". (goldband)
triggers:
  - setup gbrain
  - install gbrain
  - connect gbrain
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"setup-gbrain","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/goldband/bin/goldband-timeline-log '{"skill":"setup-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/goldband/bin/goldband-question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
`goldband_brain_sync_mode`, `goldband_brain_git`, `goldband_artifacts_remote`, and
...
>   ❌ Destructive — existing config moved to ~/.gbrain/config.json.goldband-bak-{ts}
...
**If A (Retry)**: re-run `~/.claude/skills/goldband/bin/goldband-gbrain-detect`
with `GOLDBAND_DETECT_NO_CACHE=1` (busts the 60s cache). If the new
...
```bash
BACKUP="$HOME/.gbrain/config.json.goldband-bak-$(date +%s)"
mv "$HOME/.gbrain/config.json" "$BACKUP"
# goldband default: voyage-code-3 (1024d) when VOYAGE_API_KEY is set — best for
...
```bash
~/.claude/skills/goldband/bin/goldband-gbrain-install
```
...
```bash
. ~/.claude/skills/goldband/bin/goldband-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.claude/skills/goldband/bin/goldband-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.claude/skills/goldband/bin/goldband-gbrain-supabase-provision list-orgs --json)
```
...
```bash
trap 'echo ""; echo "goldband-gbrain: interrupted. In-flight ref: $INFLIGHT_REF"; \
...
```bash
result=$(~/.claude/skills/goldband/bin/goldband-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.claude/skills/goldband/bin/goldband-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.claude/skills/goldband/bin/goldband-gbrain-supabase-provision \
...
```bash
# goldband default: voyage-code-3 (1024d) when VOYAGE_API_KEY is set — code
...
**4c. Verify via goldband-gbrain-mcp-verify.** Run the helper; capture the
classified JSON output:

```bash
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.claude/skills/goldband/bin/goldband-gbrain-mcp-verify "$MCP_URL")
...
- `URL_FORM_SUPPORTED` (`true|false`) — passed to `goldband-artifacts-init` in
...
```bash
~/.claude/skills/goldband/bin/goldband-gbrain-install || exit $?
...
# goldband default for local code-search PGLite: voyage-code-3 (1024d) when
...
When A was picked, Step 3 already ran (via goldband-gbrain-install) and Step 4
...
("Migration didn't complete in 3 minutes — another goldband session may be
...
```bash
current_tier=$(~/.claude/skills/goldband/bin/goldband-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.claude/skills/goldband/bin/goldband-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
Separate AskUserQuestion: "Also sync your goldband artifacts (CEO plans,
...
`goldband-artifacts-$USER` (private), and writes the canonical HTTPS URL to
`~/.goldband-artifacts-remote.txt`. Pass `--url-form-supported` from Step 4c's
verify output (Path 4) or `false` (Paths 1/2/3 — local mode doesn't probe):

```bash
URL_FORM=${URL_FORM_SUPPORTED:-false}
~/.claude/skills/goldband/bin/goldband-artifacts-init --url-form-supported "$URL_FORM"
~/.claude/skills/goldband/bin/goldband-config set artifacts_sync_mode artifacts-only
# or "full" if user picked yes-full
```

`goldband-artifacts-init` always prints a "Send this to your brain admin" block
...
In remote mode, the local `goldband-gbrain-source-wireup` helper does NOT run
...
any gbrain client. The helper creates a `git worktree` of `~/.goldband/`,
...
try:
    ...
    pass
")
~/.claude/skills/goldband/bin/goldband-gbrain-source-wireup --strict \
  ${GBRAIN_URL:+--database-url "$GBRAIN_URL"}
```

`--strict` exits non-zero on missing prereqs (gbrain not installed, < 0.18.0,
or no `~/.goldband/.git` yet) so the user sees the failure rather than silently
...
this machine's transcripts indexed, they pull from your `goldband-artifacts-$USER`
repo (set up in Step 7) on whatever schedule they prefer. Set
`goldband-config set transcript_ingest_mode off` and continue to Step 8.
...
>
> What you get after this: every goldband skill auto-loads recent salience
...
> it in prior commits. Use `goldband-transcript-prune` to delete in bulk;
...
After answer:
```bash
~/.claude/skills/goldband/bin/goldband-config set transcript_ingest_mode <choice>
~/.claude/skills/goldband/bin/goldband-gbrain-sync --full --no-brain-sync
```
...
hook runs `goldband-gbrain-sync --incremental --quiet` on every skill
...
- Artifacts repo: {goldband_artifacts_remote URL or "none"}
...
<!-- goldband-gbrain-search-guidance:start -->

GBrain is set up and synced on this machine. The agent should prefer gbrain
over Grep when the question is semantic or when you don't know the exact
identifier yet. Two indexed corpora available via the `gbrain` CLI:
- This repo's code (registered as `goldband-code-<repo>` source).
- `~/.goldband/` curated memory (registered as `goldband-brain-<user>` source via
...
- "What did we decide last time?" / past plans, retros, learnings:
    `gbrain search "<terms>" --source goldband-brain-<user>`

Grep is still right for known exact strings, regex, multiline patterns, and
file globs. The brain auto-syncs incrementally on every goldband skill start.
Run `/sync-gbrain` to force-refresh, `/sync-gbrain --full` for full reindex.

<!-- goldband-gbrain-search-guidance:end -->
```
...
```bash
~/.claude/skills/goldband/bin/goldband-gbrain-detect 2>/dev/null || true
~/.claude/skills/goldband/bin/goldband-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.claude/skills/goldband/bin/goldband-config get artifacts_sync_mode 2>/dev/null || echo "off"
[ -f ~/.goldband/.gbrain-sync-state.json ] && cat ~/.goldband/.gbrain-sync-state.json || echo "{}"
```
...
gbrain status: GREEN  (mode: remote-http)
  ...
  Artifacts repo .. OK   {goldband_artifacts_remote URL}
...
- If user picked B (No): `N/A declined at Step 4d` — `goldband-config set local_code_index_offered true` to silence future migration notices.

The **Transcripts** row changed in v1.34.0.0: in remote-http mode,
goldband-memory-ingest now persists staged transcripts to
`~/.goldband/transcripts/run-<pid>-<ts>/` and goldband-brain-sync pushes them
...
- **Concurrent-run lock.** At skill start, `mkdir ~/.goldband/.setup-gbrain.lock.d`
  (atomic). If the mkdir fails, abort with: "Another `/setup-gbrain` instance
  is running. Wait for it, or `rm -rf ~/.goldband/.setup-gbrain.lock.d` if
