# goldband memory ingest — what it does, what stays local, what you can do with it
...
| Eureka log | `eureka` | `~/.goldband/analytics/eureka.jsonl` | Medium — your insights, often non-secret |
| Project learnings | `learning` | `~/.goldband/projects/<slug>/learnings.jsonl` | Medium |
| Project timeline | `timeline` | `~/.goldband/projects/<slug>/timeline.jsonl` | Low |
| CEO plans | `ceo-plan` | `~/.goldband/projects/<slug>/ceo-plans/*.md` | Medium |
| Design docs | `design-doc` | `~/.goldband/projects/<slug>/*-design-*.md` | Medium |
| Retros | `retro` | `~/.goldband/projects/<slug>/retros/*.md` | Medium |
| Builder profile | `builder-profile-entry` | `~/.goldband/builder-profile.jsonl` | Low |

## What stays local

- **State files** (`~/.goldband/.gbrain-sync-state.json`,
  `~/.goldband/.transcript-ingest-state.json`,
  `~/.goldband/.gbrain-engine-cache.json`,
  `~/.goldband/.gbrain-errors.jsonl`) are local-only per ED1 (state file
...
The cross-machine secret boundary is `goldband-brain-sync` (the git push
...
```bash
goldband-memory-ingest --bulk --scan-secrets
# or
GOLDBAND_MEMORY_INGEST_SCAN_SECRETS=1 goldband-memory-ingest --bulk
```
...
was skipped via `bun run bin/goldband-memory-ingest.ts --probe` (which
...
- **Supabase configured:** code + transcripts go to Supabase Storage
  ...
  brain-linked git repo via `goldband-brain-sync`.
...
- **Bulk-delete by criteria** (V1.0.1 follow-up — `goldband-transcript-prune`
  helper). For V1.0, use `gbrain delete_page <slug>` per-page or write
  a small loop over `gbrain list_pages` output.

- **Disable entirely:**
  ```bash
  goldband-config set transcript_ingest_mode off
  goldband-config set gbrain_context_load off  # also disables retrieval
  ```

## How the agent uses it

At every goldband skill start, the preamble runs
`goldband-brain-context-load` which:
...
- **Salience block is empty** — your transcripts may not be ingested
  yet. Run `goldband-gbrain-sync --full` to do a full pass.
...
- **A page has stale or wrong content** — `gbrain delete_page <slug>`,
  then re-run `goldband-gbrain-sync --incremental` to re-ingest from
  source if the source file is still on disk and unchanged.

## Privacy + audit

- Every `secretScanFile` finding is logged to stderr at ingest time.
- Every gbrain put/delete is logged to `~/.goldband/.gbrain-errors.jsonl`
  with `{ts, op, duration_ms, outcome}` for forensic tracing.
- `~/.goldband/.gbrain-engine-cache.json` shows which storage tier is
...
1. Verifies the URL + token via `goldband-gbrain-mcp-verify`. Three failure
...
3. Skips local install, local doctor, transcript ingest, and federated
   ...
   doesn't install.
4. Optionally provisions a `goldband-artifacts-$USER` private repo on
...
`goldband-artifacts-init` always prints the `gbrain sources add` command
