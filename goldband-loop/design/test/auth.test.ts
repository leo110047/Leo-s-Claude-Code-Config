 ...
 *   - prefer ~/.goldband/openai.json over OPENAI_API_KEY
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "goldband-design-auth-"));
...
describe("resolveApiKeyInfo", () => {
  test("uses ~/.goldband/openai.json before OPENAI_API_KEY", () => {
    ...
    expect(describeApiKeySource(resolution!)).toBe("~/.goldband/openai.json");
