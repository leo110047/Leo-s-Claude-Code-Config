...
export const CMDLINE_MARKER = "goldband-design-daemon";
...
export function resolveStateFilePath(): string {
  ...
  try {
    ...
    if (root) return path.join(root, ".goldband", "design.json");
  } catch {
    // not in a git repo — fall through
  }
  return path.join(process.cwd(), ".goldband", "design.json");
}
...
export function resolveDaemonLogPath(): string {
  return path.join(os.homedir(), ".goldband", "design-daemon.log");
}

export function resolveStartupLogPath(): string {
  return path.join(os.homedir(), ".goldband", "design-daemon-startup.log");
}

/**
 * Read the goldband version both client and daemon should agree on. Looks
