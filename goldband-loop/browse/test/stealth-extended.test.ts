 ...
 * 2. GOLDBAND_STEALTH=extended adds EXTENDED_STEALTH_SCRIPT on top.
...
beforeEach(() => {
  originalEnv = process.env.GOLDBAND_STEALTH;
});

afterEach(() => {
  if (originalEnv === undefined) delete process.env.GOLDBAND_STEALTH;
  else process.env.GOLDBAND_STEALTH = originalEnv;
});

describe('extended stealth — opt-in mode flag', () => {
  test('default mode is OFF (consistency-first contract)', () => {
    delete process.env.GOLDBAND_STEALTH;
    expect(isExtendedStealthEnabled()).toBe(false);
  });

  test('GOLDBAND_STEALTH=extended enables extended mode', () => {
    process.env.GOLDBAND_STEALTH = 'extended';
    expect(isExtendedStealthEnabled()).toBe(true);
  });

  test('GOLDBAND_STEALTH=1 also enables (env-style boolean)', () => {
    process.env.GOLDBAND_STEALTH = '1';
    expect(isExtendedStealthEnabled()).toBe(true);
  });

  test('GOLDBAND_STEALTH=anything-else does NOT enable', () => {
    process.env.GOLDBAND_STEALTH = 'verbose';
    expect(isExtendedStealthEnabled()).toBe(false);
  });
});
