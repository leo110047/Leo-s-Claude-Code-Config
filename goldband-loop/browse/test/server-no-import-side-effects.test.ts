 ...
 * were registered. The subprocess uses HOME=tmp + GOLDBAND_HOME=tmp so any
...
describe('server.ts module import has no auto-start side effects', () => {
  test('importing server.ts does not bind Bun.serve, register signal handlers, or write state', async () => {
    ...
    const tmpGoldband = path.join(tmpHome, '.goldband');
...
// Check that the goldband home directory wasn't populated as a side effect.
let goldbandPopulated = false;
try {
  const entries = fs.readdirSync(${JSON.stringify(tmpGoldband)});
  goldbandPopulated = entries.length > 0;
} catch {
...
`;
    ...
    const proc = Bun.spawn(['bun', '-e', childScript], {
      env: {
        ...
        // happens) won't crawl the host's real .goldband/.
        BROWSE_STATE_FILE: path.join(tmpGoldband, 'browse.json'),
      },
      stdout: 'pipe',
      stderr: 'pipe',
    });
    ...
    // goldband home should remain empty — initRegistry/initAuditLog/etc. side
    ...
    expect(snapshot.goldbandPopulated).toBe(false);
