 ...
 * never touch the user's real ~/.goldband/ tree. The contract under test:
 *
 *   stageSkill    → writes files into ~/.goldband/.tmp/skillify-<spawnId>/<name>/
...
beforeEach(() => {
  ...
  // Synthetic tmp root keeps tests off the real ~/.goldband/.tmp/.
  stagingTmpRoot = path.join(tmpRoot, 'home', '.goldband', '.tmp');
});
