...
describe('security-classifier: missing claude CLI degraded path', () => {
  ...
  let origGoldbandClaudeBin: string | undefined;
  let origClaudeBin: string | undefined;

  beforeEach(() => {
    origPath = process.env.PATH;
    origGoldbandClaudeBin = process.env.GOLDBAND_CLAUDE_BIN;
    origClaudeBin = process.env.CLAUDE_BIN;
    // Force resolveClaudeCommand() to fail: clear PATH AND override env vars
    // (resolveClaudeCommand in browse/src/claude-bin.ts honors GOLDBAND_CLAUDE_BIN
    // and CLAUDE_BIN before falling back to Bun.which(PATH)).
    process.env.PATH = '/nonexistent';
    delete process.env.GOLDBAND_CLAUDE_BIN;
  ...
  afterEach(() => {
    ...
    if (origGoldbandClaudeBin !== undefined) process.env.GOLDBAND_CLAUDE_BIN = origGoldbandClaudeBin;
