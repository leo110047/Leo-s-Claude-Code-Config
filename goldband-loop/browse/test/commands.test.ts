...
describe('CLI server script resolution', () => {
  test('prefers adjacent browse/src/server.ts for compiled project installs', () => {
    const root = fs.mkdtempSync('/tmp/goldband-cli-');
    const execPath = path.join(root, '.claude/skills/goldband/browse/dist/browse');
    const serverPath = path.join(root, '.claude/skills/goldband/browse/src/server.ts');
