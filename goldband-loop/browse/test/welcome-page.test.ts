...
describe('welcome page served via HTTP', () => {
  ...
  test('has Goldband Loop Browser title and branding', () => {
    expect(html).toContain('<title>Goldband Loop Browser</title>');
    expect(html).toContain('Goldband Loop Browser');
  });
  ...
  test('hides sidebar prompt when extension is detected', () => {
    ...
    expect(html).toContain("'goldband-extension-ready'");
  ...
  test('does NOT auto-hide based on extension detection alone', () => {
    ...
    expect(html).not.toContain('goldband-status-pill');
