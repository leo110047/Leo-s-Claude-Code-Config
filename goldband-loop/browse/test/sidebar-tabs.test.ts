...
describe('sidepanel.js: chat helpers ripped, terminal-injection helper survives', () => {
  ...
  test('Cleanup runs through the live PTY (no /sidebar-command POST)', () => {
    ...
    // PTY via goldbandInjectToTerminal. The dead code path was a POST to
    // /sidebar-command which kicked off a fresh claude -p subprocess.
    const cleanup = JS.slice(JS.indexOf('async function runCleanup'));
    expect(cleanup).toContain('window.goldbandInjectToTerminal');
  ...
  test('Inspector "Send to Code" routes through the live PTY', () => {
    ...
    expect(sendBtn).toContain('window.goldbandInjectToTerminal');
  ...
  test('updateConnection no longer kicks off chat / tab polling', () => {
    ...
    expect(update).toContain('window.goldbandServerPort');
    expect(update).toContain('window.goldbandAuthToken');
  });
});

describe('sidepanel-terminal.js: eager auto-connect + injection API', () => {
  test('Exposes window.goldbandInjectToTerminal for cross-pane use', () => {
    expect(TERM_JS).toContain('window.goldbandInjectToTerminal');
    // Returns false when no live session, true when bytes go out.
    const inject = TERM_JS.slice(TERM_JS.indexOf('window.goldbandInjectToTerminal'));
  ...
  test('Repaint hook fires when Terminal pane becomes visible', () => {
    // The chat-tab rip removed goldband:primary-tab-changed; we use a
...
describe('files: sidebar-agent.ts and its tests are deleted', () => {
  ...
  test('sidebar-agent test files are gone', () => {
    ...
    expect(fs.existsSync(path.join(import.meta.dir, 'sidebar-integration.test.ts'))).toBe(false);
    expect(fs.existsSync(path.join(import.meta.dir, 'sidebar-security.test.ts'))).toBe(false);
    expect(fs.existsSync(path.join(import.meta.dir, 'sidebar-ux.test.ts'))).toBe(false);
    expect(fs.existsSync(path.join(import.meta.dir, 'security-source-contracts.test.ts'))).toBe(false);
    expect(fs.existsSync(path.join(import.meta.dir, 'security-sidepanel-dom.test.ts'))).toBe(false);
  });
});
