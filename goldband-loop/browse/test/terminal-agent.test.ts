...
describe('pty-session-cookie: mint/validate/revoke', () => {
  ...
  test('extractPtyCookie reads goldband_pty from a Cookie header', () => {
    const { token } = mintPtySessionToken();
    const req = new Request('http://127.0.0.1/ws', {
      headers: { 'cookie': `othercookie=foo; goldband_pty=${token}; baz=qux` },
    });
...
describe('Source-level guard: /health does NOT surface ptyToken', () => {
  test('/health response body does not include ptyToken', () => {
    ...
    expect(slice).not.toContain('goldband_pty');
...
describe('Source-level guard: terminal-agent', () => {
  ...
  test('validates the session token against an in-memory token set', () => {
    ...
    // Cookie goldband_pty (fallback). Both verify against validTokens.
    expect(wsHandler).toContain('sec-websocket-protocol');
    expect(wsHandler).toContain('goldband_pty');
    expect(wsHandler).toContain('validTokens.has');
  });

  test('Sec-WebSocket-Protocol auth: strips goldband-pty. prefix and echoes back', () => {
    const wsHandler = AGENT_SRC.slice(AGENT_SRC.indexOf("if (url.pathname === '/ws')"));
    // Browsers send `Sec-WebSocket-Protocol: goldband-pty.<token>`. The agent
    ...
    expect(wsHandler).toContain("'goldband-pty.'");
    expect(wsHandler).toContain('Sec-WebSocket-Protocol');
  ...
  test('lazy spawn: claude PTY is spawned in message handler, not on upgrade', () => {
    ...
    // Spawn must be reachable from the message handler (lazy on first byte).
    // v1.44 routes both the explicit `{type:"start"}` frame and the legacy
    // first-binary-byte path through maybeSpawnPty for parity.
    ...
    const maybeSpawnPty = AGENT_SRC.slice(AGENT_SRC.indexOf('function maybeSpawnPty'));
    expect(maybeSpawnPty).toContain('spawnClaude(');
  });
  ...
  test('claude is spawned with --append-system-prompt tab-awareness hint', () => {
    ...
    // those are the two affordances that distinguish a goldband-PTY claude
