...
beforeEach(() => {
  ...
  tiers = {
    project: path.join(tmpRoot, 'project', '.goldband', 'browser-skills'),
    global: path.join(tmpRoot, 'home', '.goldband', 'browser-skills'),
    bundled: path.join(tmpRoot, 'goldband-install', 'browser-skills'),
  };
...
describe('buildSpawnEnv', () => {
  ...
  beforeEach(() => {
    ...
    process.env.GOLDBAND_TOKEN = 'root-token';
  ...
  it('untrusted: drops $HOME and secrets', () => {
    ...
    expect(env.GOLDBAND_TOKEN).toBeUndefined();
  });
  ...
  it('untrusted: injects GOLDBAND_PORT + GOLDBAND_SKILL_TOKEN', () => {
    const env = buildSpawnEnv({ trusted: false, port: 1234, skillToken: 'tok-xyz' });
    expect(env.GOLDBAND_PORT).toBe('1234');
    expect(env.GOLDBAND_SKILL_TOKEN).toBe('tok-xyz');
  });
  ...
  it('trusted: still strips GOLDBAND_TOKEN (defense in depth)', () => {
    const env = buildSpawnEnv({ trusted: true, port: 1234, skillToken: 'tok' });
    expect(env.GOLDBAND_TOKEN).toBeUndefined();
  });
  ...
  it('GOLDBAND_PORT/GOLDBAND_SKILL_TOKEN can never be overridden by parent env', () => {
    process.env.GOLDBAND_PORT = '99999'; // attacker-set
    process.env.GOLDBAND_SKILL_TOKEN = 'attacker-tok';
    const env = buildSpawnEnv({ trusted: true, port: 1234, skillToken: 'real-tok' });
    expect(env.GOLDBAND_PORT).toBe('1234');
    expect(env.GOLDBAND_SKILL_TOKEN).toBe('real-tok');
  });
});
...
describe.skipIf(SKIP_SPAWN)('spawnSkill: lifecycle', () => {
  ...
  it('untrusted spawn: GOLDBAND_SKILL_TOKEN visible, root env scrubbed', async () => {
    const dir = makeSkillDir(tiers.bundled, 'env-probe',
      'name: env-probe\nhost: x.com',  // trusted defaults to false
      `console.log(JSON.stringify({
        port: process.env.GOLDBAND_PORT,
        token: process.env.GOLDBAND_SKILL_TOKEN,
        home: process.env.HOME ?? null,
        gh: process.env.GITHUB_TOKEN ?? null,
        goldband: process.env.GOLDBAND_TOKEN ?? null,
      }));`,
    );
    const origEnv = { ...process.env };
    process.env.GITHUB_TOKEN = 'gh-secret';
    process.env.GOLDBAND_TOKEN = 'root';
    try {
      ...
      expect(parsed.goldband).toBeNull();
    } finally {
