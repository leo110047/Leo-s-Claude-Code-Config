...
describe('browse-client', () => {
  ...
  beforeEach(async () => {
    ...
    for (const k of ['GOLDBAND_PORT', 'GOLDBAND_SKILL_TOKEN', 'BROWSE_STATE_FILE', 'BROWSE_TAB']) {
  ...
  describe('resolveBrowseAuth', () => {
    it('uses GOLDBAND_PORT + GOLDBAND_SKILL_TOKEN env when present', () => {
      process.env.GOLDBAND_PORT = String(server.port);
      process.env.GOLDBAND_SKILL_TOKEN = 'scoped-token';
    ...
    it('rejects GOLDBAND_PORT env values with trailing characters', () => {
      process.env.GOLDBAND_PORT = `${server.port}abc`;
      process.env.GOLDBAND_SKILL_TOKEN = 'scoped-token';
