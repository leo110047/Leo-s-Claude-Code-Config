 ...
 *
 * Piggybacks on ~/.goldband/analytics/skill-usage.jsonl pattern (existing
 * goldband telemetry). Hostname + aggregate counters only; no body content,
 * no agent text, no command args. Respects the user's telemetry tier
 * setting (off | anonymous | community) via goldband-config.
 *
...
function analyticsDir(): string {
  return path.join(goldbandHome(), 'analytics');
}
...
function isDisabled(): boolean {
  ...
  if (process.env.GOLDBAND_TELEMETRY_OFF === '1') {
    telemetryDisabled = true;
    return true;
  }
  // Conservative default: telemetry ON unless explicitly off. Users opt out via
  // goldband-config set telemetry off (preamble reads this; we trust the env hint).
