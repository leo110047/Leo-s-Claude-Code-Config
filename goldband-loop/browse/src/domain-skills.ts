 ...
 *
 * Scope:
 *   - per-project: ~/.goldband/projects/<slug>/learnings.jsonl
 *   - global:      ~/.goldband/global-domain-skills.jsonl
 *
...
function globalFile(): string {
  return path.join(goldbandHome(), 'global-domain-skills.jsonl');
}

function projectFile(slug: string): string {
  return path.join(goldbandHome(), 'projects', slug, 'learnings.jsonl');
}
