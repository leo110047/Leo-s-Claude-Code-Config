 ...
 *
 * The session cookie (goldband_picker) is isolated from the scoped token system.
...
/** Extract session ID from the goldband_picker cookie. */
function getSessionFromCookie(req: Request): string | null {
  const cookie = req.headers.get('cookie');
  if (!cookie) return null;
  const match = cookie.match(/goldband_picker=([^;]+)/);
...
export async function handleCookiePickerRoute(
  ...
  try {
    ...
    if (pathname === '/cookie-picker' && req.method === 'GET') {
      ...
      if (code) {
        ...
        return new Response(null, {
          ...
          headers: {
            ...
            'Set-Cookie': `goldband_picker=${session}; HttpOnly; SameSite=Strict; Path=/cookie-picker; Max-Age=3600`,
      ...
      return new Response('Access denied. Open the cookie picker from goldband.', {
        status: 403,
        headers: { 'Content-Type': 'text/plain' },
      });
    }
      ...
      if (result.cookies.length === 0 && result.failed > 0 && hasV20Cookies(browser, selectedProfile)) {
        ...
        try {
          ...
          return jsonResponse({
            ...
            domainCounts: {},
            message: `Cookies use App-Bound Encryption (v20). Close ${browser}, retry, or use /goldband-open-goldband-browser to browse with your real browser directly.`,
            code: 'v20_encryption',
          }, { port });
        }
      }
