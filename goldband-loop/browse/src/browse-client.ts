 ...
 * goldband daemon over loopback HTTP.
 *
 * Distribution model:
 ...
 *
 * Auth resolution:
 *   1. GOLDBAND_PORT + GOLDBAND_SKILL_TOKEN env vars (set by `$B skill run` when
 ...
 *   2. State file fallback: read `BROWSE_STATE_FILE` env or `<git-root>/.goldband/browse.json`
...
export interface BrowseClientOptions {
  ...
  timeoutMs?: number;
  /** Override state-file path. Default: BROWSE_STATE_FILE env or <git-root>/.goldband/browse.json. */
  stateFile?: string;
}
...
export function resolveBrowseAuth(opts: BrowseClientOptions = {}): ResolvedAuth {
  ...
  const envPort = process.env.GOLDBAND_PORT;
  const envToken = process.env.GOLDBAND_SKILL_TOKEN;
  if (envPort && envToken) {
  ...
  throw new Error(
    ...
    '(sets GOLDBAND_PORT + GOLDBAND_SKILL_TOKEN) or run from a project with a live daemon ' +
    '(.goldband/browse.json must exist).'
  );
}
...
function defaultStateFile(): string | null {
  try {
    ...
    return path.join(base, '.goldband', 'browse.json');
  } catch {
    return path.join(process.cwd(), '.goldband', 'browse.json');
  }
}
