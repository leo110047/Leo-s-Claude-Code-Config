 ...
 *
 * Cross-process state lives at ~/.goldband/security/session-state.json
...
function goldbandHome(): string {
  return (
    process.env.GOLDBAND_HOME ||
    path.join(process.env.HOME || os.homedir(), '.goldband')
  );
}

function securityDir(): string {
  return path.join(goldbandHome(), 'security');
}

function attemptsLog(): string {
  return path.join(securityDir(), 'attempts.jsonl');
}

function saltFile(): string {
  return path.join(securityDir(), 'device-salt');
}

/**
 * Read-or-create the per-device salt used for payload hashing. Salt lives at
 * ~/.goldband/security/device-salt (0600). Random per-device, prevents rainbow
...
function getDeviceSalt(): string {
  ...
  try {
    mkdirSecure(securityDir());
  } catch {}
  cachedSalt = randomBytes(16).toString('hex');
  try {
    writeSecureFile(file, cachedSalt);
  } catch {
...
/**
 * Try to locate the goldband-telemetry-log binary. Resolution order matches
 * the existing skill preamble pattern (never relies on PATH — packaged
 * binary layouts can break that).
 *
 * Order:
 *  1. ~/.claude/skills/goldband/bin/goldband-telemetry-log  (global install)
 *  2. .claude/skills/goldband/bin/goldband-telemetry-log    (symlinked dev)
 *  3. bin/goldband-telemetry-log                          (in-repo dev)
 */
function findTelemetryBinary(): string | null {
  const candidates = [
    path.join(os.homedir(), '.claude', 'skills', 'goldband', 'bin', 'goldband-telemetry-log'),
    path.resolve(process.cwd(), '.claude', 'skills', 'goldband', 'bin', 'goldband-telemetry-log'),
    path.resolve(process.cwd(), 'bin', 'goldband-telemetry-log'),
  ];
 ...
 * GOLDBAND_*_BIN override pattern from `browse/src/claude-bin.ts:resolveClaudeCommand`
 * (introduced in v1.24.0.0 #1252) so users on WSL/MSYS2/non-default Git Bash
 * installs can redirect.
 *
 * Override precedence:
 *   1. GOLDBAND_BASH_BIN (or BASH_BIN) — absolute path or PATH-resolvable command.
...
export function resolveBashBinary(env: NodeJS.ProcessEnv = process.env): string | null {
  ...
  const override = (env.GOLDBAND_BASH_BIN ?? env.BASH_BIN)?.trim();
  if (override) {
 ...
 * Win32: wraps in bash explicitly. `goldband-telemetry-log` is a shell script
 ...
 * Bun.which + GOLDBAND_*_BIN override pattern as claude-bin.ts.
 *
 * Returns null when bash can't be resolved on Windows (rare — Git Bash ships
 * with the standard goldband install path). Caller skips spawn; the local
...
/**
 * Fire-and-forget subprocess invocation of goldband-telemetry-log with the
 ...
 * goldband-telemetry-log (which respects the user's telemetry tier setting).
...
export function logAttempt(record: AttemptRecord): boolean {
  ...
  try {
    ...
    appendSecureFile(attemptsLog(), line);
    return true;
  } catch (err) {
...
function stateFile(): string {
  return path.join(securityDir(), 'session-state.json');
}
...
// per-tab file under ~/.goldband/security/decisions/, sidebar-agent polls
