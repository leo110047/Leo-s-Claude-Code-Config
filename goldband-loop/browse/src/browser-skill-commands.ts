 ...
 * and passes it via GOLDBAND_SKILL_TOKEN. The skill never sees the daemon root
 ...
 *    GOLDBAND_PORT and GOLDBAND_SKILL_TOKEN are always set.
 ...
 * Includes: minimal PATH, locale, terminal type. Skills get GOLDBAND_PORT +
 * GOLDBAND_SKILL_TOKEN injected separately.
 */
...
export function buildSpawnEnv(opts: BuildEnvOptions): Record<string, string> {
  ...
  if (opts.trusted) {
    ...
    for (const [k, v] of Object.entries(process.env)) {
      ...
      if (k === 'GOLDBAND_TOKEN') continue; // never propagate root token
      out[k] = v;
    }
  ...
  // intentionally — e.g. an internal-tool skill — but we still strip GOLDBAND_TOKEN
  // above.)
  if (!opts.trusted) {
  ...
  out.GOLDBAND_PORT = String(opts.port);
  out.GOLDBAND_SKILL_TOKEN = opts.skillToken;
