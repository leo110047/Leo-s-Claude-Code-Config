 ...
 *
 * To upgrade: bump this string, run `GOLDBAND_BENCH_ENSEMBLE=1 bun test
 ...
 * layout at ~/.goldband/models/testsavant-small/ on first use.
 *
 ...
 */
const MODELS_DIR = path.join(os.homedir(), '.goldband', 'models');
...
// GOLDBAND_SECURITY_ENSEMBLE=deberta. Not forced on every install because
...
function isDebertaEnabled(): boolean {
  const setting = (process.env.GOLDBAND_SECURITY_ENSEMBLE ?? '').toLowerCase();
...
export function loadTestsavant(onProgress?: (msg: string) => void): Promise<void> {
  if (process.env.GOLDBAND_SECURITY_OFF === '1') {
...
export async function checkTranscript(params: {
  ...
  return new Promise((resolve) => {
    ...
    // repo with a prompt-injection-defense CLAUDE.md (like goldband itself),
    ...
    // Env var GOLDBAND_HAIKU_TIMEOUT_MS (milliseconds) overrides for benches
    // that want a different budget.
    const timeoutMs = process.env.GOLDBAND_HAIKU_TIMEOUT_MS
      ? Number(process.env.GOLDBAND_HAIKU_TIMEOUT_MS)
      : 45000;
    setTimeout(() => {
