/**
 * goldband browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.goldband/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.goldband/browse-{console,network,dialog}.log
 ...
 * `/Users/example/.claude/plans/system-instruction-you-are-working-swirling-fountain.md`
 * Part 1.
 */
export interface ServerConfig {
   ...
   * (no token presented). Returning a Response short-circuits goldband
   * dispatch; returning null falls through.
   */
  beforeRoute?: (req: Request, surface: Surface, auth: TokenInfo | null) => Promise<Response | null>;
  /**
   * Whether goldband owns the lifecycle of the terminal-agent process and its
   ...
   *      matched sibling goldband sessions on the same host — see
   ...
   *
   * This is correct for goldband's CLI path, which spawns `terminal-agent.ts` as
...
/**
 * Build a ServerConfig-shaped object from process.env. Used by goldband's
...
/**
 * Read ngrok authtoken from env var, ~/.goldband/ngrok.env, or ngrok's native
...
function resolveNgrokAuthtoken(): string | null {
  ...
  const ngrokEnvPath = path.join(home, '.goldband', 'ngrok.env');
...
function generateHelpText(): string {
  ...
  const lines = ['goldband browse — headless browser for AI agents', '', 'Commands:'];
...
// so this branch is the normal path for /open-goldband-browser.
...
if (BROWSE_PARENT_PID > 0 && !IS_HEADED_WATCHDOG) {
  ...
  setInterval(() => {
    try {
      ...
      //    /pair-agent or /open-goldband-browser sessions.
...
// fighting with goldband's. CLI path is unchanged.
if (import.meta.main) {
...
export function buildFetchHandler(cfg: ServerConfig): ServerHandle {
  ...
  const AGENT_WATCHDOG_TICK_MS = parseInt(
    process.env.GOLDBAND_AGENT_WATCHDOG_TICK_MS || '60000',
    10,
  );
  ...
  // block goldband shutdown — defensive symmetry with the safeUnlinkQuiet /
  ...
  const makeFetchHandler = (surface: Surface) => async (req: Request): Promise<Response> => {
    ...
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      // Welcome page — served when Goldband Loop Browser launches in headed mode
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          // Gate GOLDBAND_SLUG on a strict regex BEFORE interpolating it into
          // the filesystem path. Without this, a slug like "../../etc/passwd"
          // would resolve to ~/.goldband/projects/../../etc/passwd/... — path
          ...
          const rawSlug = process.env.GOLDBAND_SLUG || 'unknown';
          const slug = /^[a-z0-9_-]+$/.test(rawSlug) ? rawSlug : 'unknown';
          const homeDir = process.env.HOME || process.env.USERPROFILE || '/tmp';
          const projectWelcome = `${homeDir}/.goldband/projects/${slug}/designs/welcome-page-20260331/finalized.html`;
          if (fs.existsSync(projectWelcome)) return projectWelcome;
          // Fallback: built-in welcome page from goldband install.  Reject
          // SKILL_ROOT values containing '..' for the same defense-in-depth
          // reason as the GOLDBAND_SLUG regex above.  Not exploitable today
          // (env set at install time), but the gate is one check.
          const rawSkillRoot = process.env.GOLDBAND_SKILL_ROOT || `${homeDir}/.claude/skills/goldband`;
        ...
        return new Response(
          `<!DOCTYPE html><html><head><title>Goldband Loop Browser</title>
          ...
          <body><div class="msg"><div class="gold">◈</div><p>Goldband Loop Browser ready.</p><p style="font-size:.85em">Waiting for commands from Claude Code.</p></div></body></html>`,
      ...
      // extension's goldbandInjectToTerminal callers. The extension routes
      ...
      if (url.pathname === '/tunnel/start' && req.method === 'POST') {
        ...
        // 1) Resolve ngrok authtoken from env / .goldband / native config
...
export async function start() {
  ...
  if (process.env.BROWSE_TUNNEL === '1') {
    const authtoken = resolveNgrokAuthtoken();
    if (!authtoken) {
      console.error('[browse] BROWSE_TUNNEL=1 but no NGROK_AUTHTOKEN found. Set it via env var or ~/.goldband/ngrok.env');
    } else {
