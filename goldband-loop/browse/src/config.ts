 ...
 *   2. git rev-parse --show-toplevel → projectDir/.goldband/
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    projectDir = path.dirname(stateDir); // parent of .goldband/
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path.join(projectDir, '.goldband');
...
/**
 * Create the .goldband/ state directory if it doesn't exist.
...
export function ensureStateDir(config: BrowseConfig): void {
  ...
  // Ensure .goldband/ is in the project's .gitignore
  const gitignorePath = path.join(config.projectDir, '.gitignore');
  try {
    const content = fs.readFileSync(gitignorePath, 'utf-8');
    if (!content.match(/^\.goldband\/?$/m)) {
      const separator = content.endsWith('\n') ? '' : '\n';
      fs.appendFileSync(gitignorePath, `${separator}.goldband/\n`);
    }
  } catch (err: any) {
    if (err.code !== 'ENOENT') {
...
/**
 * Resolve the goldband home directory.
 *
 * Honors the existing convention used by telemetry.ts and domain-skills.ts:
 *   1. GOLDBAND_HOME env (explicit override)
