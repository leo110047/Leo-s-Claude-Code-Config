 ...
 *   project:  <project>/.goldband/browser-skills/<name>/
 *   global:   ~/.goldband/browser-skills/<name>/
 *   bundled:  <goldband-install>/browser-skills/<name>/   (read-only, ships with goldband)
 *
...
export function defaultTierPaths(opts: { projectRoot?: string; home?: string; bundledRoot?: string } = {}): TierPaths {
  ...
  return {
    project: projectRoot ? path.join(projectRoot, '.goldband', 'browser-skills') : null,
    global: path.join(home, '.goldband', 'browser-skills'),
...
function detectBundledRoot(): string {
  // The browse binary lives at <goldband-install>/browse/dist/browse.
  // The bundled browser-skills/ dir is a sibling of browse/ (i.e. <goldband-install>/browser-skills/).
  ...
  // browse/src/browser-skills.ts → ../../  (the goldband root).
 ...
 *
 * Cannot tombstone bundled skills — they ship with goldband and are read-only.
 * To remove a bundled skill, override it with a global/project entry, or
 * remove the file from the goldband source tree.
 */
