 ...
 *
 * /skillify stages a candidate skill into ~/.goldband/.tmp/skillify-<spawnId>/,
 ...
 *   <tmpRoot>/.goldband/.tmp/skillify-<spawnId>/<name>/
 *
...
export function stageSkill(opts: StageSkillOptions): string {
  ...
  const tmpRoot = opts.tmpRoot ?? path.join(os.homedir(), '.goldband', '.tmp');
