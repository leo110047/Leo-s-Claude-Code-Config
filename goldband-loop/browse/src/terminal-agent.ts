/**
 * Terminal Agent — PTY-backed Claude Code terminal for the goldband browser
...
const STATE_FILE = process.env.BROWSE_STATE_FILE || path.join(process.env.HOME || '/tmp', '.goldband', 'browse.json');
...
const KEEPALIVE_INTERVAL_MS = parseInt(
  process.env.GOLDBAND_PTY_KEEPALIVE_INTERVAL_MS || '25000',
  10,
);
...
const RING_BUFFER_MAX_BYTES = parseInt(
  process.env.GOLDBAND_PTY_RING_BUFFER_BYTES || `${1024 * 1024}`,
  10,
);
...
const DETACH_WINDOW_MS = parseInt(
  process.env.GOLDBAND_PTY_DETACH_WINDOW_MS || '60000',
  10,
);
 ...
 *      (updated continuously by the goldband browser extension).
...
function buildTabAwarenessHint(stateDir: string): string {
  ...
  return [
    'You are running inside the goldband browser sidebar with live access to the user\'s browser tabs.',
    '',
...
function spawnClaude(cols: number, rows: number, onData: (chunk: Buffer) => void) {
  ...
  // headed-mode browser; BROWSE_NO_AUTOSTART prevents claude's goldband
...
function buildServer() {
  return Bun.serve<{ cookie: string; sessionId: string | null }>({
    ...
    fetch(req, server) {
      ...
      //         - Cookie goldband_pty (works for non-browser callers and
      ...
      if (url.pathname === '/ws') {
        ...
        // with a `goldband-pty.` prefix (which we strip). Browsers send a
        ...
        for (const raw of protoHeader.split(',').map(s => s.trim()).filter(Boolean)) {
          const candidate = raw.startsWith('goldband-pty.') ? raw.slice('goldband-pty.'.length) : raw;
        ...
        // Fallback: Cookie goldband_pty (legacy / non-browser callers).
        if (!token) {
          const cookieHeader = req.headers.get('cookie') || '';
          for (const part of cookieHeader.split(';')) {
            const [name, ...rest] = part.trim().split('=');
            if (name === 'goldband_pty') {
...
function main() {
  ...
  // sibling goldband sessions. Callers (cli.ts spawn site, server.ts
