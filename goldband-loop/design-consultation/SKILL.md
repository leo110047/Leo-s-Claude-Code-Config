---
name: goldband-design-consultation
preamble-tier: 3
version: 1.0.0
description: |
  ...
  design system or DESIGN.md. (goldband)
allowed-tools:
  - Bash
  - Read
  - Write
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"design-consultation","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
$GOLDBAND_BIN/goldband-timeline-log '{"skill":"design-consultation","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-question-log '{"skill":"design-consultation","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
eval "$($GOLDBAND_BIN/goldband-slug 2>/dev/null)"
ls ~/.goldband/projects/$SLUG/*office-hours* 2>/dev/null | head -5
...
**Find the goldband designer (optional — enables AI mockup generation):**
...
```bash
_TASTE_PROFILE=~/.goldband/projects/$SLUG/taste-profile.json
...
the legacy approved.json aggregate — `$GOLDBAND_BIN/goldband-taste-update`
...
This phase generates visual previews of the proposed design system. Two paths depending on whether the goldband designer is available.
...
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
eval "$($GOLDBAND_BIN/goldband-slug 2>/dev/null)"
_DESIGN_DIR="$HOME/.goldband/projects/$SLUG/designs/design-system-$(date +%Y%m%d)"
mkdir -p "$_DESIGN_DIR"
...
```bash
. "$HOME/.claude/skills/goldband/bin/goldband-env" || exit $?
$GOLDBAND_BIN/goldband-learnings-log '{"skill":"design-consultation","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
