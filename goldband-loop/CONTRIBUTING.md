# Contributing to goldband

Thanks for wanting to make goldband better. Whether you're fixing a typo in a skill prompt or building an entirely new workflow, this guide will get you up and running fast.

## Quick start

goldband skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `~/.claude/skills/goldband/` (your global install). But when you're developing goldband itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.
...
goldband automatically learns from failures. At the end of every skill session, the agent
...
4. **Symlink your fork into the project where you hit the bug:**
   ```bash
   # In your core project (the one where goldband annoyed you)
   ln -sfn /path/to/your/goldband-fork .claude/skills/goldband
   cd .claude/skills/goldband && bun install && bun run build && ./setup
   ```
   Setup creates per-skill directories with SKILL.md symlinks inside (`qa/SKILL.md -> goldband/qa/SKILL.md`)
   and asks your prefix preference. Pass `--no-prefix` to skip the prompt and use short names.
5. **Fix the issue** — your changes are live immediately in this project
6. **Test by actually using goldband** — do the thing that annoyed you, verify it's fixed
7. **Open a PR from your fork**

This is the best way to contribute: fix goldband while doing your real work, in the
project where you actually felt the pain.

### Session awareness

When you have 3+ goldband sessions open simultaneously, every question tells you which project, which branch, and what's happening. No more staring at a question thinking "wait, which window is this?" The format is consistent across all skills.

## Working on goldband inside the goldband repo

When you're editing goldband skills and want to test them by actually using goldband
...
When E2E tests run, they produce machine-readable artifacts in `~/.goldband-dev/`:
...
goldband's Writing Style section (injected into every tier-≥2 skill's preamble)
...
|--------|---------------------------|
| Output directory | `{skill}/SKILL.md` vs `.agents/skills/goldband-{skill}/SKILL.md` |
| Frontmatter | Full (name, description, hooks, version) vs minimal (name + description) |
| Paths | `~/.claude/skills/goldband` vs `$GOLDBAND_ROOT` |
...
**This is the recommended way to develop goldband.** Symlink your goldband checkout
...
```bash
# In your core project (not the goldband repo)
...
Remove the project-local symlink. Claude Code falls back to `~/.claude/skills/goldband/`:
...
- Renamed or moved config keys in `~/.goldband/config.yaml`
- Need to delete orphaned files from a previous version
- Changed the format of `~/.goldband/` state files
...
1. Create `goldband-upgrade/migrations/v{VERSION}.sh` where `{VERSION}` matches
   the VERSION file for the release that needs the fix.
2. Make it executable: `chmod +x goldband-upgrade/migrations/v{VERSION}.sh`
...
"$SCRIPT_DIR/bin/goldband-relink" 2>/dev/null || true
```
...
verifies that all migration scripts in `goldband-upgrade/migrations/` are
