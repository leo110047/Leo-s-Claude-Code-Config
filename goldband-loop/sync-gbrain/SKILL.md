---
name: goldband-sync-gbrain
preamble-tier: 2
version: 1.0.0
description: |
  Keep gbrain current with this repo's code and refresh agent search
  guidance in CLAUDE.md. Wraps the goldband-gbrain-sync orchestrator with
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"sync-gbrain","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/goldband/bin/goldband-timeline-log '{"skill":"sync-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/goldband/bin/goldband-question-log '{"skill":"sync-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
It does NOT touch `~/.goldband/` indexing (the existing `goldband-gbrain-source-wireup`
...
`~/.claude/skills/goldband/bin/goldband-gbrain-sync.ts`.
...
```bash
~/.claude/skills/goldband/bin/goldband-gbrain-detect 2>/dev/null
```
...
also runs locally** in local-stdio MCP mode — `goldband-memory-ingest` shells
...
`~/.goldband/transcripts/<run-id>/` and the artifacts pipeline pushes it to
the brain admin's pull job (plan D11). Brain-sync (the `goldband-brain-sync`
...
Also check the per-repo trust policy. If `goldband-gbrain-repo-policy get` for
...
```bash
bun run ~/.claude/skills/goldband/bin/goldband-gbrain-sync.ts <user-args>
```
...
still run. State is persisted to `~/.goldband/.gbrain-sync-state.json` via
tmp-file + atomic rename. Concurrent runs are blocked by a lock file at
`~/.goldband/.sync-gbrain.lock` (5-min stale-takeover).
...
```bash
SOURCE_ID=$(grep -o '"source_id":"[^"]*"' ~/.goldband/.gbrain-sync-state.json 2>/dev/null \
...
from `<!-- goldband-gbrain-search-guidance:start -->` through
`<!-- goldband-gbrain-search-guidance:end -->`. If those markers are missing,
...
gbrain status: GREEN
  ...
  CWD source ...... OK   <goldband-code-{repo_slug}> (page_count=<N>)
  ~/.goldband source. OK   <goldband-brain-{user}> (page_count=<N>) — managed by /setup-gbrain
...
Mac. The orchestrator acquires a lock at `~/.goldband/.sync-gbrain.lock` before
...
and travels with `git push`/`git pull` — NOT through `~/.goldband/.brain-allowlist`
(which is for `~/.goldband/` brain-sync only). On a different Mac with a synced
