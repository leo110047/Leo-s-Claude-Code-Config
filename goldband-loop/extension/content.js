/**
 * goldband browse — content script
 *
...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'goldband-status-pill';
  ...
  statusPill.innerHTML = `<span class="goldband-pill-dot"></span> goldband${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'goldband-ref-overlays';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'goldband-ref-panel';

  const header = document.createElement('div');
  header.className = 'goldband-ref-panel-header';
  header.textContent = `goldband refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'goldband-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'goldband-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'goldband-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'goldband-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'goldband-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'goldband-ref-panel-more';
...
// Do NOT dispatch goldband-extension-ready here — the extension being loaded
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'sidebarOpened') {
    document.dispatchEvent(new CustomEvent('goldband-extension-ready'));
    return;
  }
