...
document.getElementById('chat-cookies-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    console.error('[goldband sidebar] Failed to open cookie picker:', err.message);
  }
});
...
async function ensureSseSessionCookie() {
  ...
  try {
    ...
    console.warn('[goldband sidebar] Failed to mint SSE session cookie:', err && err.message);
    return false;
  }
}
...
async function connectSSE() {
  ...
  eventSource.addEventListener('activity', (e) => {
    try { addEntry(JSON.parse(e.data)); } catch (err) {
      console.error('[goldband sidebar] Failed to parse activity event:', err.message);
    }
  });
  ...
  eventSource.addEventListener('gap', (e) => {
    try {
      ...
      console.error('[goldband sidebar] Failed to parse gap event:', err.message);
    }
  });
}
...
async function fetchRefs() {
  ...
  try {
    ...
    console.error('[goldband sidebar] Failed to fetch refs:', err.message);
  }
}
...
inspectorSendBtn.addEventListener('click', async () => {
  ...
  // goldbandScanForPTYInject is async; goldbandInjectToTerminal stays sync.
  const verdict = await window.goldbandScanForPTYInject?.(message + '\n', 'inspector-send');
  if (verdict?.verdict === 'BLOCK') {
    console.warn('[goldband sidebar] Inspector send BLOCKED by /pty-inject-scan:', verdict.reasons);
    return;
  }
  ...
  const ok = window.goldbandInjectToTerminal?.(message + '\n');
  if (!ok) {
    console.warn('[goldband sidebar] Inspector send needs an active Terminal session.');
  }
});
...
async function runCleanup(...buttons) {
  ...
  // confirms every call site goes through goldbandScanForPTYInject first.
  const verdict = await window.goldbandScanForPTYInject?.(cleanupPrompt + '\n', 'cleanup-button');
  if (verdict?.verdict === 'BLOCK') {
    console.warn('[goldband sidebar] Cleanup BLOCKED by /pty-inject-scan:', verdict.reasons);
  ...
  const sent = window.goldbandInjectToTerminal?.(cleanupPrompt + '\n');
  if (!sent) {
    console.warn('[goldband sidebar] Cleanup needs an active Terminal session.');
  }
...
async function runScreenshot(...buttons) {
  ...
  try {
    ...
    if (!resp.ok) {
      console.warn('[goldband sidebar] Screenshot failed:', text);
    } else {
      console.log('[goldband sidebar] Screenshot:', text);
    }
  } catch (err) {
    console.error('[goldband sidebar] Screenshot error:', err.message);
  } finally {
  ...
  try {
    ...
    inspectorSSE.addEventListener('inspectResult', (e) => {
      try {
        ...
        console.error('[goldband sidebar] Failed to parse inspectResult:', err.message);
      }
    });
    ...
    console.debug('[goldband sidebar] Inspector SSE not available:', err.message);
  }
}
...
function updateConnection(url, token) {
  ...
  if (url) {
    try { window.goldbandServerPort = parseInt(new URL(url).port, 10); } catch {}
    window.goldbandAuthToken = token || null;
  } else {
    window.goldbandServerPort = null;
    window.goldbandAuthToken = null;
  }
  if (url) {
...
document.getElementById('conn-copy').addEventListener('click', () => {
  navigator.clipboard.writeText('/open-goldband-browser').then(() => {
    const btn = document.getElementById('conn-copy');
    btn.textContent = 'copied!';
    setTimeout(() => { btn.textContent = '/open-goldband-browser'; }, 2000);
  });
});
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'browserTabState') {
    document.dispatchEvent(new CustomEvent('goldband:tab-state', {
...
window.addEventListener('pagehide', () => {
  const sessionId = window.goldbandPtySession;
  const authToken = window.goldbandAuthToken;
  const port = window.goldbandServerPort;
