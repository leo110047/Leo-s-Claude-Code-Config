...
(function () {
  ...
  if (!Terminal) {
    console.error('[goldband terminal] xterm not loaded');
    return;
  }
   ...
   * /pty-session and surfaced back via window.goldbandPtySession so the
  ...
  function getAuthToken() {
    return window.goldbandAuthToken || null;
  }
  ...
  function startReattachLoop(prevSessionId) {
    ...
    const tick = async () => {
      ...
      if (resp.status === 401) {
        ...
        setState(STATE.IDLE, {
          message: 'Auth invalid — reload the sidebar or restart your goldband session.',
        });
        return;
      }
      if (!resp.ok) {
  ...
  function openReattachWebSocket(terminalPort, attachToken, sessionId) {
    ...
    try { window.goldbandPtySession = currentSessionId; } catch {}
    setState(STATE.LIVE);
    ensureXterm();
    nextBinaryIsReplay = false;
    ws = new WebSocket(`ws://127.0.0.1:${terminalPort}/ws`, [`goldband-pty.${attachToken}`]);
    ...
    ws.addEventListener('error', (err) => {
      console.error('[goldband terminal] reattach ws error', err);
    });
  }
   ...
   * derived input MUST be pre-scanned via window.goldbandScanForPTYInject()
   ...
   * sync `const ok = goldbandInjectToTerminal?.(text)` pattern. Making the
   ...
   */
  window.goldbandInjectToTerminal = function (text) {
   ...
   * Closes #1370.
   */
  window.goldbandScanForPTYInject = async function (text, origin) {
  ...
  async function getAuthTokenForScan() {
    if (window.__goldbandPtyScanToken) return window.__goldbandPtyScanToken;
    try {
      const resp = await fetch('http://127.0.0.1:34567/health');
      const body = await resp.json();
      const token = body.AUTH_TOKEN || body.authToken || '';
      if (token) window.__goldbandPtyScanToken = token;
      return token;
    } catch {
      return '';
    }
  ...
  async function connect() {
    ...
    ws.addEventListener('error', (err) => {
      console.error('[goldband terminal] ws error', err);
    });
  }
  ...
  async function forceRestart() {
    ...
    if (!nextTuple) {
      ...
      try { window.goldbandPtySession = null; } catch {}
      tryAutoConnect();
      return;
    }
    ...
    setState(STATE.LIVE);
    ensureXterm();
    ws = new WebSocket(`ws://127.0.0.1:${terminalPort}/ws`, [`goldband-pty.${token}`]);
    ws.binaryType = 'arraybuffer';
  ...
  function init() {
    ...
    document.addEventListener('goldband:tab-state', (ev) => {
      if (!ws || ws.readyState !== WebSocket.OPEN) return;
      try {
        ws.send(JSON.stringify({
          type: 'tabState',
   ...
   * window.goldbandServerPort + window.goldbandAuthToken (which it does as soon
