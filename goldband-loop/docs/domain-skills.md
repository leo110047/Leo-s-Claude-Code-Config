...
This is goldband's borrow from [browser-use/browser-harness](https://github.com/browser-use/browser-harness).
goldband copies the per-site-notes pattern, NOT the self-modifying-runtime
...
on every goldband project I work on"), explicitly run
...
following events are written to `~/.goldband/analytics/browse-telemetry.jsonl`:
...
`goldband-config set telemetry off` or `GOLDBAND_TELEMETRY_OFF=1`.
