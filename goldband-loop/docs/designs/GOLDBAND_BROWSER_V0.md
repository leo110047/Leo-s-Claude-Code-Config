...
consumer browser and bolts AI onto it. Goldband Loop Browser inverts this. It starts
...
for text editors, Goldband Loop Browser does for the browser.

## What It Is Today (Phase 1a, shipped)

A double-clickable macOS .app that wraps Playwright's Chromium with the goldband
...
and run any goldband skill. All without touching a terminal.

```
Goldband Loop Browser.app (389MB, 189MB DMG)
...
When the extension side panel hits hard API limits, Goldband Loop Browser ships to
...
Every goldband skill becomes a browser capability.
...
| **Goldband Loop Browser** | CC runtime + browser viewport | See bug in browser, fix in code, verify | Currently macOS-only, no consumer features |

Goldband Loop Browser doesn't compete with consumer browsers. It competes with the
...
| `GOLDBAND_CHROMIUM_PATH` | **SHIPPED** | Custom Chromium binary support |
...
The 12-month ideal: you open Goldband Loop Browser, it detects your project, starts
