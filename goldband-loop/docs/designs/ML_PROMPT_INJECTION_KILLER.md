...
The goldband Chrome extension sidebar gives Claude bash access to control the browser.
...
```typescript
// Public API -- any goldband component can call these
...
```json
// ~/.goldband/security/attempts.jsonl
{
...
```
AskUserQuestion:
  "goldband just blocked a prompt injection attempt from {domain}. These detections
   are rare and valuable for improving defenses for all goldband users. Can we
...
2. Cache to ~/.goldband/models/browsesafe-bench/ (not re-downloaded in CI)
...
- goldband becomes the only open source tool with native-speed prompt injection detection.
...
**Limitation:** macOS-only (Linux would need OpenBLAS FFI). But goldband already
...
- [ ] Implement `loadModel()` with download-on-first-use to ~/.goldband/models/
