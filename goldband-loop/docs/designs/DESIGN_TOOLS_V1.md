# Design: goldband Visual Design Generation (`design` binary)

Generated by /office-hours on 2026-03-26
Branch: project-owner/agent-design-tools
Repo: goldband
**Status:** Implemented — the design CLI and command registry shipped; current behavior lives in `design/src/` and `ARCHITECTURE.md`.
Mode: Intrapreneurship

## Context

goldband's design skills (/office-hours, /design-consultation, /plan-design-review, /design-review) all produce **text descriptions** of design — DESIGN.md files with hex codes, plan docs with pixel specs in prose, ASCII art wireframes. The creator is a designer who hand-designed HelloSign in OmniGraffle and finds this embarrassing.
...
**Screenshot consent (first-time only for $D evolve):** "This will send a screenshot of your live site to OpenAI for design evolution. [Proceed] [Don't ask again]" Stored in ~/.goldband/config.yaml as design_screenshot_consent.
...
- Exploration variants go to `/tmp/goldband-mockups-{session}/` (ephemeral, not committed)
...
- Fallback: if not in a git repo, save to `/tmp/goldband-mockup-{timestamp}.png`
...
```
Note: Codex runtime setup (`setup` script) must also export `GOLDBAND_DESIGN` env var, similar to how `GOLDBAND_BROWSE` is set.
...
[ -z "$D" ] && D=~/.claude/skills/goldband/design/dist/design
if [ -x "$D" ]; then
  echo "DESIGN_READY: $D"
else
...
| `scripts/resolvers/preamble.ts` | Add `GOLDBAND_DESIGN` env var export for Codex host |
...
1. Read `~/.goldband/openai.json` → `{ "api_key": "sk-..." }` (file permissions 0600)
...
3. If neither exists → guided setup flow:
   ...
   - Write to `~/.goldband/openai.json` with 0600 permissions
...
- Symlinked via existing `~/.claude/skills/goldband/` install path
...
- Auth module (read ~/.goldband/openai.json, fallback to env var, guided setup flow)
...
- Add GOLDBAND_DESIGN env var export to `scripts/resolvers/preamble.ts` (Codex host)
