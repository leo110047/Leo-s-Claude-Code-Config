...
goldband projects... CLAUDE.md works as-is.
...
### Critical env vars for goldband integration
...
Without native `.slate/` publishing, goldband skills vanish when this flag is set.
...
goldband skills already work in Slate via the `.claude/skills/` fallback path.
No changes needed for basic functionality. Users who install goldband for Claude Code
...
- Host names duplicated in `bin/goldband-platform-detect`, `bin/goldband-uninstall`, `bin/dev-setup`
...
- `lib/worktree.ts` only copies `.agents/`, not `.slate/` — E2E tests in worktrees won't
  have Slate skills
- `bin/goldband-uninstall` doesn't know about `.slate/`
- `bin/dev-setup` doesn't wire `.slate/` for contributor dev mode
- `bin/goldband-platform-detect` doesn't detect Slate
