...
goldband already produces valuable artifacts that survive on disk: CEO plans,
...
survive compaction. Because nobody else has goldband's artifact architecture.

## The Insight

goldband already writes structured artifacts to `~/.goldband/projects/$SLUG/`:
- CEO plans: `ceo-plans/`
...
the agent checks `~/.goldband/projects/$SLUG/` for recent plans, reviews, and
...
Plan: ~/.goldband/projects/$SLUG/checkpoints/latest.md"
...
- Not a replacement for Claude's built-in compaction (that handles session
  state; we handle goldband artifacts)
