# Design: Goldband Loop Self-Learning Infrastructure
...
Branch: project-owner/ce-features
Repo: goldband
**Status:** Superseded — shipped pieces remain, but the legacy flat-workflow roadmap is replaced by the capability manifest and focused design docs.
Mode: Open Source / Community

## Problem Statement

Goldband Loop runs 30+ skills across sessions but learns nothing between them. A /review
...
Structured, typed, confidence-scored learnings that every goldband skill can read and
write. The goal: after 20 sessions on the same codebase, goldband knows every
...
YC founders building with AI. The people who run goldband on real codebases 20+ times
...
| **Goldband Loop** | **Per-project JSONL** | **Cross-session, cross-skill** | **Typed, scored, decaying** |

---

## State Systems

goldband has four distinct persistence layers. They share storage patterns
(JSONL in `~/.goldband/projects/$SLUG/`) but serve different purposes:
...
### Release 1: "Goldband Loop Learns" (v0.13-0.14) — SHIPPED

**Headline:** Every session makes the next one smarter.

What shipped:
- Learnings persistence at `~/.goldband/projects/{slug}/learnings.jsonl`
...
- E1: Adaptive specialist gating, auto-skip specialists with 0-finding track record.
  Store per-project hit rates via goldband-learnings-log. User can force with --security etc.
...
- E5: Cross-review finding dedup, read goldband-review-read for prior review entries.
  Suppress findings matching a prior user-skipped finding.
- E7: Specialist performance tracking, log per-specialist metrics via goldband-review-log.
...
- Session timeline: every skill auto-logs start/complete events to
  `~/.goldband/projects/$SLUG/timeline.jsonl`. Local-only, never sent anywhere,
...
- Predictive skill suggestion: if your last 3 sessions follow a pattern
  (review, ship, review), goldband suggests what you probably want next.
...
- Timeline binaries: `bin/goldband-timeline-log` and `bin/goldband-timeline-read`.
...
**Headline:** Goldband Loop respects your time without compromising your safety.
...
The self-learning roadmap was inspired by ideas from the [Compound Engineering](https://github.com/nicobailon/compound-engineering) project by Nico Bailon. Their exploration of learnings persistence, parallel review agents, and autonomous pipelines catalyzed the design of Goldband Loop's approach. We adapted every concept to fit Goldband Loop's template system, voice, and architecture rather than porting directly.
