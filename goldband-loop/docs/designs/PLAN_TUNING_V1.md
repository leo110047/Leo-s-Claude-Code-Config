...
This plan exists because of **[Louise de Sadeleer](https://x.com/LouiseDSadeleer/status/2045139351227478199)**, who sat through a complete goldband run as a non-technical user and told us the truth about how it feels. Her specific feedback:
...
3. **Terse opt-out** (`goldband-config set explain_level terse`). Binary: `default` vs `terse`. Terse skips the Writing Style block entirely and uses V0 prose style.
...
- **User-editable jargon list at runtime.** Revision 3 proposed `~/.goldband/jargon-list.json` as user override. Codex Pass 2 caught the contradiction with gen-time inlining. Resolved: repo-owned only, PRs to add/remove, regenerate to take effect.
...
- **Using the same string as replacement anchor AND CI-reject marker in README.** Eng review Pass 2 / Codex Pass 2 caught that this makes the pipeline destroy its own update path. Two-string solution: `GOLDBAND-THROUGHPUT-PLACEHOLDER` (anchor, stays across runs) vs `GOLDBAND-THROUGHPUT-PENDING` (explicit "build didn't run" marker that CI rejects).
...
~/.claude/skills/goldband/bin/
  goldband-config                     # MODIFIED: explain_level header + validation

goldband-upgrade/migrations/
...
Preamble bash (per-invocation):
  _EXPLAIN_LEVEL=$(${binDir}/goldband-config get explain_level 2>/dev/null || "default")
...
**User-editable at runtime (rejected):** `~/.goldband/jargon-list.json` overrides `scripts/jargon-list.json`.
...
**Single string (rejected):** `<!-- GOLDBAND-THROUGHPUT-MULTIPLE: N× -->` as both replacement anchor AND CI-reject marker.
- Pros: Simple.
- Cons (Codex Pass 2): Pipeline breaks on itself — CI rejects commits containing the marker, but the marker IS the anchor.

**Two-string (chosen):** `GOLDBAND-THROUGHPUT-PLACEHOLDER` (anchor, stable) + `GOLDBAND-THROUGHPUT-PENDING` (explicit missing-build marker, CI rejects).
...
Review report persisted in `~/.goldband/` via `goldband-review-log`. Plan file retained with full history at `~/.claude/plans/system-instruction-you-are-working-transient-sunbeam.md`.
