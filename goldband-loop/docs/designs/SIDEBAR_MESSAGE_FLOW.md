...
T+1-3s    Extension loads, sidebar opens
            ...
            └── tryAutoConnect() polls until window.goldbandServerPort + token are set
...
2. Extension calls `new WebSocket(url, ['goldband-pty.<token>'])`.
3. Agent reads `Sec-WebSocket-Protocol`, strips `goldband-pty.`, validates
...
A `Set-Cookie: goldband_pty=...` header is also returned for non-browser
...
| `goldband-pty.<...>` (Sec-WebSocket-Protocol) | Browser memory only; agent `validTokens` Set | `/ws` upgrade auth | 30 min, auto-revoked on WS close |
...
| Button | Behavior |
|--------|----------|
| 🧹 Cleanup | `window.goldbandInjectToTerminal(prompt)` — pipes a "remove ads/banners" instruction into the live PTY. claude in the terminal sees it and acts. |
...
The Inspector's "Send to Code" button uses the same `goldbandInjectToTerminal`
