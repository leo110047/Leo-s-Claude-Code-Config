# Remote Browser Access — How to Pair With a Goldband Loop Browser

A Goldband Loop Browser server can be shared with any AI agent that can make HTTP requests.
...
─────────────                         ────────────
Goldband Loop Browser Server                 Any AI agent
...
1. **User runs** `$B pair-agent` from a `/goldband browser session`
...
|------|---------|------------|
| 401 | Token invalid, expired, or revoked | Ask user to run `$B pair-agent` again |
...
- **Path traversal guarded** on `/welcome` — `GOLDBAND_SLUG` must match `^[a-z0-9_-]+$` or falls back to the built-in template.
- **SSRF guards** on `goto`, `download`, and scrape paths — validates URL target against a localhost/private-range blocklist.
- **Tunnel surface denial logging.** Every rejection on the tunnel listener (`path_not_on_tunnel`, `root_token_on_tunnel`, `missing_scoped_token`, `disallowed_command:*`) is appended to `~/.goldband/security/attempts.jsonl` with timestamp, source IP, path, method. Rate-capped at 60 writes/min.
...
```bash
$B pair-agent --local openclaw    # writes to ~/.openclaw/skills/goldband/browse-remote.json
$B pair-agent --local codex       # writes to ~/.codex/skills/goldband/browse-remote.json
$B pair-agent --local cursor      # writes to ~/.cursor/skills/goldband/browse-remote.json
```
...
3. Save it: `echo 'NGROK_AUTHTOKEN=your_token' > ~/.goldband/ngrok.env`
4. Optionally claim a stable domain: `echo 'NGROK_DOMAIN=your-name.ngrok-free.dev' >> ~/.goldband/ngrok.env`
