...
description: |
  ...
  or "strip the goldband iOS instrumentation". (goldband)
  Voice triggers (speech-to-text aliases): "clean the iOS debug bridge", "remove DebugBridge", "strip the goldband iOS instrumentation".
allowed-tools:
  - Bash
  - Read
  - Edit
...
triggers:
  ...
  - remove debugbridge
  - strip the goldband ios instrumentation
---
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"ios-clean","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/goldband/bin/goldband-timeline-log '{"skill":"ios-clean","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/goldband/bin/goldband-question-log '{"skill":"ios-clean","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
- Are migrating away from goldband and want a clean exit.
...
5. The `goldband-ios-qa.token` file under `NSTemporaryDirectory()` on the
