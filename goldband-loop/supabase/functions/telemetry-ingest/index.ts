// goldband telemetry-ingest edge function
// Validates and inserts a batch of telemetry events.
// Called by bin/goldband-telemetry-sync.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const event of events) {
      // Required fields
      if (!event.ts || !event.goldband_version || !event.os || !event.outcome) {
      ...
      rows.push({
        ...
        goldband_version: String(event.goldband_version).slice(0, 20),
      ...
      if (event.installation_id) {
        installationUpserts.set(event.installation_id, {
          version: event.goldband_version,
          os: event.os,
        });
      }
    }
