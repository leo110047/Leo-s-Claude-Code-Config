// goldband community-pulse edge function
...
Deno.serve(async () => {
  ...
  try {
    ...
    const { data: crashes } = await supabase
      .from("crash_clusters")
      .select("error_class, goldband_version, total_occurrences, identified_users")
      .limit(5);
    ...
    const { data: versionRows } = await supabase
      .from("telemetry_events")
      .select("goldband_version")
      .eq("event_type", "skill_run")
      .gte("event_timestamp", weekAgo)
      .limit(1000);

    for (const row of versionRows ?? []) {
      if (row.goldband_version) {
        versionCounts[row.goldband_version] = (versionCounts[row.goldband_version] ?? 0) + 1;
      }
    }
    ...
    // Fields emitted by goldband-telemetry-log --event-type attack_attempt:
    ...
    // goldband user who polls /community-pulse. With it, the dashboard shows
