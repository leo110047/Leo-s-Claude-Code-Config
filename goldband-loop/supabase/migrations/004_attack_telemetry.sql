-- goldband attack telemetry — schema extension for prompt injection events.
--
-- Ships alongside the goldband-telemetry-log `--event-type attack_attempt`
-- flag (bin/goldband-telemetry-log, commits 28ce883c + f68fa4a9). These
-- columns are nullable so the existing skill_run events continue inserting
-- unchanged.
--
-- Fields (1:1 with goldband-telemetry-log flags):
...
--   * salt is per-device local file (~/.goldband/security/device-salt) —
