# Using GBrain with Goldband Loop

Your coding agent, with a memory it actually keeps.

[GBrain](https://github.com/project-owner/gbrain) is a persistent knowledge base designed for AI agents. It stores what your agent learns, what you've decided, what worked and what didn't, and lets the agent search all of it on demand. Goldband Loop gives you a one-command path from zero to "gbrain is running, and my agent can call it" — with paths for try-it-local, share-with-your-team, and everything between.
...
The high-risk setup automation is currently an experimental operator flow, so
it is intentionally not exposed through the public capability router. Start
with the detect-first helper:

```bash
~/.claude/skills/goldband/bin/goldband-gbrain-detect
```
...
Once the GBrain operator setup finishes, your coding agent has two retrieval surfaces it didn't have before:
...
**If you Ctrl-C mid-provision:** The SIGINT trap prints your in-flight project ref + a resume command. You can delete the orphan at the Supabase dashboard, or resume polling with `goldband-gbrain-supabase-provision wait <ref>` after restoring its required secret environment.
...
The skill asks once per repo the first time you run a goldband skill there. After that the decision is sticky — every worktree + branch of the same git remote shares the same policy, so you set it once and it follows you.
...
```bash
~/.claude/skills/goldband/bin/goldband-gbrain-repo-policy list
```

Storage: `~/.goldband/gbrain-repo-policy.json`, mode 0600, schema-versioned so future migrations stay deterministic.

## Keeping the brain current with the sync helper

GBrain setup is one-time onboarding. The sync helper is the operator command you run every time you want gbrain to see fresh changes in this repo's code.

```bash
~/.claude/skills/goldband/bin/goldband-gbrain-sync.ts                # incremental
~/.claude/skills/goldband/bin/goldband-gbrain-sync.ts --full         # full reindex
~/.claude/skills/goldband/bin/goldband-gbrain-sync.ts --code-only    # code stage only
~/.claude/skills/goldband/bin/goldband-gbrain-sync.ts --dry-run      # preview only
```
...
3. **Memory stage.** Stages your `~/.goldband/` transcripts + curated memory. In local-stdio MCP mode, ingests into the local engine. In remote-http MCP mode, persists staged markdown to `~/.goldband/transcripts/run-<pid>-<ts>/` for the remote brain admin's pull pipeline.
...
Re-runnable, idempotent, safe to run from multiple terminals on the same machine (locked at `~/.goldband/.sync-gbrain.lock`).

## Switching engines later

Picked PGLite and now want to join a team brain? One command:

Use `gbrain migrate --to supabase --url "$URL"` or
`gbrain migrate --to pglite` after reviewing the destination and backup.
...
**If migration hangs:** another goldband session may be holding a lock on the source brain. The timeout fires at 3 minutes with an actionable message. Close other workspaces and re-run.

## Goldband Loop memory sync (a separate concern)

This is different from gbrain itself. Your goldband state (`~/.goldband/` — learnings, plans, retros, timeline, developer profile) is machine-local by default. "Goldband Loop memory sync" optionally pushes a curated, secret-scanned subset to a private git repo so your memory follows you across machines — and, if you're running gbrain, that git repo becomes indexable there too.
...
**On a new machine:** Copy `~/.goldband-brain-remote.txt` over, run `goldband-brain-restore`, and yesterday's learnings surface on today's laptop.

Full guide: [docs/gbrain-sync.md](docs/gbrain-sync.md). Error index: [docs/gbrain-sync-errors.md](docs/gbrain-sync-errors.md).

The experimental setup operator flow can wire this up at the end of initial setup; it remains outside the public capability router until its authorization and readback contract is typed.
...
Use `goldband-gbrain-supabase-provision list-orphans` to inspect candidates,
then `delete-project` separately for each explicitly approved project.
...
### Experimental setup operator modes

| Invocation | What it does |
|---|---|
| Full setup | Detect state, pick path, install, init, MCP, policy, optional memory-sync |
| Repo policy | Use `goldband-gbrain-repo-policy` for the current repo |
| Switch engine | Use the reviewed `gbrain migrate` commands above |
| Resume provision | Use `goldband-gbrain-supabase-provision wait <ref>` |
| Cleanup orphans | List first, then delete each explicitly approved project |

### Bin helpers (for scripting)

| Bin | Purpose |
|---|---|
| `goldband-gbrain-detect` | Emit current state as JSON: gbrain on PATH, version, config engine, doctor status, sync mode |
| `goldband-gbrain-install` | Detect-first installer (probes `~/git/gbrain`, `~/gbrain`, then fresh clone). Has `--dry-run` and `--validate-only` flags. PATH-shadow check exits 3 with remediation menu. |
| `goldband-gbrain-lib.sh` | Sourced, not executed. Provides `read_secret_to_env VARNAME "prompt" [--echo-redacted "<sed-expr>"]` |
| `goldband-gbrain-supabase-verify` | Structural URL check. Rejects direct-connection URLs (`db.*.supabase.co:5432`) with exit 3 |
| `goldband-gbrain-supabase-provision` | Management API wrapper. Subcommands: `list-orgs`, `create`, `wait`, `pooler-url`, `list-orphans`, `delete-project`. All require `SUPABASE_ACCESS_TOKEN` in env. `create` and `pooler-url` also require `DB_PASS`. `--json` mode available on every subcommand. |
| `goldband-gbrain-repo-policy` | Per-remote trust triad. Subcommands: `get`, `set`, `list`, `normalize` |
| `goldband-gbrain-source-wireup` | Registers your `~/.goldband/` brain repo with gbrain as a federated source via `gbrain sources add` + `git worktree`, then runs an initial `gbrain sync`. Idempotent. Replaces the dead `consumers.json + /ingest-repo` HTTP wireup from v1.12.x. Flags: `--strict`, `--source-id <id>`, `--no-pull`, `--uninstall`, `--probe`. |

### gbrain CLI (upstream tool)

Gbrain itself ships with these that goldband wraps:
...
|---|---|---|
| `SUPABASE_ACCESS_TOKEN` | `goldband-gbrain-supabase-provision` | PAT for Management API calls. Discarded after each setup run. |
| `DB_PASS` | `goldband-gbrain-supabase-provision` (create, pooler-url) | Generated DB password. Never in argv. |
...
| `SUPABASE_API_BASE` | `goldband-gbrain-supabase-provision` | Override the Management API host. Used by tests to point at a mock server. |
...
| `OPENAI_API_KEY` | `gbrain embed` subprocess | Used for embeddings during GBrain sync when `VOYAGE_API_KEY` is not set (gbrain's auto-selected fallback, `text-embedding-3-large` 1536-dim). Without either key, pages are imported structurally (symbol tables, chunks) but semantic search degrades — you'll see `[gbrain] embedding failed for code file ...` in the sync log. |
...
## Conductor + GOLDBAND_* env vars

If you run goldband inside a [Conductor](https://conductor.build) workspace, **Conductor explicitly strips `ANTHROPIC_API_KEY` and `OPENAI_API_KEY` from the workspace env.** Setting them in `~/.zshrc` or `.env` won't help — the strip happens after env inheritance. To get a usable API key into a workspace, set `GOLDBAND_ANTHROPIC_API_KEY` and `GOLDBAND_OPENAI_API_KEY` in Conductor's workspace env config instead. Conductor passes those through untouched.
...
- `bin/goldband-gbrain-sync.ts` — so the sync helper picks up OpenAI for embeddings
...
`bin/goldband-codex-probe` is bash and doesn't read these directly — it relies on `~/.codex/` auth managed by the Codex CLI.
...
- CI grep test fails if `--insecure`, `-k`, or `NODE_TLS_REJECT_UNAUTHORIZED=0` appear in `bin/goldband-gbrain-supabase-provision`
...
Then re-run the detect-first installation procedure.
...
Resume with `goldband-gbrain-supabase-provision wait <ref>` after restoring the required secret environment.

The skill re-collects a PAT, skips project creation, resumes polling.

### "Another GBrain setup instance is running"

You have a stale lock directory. If you're sure no other instance is actually running:

```bash
rm -rf ~/.goldband/.setup-gbrain.lock.d
```
...
You edited `~/.goldband/gbrain-repo-policy.json` by hand with legacy `allow` values? No problem. On the next read, goldband auto-migrates `allow` → `read-write` and adds `_schema_version: 2`. One log line on stderr, idempotent, deterministic.
...
### The sync helper reports `OK` but `gbrain search` returns nothing semantic
...
Another goldband session in a sibling Conductor workspace may be holding a lock on your local PGLite file via its preamble's `goldband-brain-sync` call. Close other workspaces, then re-run the reviewed `gbrain migrate` command. The timeout is bounded at 180s so you'll never actually wait forever.
...
**Why not bundle gbrain into goldband?** Gbrain is a separate, actively-developed project with its own release cadence, schema migrations, and MCP surface. Bundling would mean goldband has to gate gbrain updates, which slows gbrain improvements from reaching users. Separate-but-integrated lets each ship on its own cadence.
...
**Why not auto-import every repo?** Privacy + noise. An auto-import preamble hook that ingests every repo you touch would: (a) leak work code into a shared brain without consent, and (b) clog search with throwaway repos. The per-remote policy makes ingestion an explicit, per-repo decision. The setup procedure doesn't install any auto-import hook today — but the policy store is forward-compatible for one later.
...
- `/goldband system upgrade` — keeps goldband itself up to date. Does NOT upgrade gbrain independently. To bump gbrain, update `PINNED_COMMIT` in `bin/goldband-gbrain-install` and re-run the detect-first installer.
...
Run `goldband-gbrain-detect`, choose the documented operator path, and see what sticks.
