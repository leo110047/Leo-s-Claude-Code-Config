...
**If `AUTO_UPGRADE=true` or `AUTO_UPGRADE=1`:** Skip AskUserQuestion. Log "Auto-upgrading goldband v{old} → v{new}..." and proceed directly to Step 2. If `./setup` fails during auto-upgrade, restore from backup (`.bak` directory) and warn the user: "Auto-upgrade failed — restored previous version. Run `/goldband-upgrade` manually to retry."

**Otherwise**, use AskUserQuestion:
- Question: "goldband **v{new}** is available (you're on v{old}). Upgrade now?"
...
```bash
_SNOOZE_FILE="$HOME/.goldband/update-snoozed"
_REMOTE_VER="{new}"
_CUR_LEVEL=0
...
Tell user the snooze duration: "Next reminder in 24h" (or 48h or 1 week, depending on level). Tip: "Set `auto_upgrade: true` in `~/.goldband/config.yaml` for automatic upgrades."
...
```
Tell user: "Removed vendored copy at `$LOCAL_GOLDBAND` (team mode active — global install is the source of truth). Commit the `.gitignore` change when ready."
...
```
Tell user: "Sync failed — restored previous version at `$LOCAL_GOLDBAND`. Run `/goldband-upgrade` manually to retry."
...
Migrations are idempotent bash scripts in `goldband-upgrade/migrations/`. Each is named
...
Format:
```
goldband v{new} — upgraded from v{old}!
...
When invoked directly as `/goldband-upgrade` (not from a preamble):
...
Run the Step 2 bash block above to detect the primary install type and directory (`INSTALL_TYPE` and `INSTALL_DIR`). Then run the Step 4.5 detection bash block above to check for a local vendored copy (`LOCAL_GOLDBAND`) and team mode status (`TEAM_MODE`).

**If `LOCAL_GOLDBAND` is empty** (no local vendored copy): tell the user "You're already on the latest version (v{version})."

**If `LOCAL_GOLDBAND` is non-empty AND `TEAM_MODE` is `true`:** Remove the vendored copy using the Step 4.5 team-mode removal bash block above. Tell user: "Global v{version} is up to date. Removed stale vendored copy (team mode active). Commit the `.gitignore` change when ready."
