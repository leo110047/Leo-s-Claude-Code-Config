...
#
# Why a migration: goldband-artifacts-init regenerates these files but also
# does `git commit + push` on ~/.goldband/, which would clobber user state on
...
if [ -f "${PRIVACY}" ]; then
  if command -v jq >/dev/null 2>&1; then
    ...
    echo "  [v1.38.1.0] WARN: jq not found; skipping privacy-map repair. Install jq and re-run goldband-upgrade, or run goldband-artifacts-init manually." >&2
  fi
fi
...
# patches ship into their federated artifacts repo (next goldband-brain-sync
