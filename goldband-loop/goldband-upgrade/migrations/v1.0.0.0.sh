...
# the older V0 prose can set `goldband-config set explain_level terse`.
#
...
PROMPTED_FLAG="$GOLDBAND_HOME/.writing-style-prompted"
PENDING_FLAG="$GOLDBAND_HOME/.writing-style-prompt-pending"

mkdir -p "$GOLDBAND_HOME"
...
EXPLAIN_LEVEL_SET="$("${HOME}/.claude/skills/goldband/bin/goldband-config" get explain_level 2>/dev/null || true)"
