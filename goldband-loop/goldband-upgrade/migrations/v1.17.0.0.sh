...
#
# Pre-1.17.0.0 /setup-gbrain wrote ~/.goldband/consumers.json with a placeholder
...
#   - No ~/.goldband/.git (brain-init never ran)
...
# CLI is a benign skip rather than blocking the rest of /goldband-upgrade.
set -euo pipefail
...
WIREUP_BIN="${BIN_DIR}/goldband-gbrain-source-wireup"

# Skip if user never opted into brain-sync.
SYNC_MODE=""
if [ -x "$CONFIG_BIN" ]; then
  # Trim whitespace defensively: goldband-config can emit trailing newlines,
