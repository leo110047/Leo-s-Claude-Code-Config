...
# so the migration runner retries on the next /goldband-upgrade.

set -u

GOLDBAND_HOME="${HOME}/.goldband"
ALLOWLIST="${GOLDBAND_HOME}/.brain-allowlist"
PRIVACY="${GOLDBAND_HOME}/.brain-privacy-map.json"
GITATTRS="${GOLDBAND_HOME}/.gitattributes"
...
if [ -f "${PRIVACY}" ]; then
  if command -v jq >/dev/null 2>&1; then
    ...
    if ! jq -e . "${PRIVACY}" >/dev/null 2>&1; then
      echo "  [v1.40.0.0] WARN: ${PRIVACY} is not valid JSON; skipping privacy-map repair. Fix manually or run goldband-artifacts-init." >&2
      incomplete=1
    else
      for PATTERN in "${NEW_PATTERNS[@]}"; do
    ...
    echo "  [v1.40.0.0] WARN: jq not found; skipping privacy-map repair. Install jq and re-run goldband-upgrade, or run goldband-artifacts-init manually." >&2
    incomplete=1
  fi
fi
...
if [ "${incomplete}" = "0" ]; then
  ...
  # unnecessary. A fresh-init user's bin/goldband-artifacts-init now writes the
  ...
  echo "  [v1.40.0.0] INFO: marker not written; goldband-upgrade will retry once prerequisites are met." >&2
fi
