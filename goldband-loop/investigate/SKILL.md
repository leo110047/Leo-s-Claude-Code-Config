...
description: |
  ...
  yesterday", or is troubleshooting why something stopped working. (goldband)
allowed-tools:
  - Bash
  - Read
  - Write
...
hooks:
  PreToolUse:
    - matcher: "Edit"
      hooks:
        - type: command
          command: 'bash -c ''S="${CLAUDE_SKILL_DIR}/../freeze/bin/check-freeze.sh"; [ -x "$S" ] || S="${CLAUDE_SKILL_DIR}/../goldband-freeze/bin/check-freeze.sh"; [ -x "$S" ] && bash "$S" || exit 0'''
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"investigate","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
~/.claude/skills/goldband/bin/goldband-timeline-log '{"skill":"investigate","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/goldband/bin/goldband-question-log '{"skill":"investigate","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.claude/skills/goldband/bin/goldband-learnings-search --limit 10 --query "debug investigation root cause hypothesis bug fix" --cross-project 2>/dev/null || true
else
  ~/.claude/skills/goldband/bin/goldband-learnings-search --limit 10 --query "debug investigation root cause hypothesis bug fix" 2>/dev/null || true
fi
```
...
[ -x "$_FREEZE_SCRIPT" ] || _FREEZE_SCRIPT="${CLAUDE_SKILL_DIR}/../goldband-freeze/bin/check-freeze.sh"
...
1. **Fix the root cause, not the symptom.** Apply the healthiest complete fix that eliminates the actual problem and reduces recurrence risk.

2. **Blast radius intentional:** Keep unrelated refactors out, but do not compress a structural defect into a tiny patch. The blast radius intentional standard means every touched file must be justified by the confirmed root cause.
...
```bash
~/.claude/skills/goldband/bin/goldband-learnings-log '{"skill":"investigate","type":"investigation","key":"ROOT_CAUSE_KEY","insight":"ROOT_CAUSE_SUMMARY","confidence":9,"source":"observed","files":["affected/file1.ts","affected/file2.ts"]}'
```
...
```bash
~/.claude/skills/goldband/bin/goldband-learnings-log '{"skill":"investigate","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
- **If fix touches >5 files → AskUserQuestion** about blast radius before proceeding; continue only when the blast radius is intentional.
- **Completion status:**
