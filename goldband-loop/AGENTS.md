# goldband — AI Engineering Workflow

goldband is a collection of SKILL.md files that give AI agents structured roles for
software development. Each skill is a specialist: CEO reviewer, eng manager,
designer, QA lead, release engineer, debugger, and more.

## Available skills

Skills live in `.agents/skills/` (or `~/.claude/skills/goldband/` on Claude Code).
...
| `/goldband-upgrade` | Update goldband to the latest version. |
...
| `/learn` | Manage what goldband learned across sessions. |
...
| `/open-goldband-browser` | Launch the visible Goldband Loop Browser with sidebar + stealth. |
...
|---------|-------------|
| `goldband-ios-qa-daemon` | Mac-side broker. Loopback by default; `--tailnet` adds a Tailscale-facing listener with capability tiers and audit logging. |
| `goldband-ios-qa-mint` | Owner-grant CLI for the tailnet allowlist (`grant`/`revoke`/`list`). |

End-to-end walkthrough: [docs/howto-ios-testing-with-goldband.md](docs/howto-ios-testing-with-goldband.md).
...
- **Windows**: curated Windows-safe subset runs on `windows-latest` via the
  ...
  MSYS today; native PowerShell support is a future expansion. The `bin/goldband-paths`
  helper resolves state roots through `CLAUDE_PLUGIN_DATA` / `GOLDBAND_HOME` so plugin
...
- State paths resolve via `bin/goldband-paths` (sourced via `eval "$(...)"`). Honors `GOLDBAND_HOME`, `CLAUDE_PLUGIN_DATA`, `CLAUDE_PLANS_DIR`.
- The `claude` CLI binary resolves via `browse/src/claude-bin.ts` (`Bun.which()` + `GOLDBAND_CLAUDE_BIN` override). Set `GOLDBAND_CLAUDE_BIN=wsl` plus `GOLDBAND_CLAUDE_BIN_ARGS='["claude"]'` to run Claude through WSL on Windows.
