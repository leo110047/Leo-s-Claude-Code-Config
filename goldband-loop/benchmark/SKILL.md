...
description: |
  ...
  "bundle size", "load time". (goldband)
...
if [ "$_TEL" != "off" ]; then
echo '{"skill":"benchmark","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","repo":"'$(basename "$(git rev-parse --show-toplevel 2>/dev/null)" 2>/dev/null || echo "unknown")'"}'  >> ~/.goldband/analytics/skill-usage.jsonl 2>/dev/null || true
fi
...
Write to `.goldband/benchmark-reports/baselines/baseline.json`.
...
Write to `.goldband/benchmark-reports/{date}-benchmark.md` and `.goldband/benchmark-reports/{date}-benchmark.json`.
