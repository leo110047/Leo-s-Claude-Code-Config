...
bun run eval:list    # list all eval runs from ~/.goldband-dev/evals/
...
**Env keys in Conductor workspaces.** The `GOLDBAND_*` env-shim (v1.39.2.0+,
`lib/conductor-env-shim.ts`) promotes `GOLDBAND_ANTHROPIC_API_KEY` /
`GOLDBAND_OPENAI_API_KEY` to their canonical names inside goldband's TS binaries.
Tests run through goldband entrypoints inherit this promotion automatically.
...
--verbose`). Results are persisted to `~/.goldband-dev/evals/` with auto-comparison
...
├── bin/             # CLI utilities (goldband-repo-mode, goldband-slug, goldband-config, etc.)
...
├── open-goldband-browser/  # /open-goldband-browser skill (launch Goldband Loop Browser)
...
│   └── add-host/    # /goldband-contrib-add-host skill
...
2. **If missing, AskUserQuestion** — let the user tell you or let goldband search the repo
3. **Persist the answer to CLAUDE.md** so we never have to ask again

This applies to test commands, eval commands, deploy commands, and any other
project-specific behavior. The project owns its config; goldband reads it.
...
tighter V0 prose set `goldband-config set explain_level terse` (binary switch,
...
server must pass `false` so their discovery files survive goldband teardown cycles.
...
match) which would kill sibling goldband sessions on the same host; the new
...
connection immediately. `Set-Cookie: goldband_pty=...` is kept as a
...
PTY via `window.goldbandInjectToTerminal(text)`, exposed by
...
return 403. SSE endpoints use a 30-minute HttpOnly `goldband_sse` cookie minted via
`POST /sse-session` (never valid against `/command`). Tunnel-surface rejections go
to `~/.goldband/security/attempts.jsonl` via `tunnel-denial-log.ts`. Before editing
...
**Env knobs:**
- `GOLDBAND_SECURITY_OFF=1` — emergency kill switch. Classifier stays off even if
  warmed. Canary is still injected; just the ML scan is skipped.
- `GOLDBAND_SECURITY_ENSEMBLE=deberta` — opt-in DeBERTa-v3 ensemble. Adds
...
- Classifier model cache: `~/.goldband/models/testsavant-small/` (112MB, first run only)
  plus `~/.goldband/models/deberta-v3-injection/` (721MB, only when ensemble enabled)
- Attack log: `~/.goldband/security/attempts.jsonl` (salted sha256 + domain only,
  rotates at 10MB, 5 generations)
- Per-device salt: `~/.goldband/security/device-salt` (0600)
- Session state: `~/.goldband/security/session-state.json` (cross-process, atomic)

## Dev symlink awareness

When developing goldband, `.claude/skills/goldband` may be a symlink back to this
working directory (gitignored). This means skill changes are **live immediately**,
great for rapid iteration, risky during big refactors where half-written skills
could break other Claude Code sessions using goldband concurrently.

**Check once per session:** Run `ls -la .claude/skills/goldband` to see if it's a
symlink or a real copy. If it's a symlink to your working directory, be aware that:
- Template changes + `bun run gen:skill-docs` immediately affect all goldband invocations
- Breaking changes to SKILL.md.tmpl files can break concurrent goldband sessions
- During large refactors, remove the symlink (`rm .claude/skills/goldband`) so the
  global install at `~/.claude/skills/goldband/` is used instead

**Prefix setting:** Setup creates real directories (not symlinks) at the top level
with a SKILL.md symlink inside (e.g., `qa/SKILL.md -> goldband/qa/SKILL.md`). This
ensures Claude discovers them as top-level skills, not nested under `goldband/`.
Names are either short (`qa`) or namespaced (`goldband-qa`), controlled by
`skill_prefix` in `~/.goldband/config.yaml`. Pass `--no-prefix` or `--prefix` to
skip the interactive prompt.

**Note:** Vendoring goldband into a project's repo is deprecated. Use global install
+ `./setup --team` instead. See README.md for team mode instructions.

**For plan reviews:** When reviewing plans that modify skill templates or the
gen-skill-docs pipeline, consider whether the changes should be tested in isolation
before going live (especially if the user is actively using goldband in other windows).
...
migration script to `goldband-upgrade/migrations/`. Read CONTRIBUTING.md's "Upgrade
migrations" section for the format and testing requirements. The upgrade skill runs
these automatically after `./setup` during `/goldband-upgrade`.
...
`bin/goldband-next-version` advances within the chosen bump level rather than
...
When estimating or discussing effort, always show both human-team and CC+goldband time:

| Task type | Human team | CC+goldband | Compression |
|-----------|-----------|-----------|-------------|
...
Contributors can store long-range vision docs and design documents in `~/.goldband-dev/plans/`.
...
Native OpenClaw skills live in `openclaw/skills/goldband-openclaw-*/SKILL.md`. These are
...
```bash
clawhub publish openclaw/skills/goldband-openclaw-office-hours \
  --slug goldband-openclaw-office-hours --name "goldband Office Hours" \
  --version 1.0.0 --changelog "description of changes"
```

Repeat for each skill: `goldband-openclaw-ceo-review`, `goldband-openclaw-investigate`,
`goldband-openclaw-retro`. Bump `--version` on each update.
...
**Verification:** `clawhub search goldband` to confirm they're live.

## Deploying to the active skill

The active skill lives at `~/.claude/skills/goldband/`. After making changes:

1. Push your branch
2. Fetch and reset in the skill directory: `cd ~/.claude/skills/goldband && git fetch origin && git reset --hard origin/main`
3. Rebuild: `cd ~/.claude/skills/goldband && bun run build`

Or copy the binaries directly:
- `cp browse/dist/browse ~/.claude/skills/goldband/browse/dist/browse`
- `cp design/dist/design ~/.claude/skills/goldband/design/dist/design`
