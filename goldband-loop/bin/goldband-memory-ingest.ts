#!/usr/bin/env bun
/**
 * goldband-memory-ingest — V1 memory ingest helper.
 *
 * Walks coding-agent transcript sources + ~/.goldband/ curated artifacts and writes
 ...
 *
 * Usage:
 *   goldband-memory-ingest --probe                 # count what would ingest, no writes
 *   goldband-memory-ingest --incremental [--quiet] # default; mtime fast-path; cheap
 *   goldband-memory-ingest --bulk [--all-history]  # first-run; full walk
 *   goldband-memory-ingest --bulk --benchmark      # time the bulk pass + report
 *   goldband-memory-ingest --include-unattributed  # also ingest sessions with no git remote
 *
 * Sources walked:
 ...
 *   ~/.goldband/projects/<slug>/learnings.jsonl       — typed: learning
 *   ~/.goldband/projects/<slug>/timeline.jsonl        — typed: timeline
 *   ~/.goldband/projects/<slug>/ceo-plans/*.md        — typed: ceo-plan
 *   ~/.goldband/projects/<slug>/*-design-*.md         — typed: design-doc
 *   ~/.goldband/analytics/eureka.jsonl                — typed: eureka
 *   ~/.goldband/builder-profile.jsonl                 — typed: builder-profile-entry
 *
 * State: ~/.goldband/.transcript-ingest-state.json (LOCAL per ED1, never synced).
 * Secret scanning: gitleaks via lib/goldband-memory-helpers#secretScanFile (D19).
...
interface CliArgs {
   ...
   * default — the cross-machine boundary (goldband-brain-sync, git push)
...
function printUsage(): void {
  console.error(`Usage: goldband-memory-ingest [--probe|--incremental|--bulk] [options]
...
Options:
  ...
  --scan-secrets       Opt-in per-file gitleaks scan during prepare. Off by
                       default; goldband-brain-sync already gates the git-push
...
function parseArgs(): CliArgs {
  ...
  let noWrite = process.env.GOLDBAND_MEMORY_INGEST_NO_WRITE === "1";
  let scanSecrets = process.env.GOLDBAND_MEMORY_INGEST_SCAN_SECRETS === "1";
...
function loadState(): IngestState {
  if (!existsSync(STATE_PATH)) {
    return {
      schema_version: 1,
      last_writer: "goldband-memory-ingest",
      sessions: {},
    };
  }
  try {
...
function saveState(state: IngestState): void {
  ...
  // in goldband-gbrain-sync.ts:saveSyncState.
  try {
...
function* walkGoldbandArtifacts(ctx: WalkContext): Generator<{ path: string; type: MemoryType }> {
  const projectsRoot = join(GOLDBAND_HOME, "projects");

  // Eureka log: ~/.goldband/analytics/eureka.jsonl
  const eurekaLog = join(GOLDBAND_HOME, "analytics", "eureka.jsonl");
  if (existsSync(eurekaLog) && ctx.args.sources.has("eureka")) {
    yield { path: eurekaLog, type: "eureka" };
  }

  // Builder profile: ~/.goldband/builder-profile.jsonl
  const builderProfile = join(GOLDBAND_HOME, "builder-profile.jsonl");
...
function* walkAllSources(ctx: WalkContext): Generator<{ path: string; type: MemoryType }> {
  ...
  yield* walkGoldbandArtifacts(ctx);
}
...
function buildArtifactPage(path: string, type: MemoryType): PageRecord {
  ...
  // Extract repo slug from path: ~/.goldband/projects/<slug>/...
  let slug_repo = "_unattributed";
  const m = path.match(/\/\.goldband\/projects\/([^/]+)\//);
  if (m) slug_repo = m[1];
 ...
 *
 *   The actual cross-machine exfiltration boundary is `goldband-brain-sync`,
 *   which runs a regex-based secret scanner on the staged diff before
 *   `git commit` (see bin/goldband-brain-sync:78-110: AWS keys, GitHub
...
function preparePages(
  ...
  for (const { path, type } of walkAllSources(ctx)) {
    ...
    // because goldband-brain-sync already gates the cross-machine boundary
...
/**
 * Make a per-run staging directory at ~/.goldband/.staging-ingest-<pid>-<ts>/
...
function makeStagingDir(): string {
  const dir = join(GOLDBAND_HOME, `.staging-ingest-${process.pid}-${Date.now()}`);
 ...
 *
 * Instead of staging to ~/.goldband/.staging-ingest-<pid>-<ts>/ and cleaning up
 * after `gbrain import`, remote-http users get a stable path that survives.
 * goldband-brain-sync's allowlist pushes ~/.goldband/transcripts/** to the
 * artifacts repo; the brain admin's pull job indexes them into the remote
 * brain. Local PGLite (if present) stays code-only.
 *
 * Path: ~/.goldband/transcripts/<run-id>/  (run-id pid+ts so concurrent passes
 ...
 *
 * Reads ~/.claude.json directly (same fallback chain as goldband-gbrain-detect
  ...
  const forward = (signal: NodeJS.Signals) => () => {
    ...
    if (_activeStagingDir) {
      if (stagingDirIsCheckpointed(_activeStagingDir)) {
        ...
        // (in goldband-gbrain-sync.ts) will see the checkpoint + dir and
...
function runGbrainImport(
  ...
  return new Promise((resolve) => {
    ...
    // parent goldband-gbrain-sync seeding the bun grandchild's env).
...
async function ingestPass(args: CliArgs): Promise<BulkResult> {
  ...
  if (args.noWrite) {
    ...
    state.last_writer = "goldband-memory-ingest";
    saveState(state);
    return {
      written,
  ...
  // PERSISTENT dir under ~/.goldband/transcripts/ and SKIP `gbrain import`
  // entirely. goldband-brain-sync push will pick the dir up via its allowlist
  ...
  // GOLDBAND_INGEST_RESUME_DIR (because gbrain's import-checkpoint.json points
  ...
  const resumeDir = process.env.GOLDBAND_INGEST_RESUME_DIR;
  ...
  try {
    ...
    // staged markdown lives under ~/.goldband/transcripts/<run-id>/ and the
    // next goldband-brain-sync push will move it to the artifacts repo. From
    ...
    if (remoteHttpMode) {
      ...
      state.last_writer = "goldband-memory-ingest (remote-http mode)";
      saveState(state);
      if (!args.quiet) {
        console.error(
...
main().catch((err) => {
  console.error(`goldband-memory-ingest fatal: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
