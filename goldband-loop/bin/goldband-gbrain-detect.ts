...
/**
 * goldband-gbrain-detect implementation — emit current gbrain/goldband-brain state as JSON.
 *
 * Rewritten from bash to TypeScript in v{X.Y.Z.0} to share the engine-status
 * classifier with bin/goldband-gbrain-sync.ts. Single source of truth via
 ...
 *     "goldband_brain_sync_mode": "off"|"artifacts-only"|"full",
 *     "goldband_brain_git": true|false,
 *     "goldband_artifacts_remote": "https://..." | "",
 ...
 *
 * Env:
 *   GOLDBAND_HOME      — override ~/.goldband for state lookups (used by tests).
 *   HOME             — effective user home (drives ~/.gbrain/config.json path).
 *   GOLDBAND_DETECT_NO_CACHE=1 — bypass the 60s local-status cache.
 */
...
const STATE_DIR = process.env.GOLDBAND_HOME || join(userHome(), ".goldband");
const SCRIPT_DIR = __dirname;
const CONFIG_BIN = join(SCRIPT_DIR, "goldband-config");
...
// --- goldband-brain git repo present? ---
...
function detectArtifactsRemote(): string {
  const newPath = join(userHome(), ".goldband-artifacts-remote.txt");
  const oldPath = join(userHome(), ".goldband-brain-remote.txt");
...
function main(): void {
  ...
  const noCache = process.env.GOLDBAND_DETECT_NO_CACHE === "1";
  ...
  const out = {
    ...
    goldband_brain_sync_mode: detectSyncMode(),
    goldband_brain_git: detectBrainGit(),
    goldband_artifacts_remote: detectArtifactsRemote(),
