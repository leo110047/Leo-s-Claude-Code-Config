#!/usr/bin/env bun
/**
 * goldband-brain-context-load — V1 retrieval surface (Lane C).
 *
 * Called from the goldband preamble at every skill start. Reads the active skill's
...
function printUsage(): void {
  console.error(`Usage: goldband-brain-context-load [options]
...
function resolveSkillFile(args: CliArgs): string | null {
  ...
  // Look in common goldband skill locations
  const candidates = [
    join(HOME, ".claude", "skills", args.skill, "SKILL.md"),
    join(HOME, ".claude", "skills", "goldband", args.skill, "SKILL.md"),
...
main().catch((err) => {
  console.error(`goldband-brain-context-load fatal: ${err instanceof Error ? err.message : String(err)}`);
  process.exit(1);
});
