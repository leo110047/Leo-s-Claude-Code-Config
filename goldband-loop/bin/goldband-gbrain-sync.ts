#!/usr/bin/env bun
/**
 * goldband-gbrain-sync — V1 unified sync verb.
 *
 ...
 *   2. Transcripts + curated memory → goldband-memory-ingest (typed put_page)
 *   3. Curated artifacts to git    → goldband-brain-sync (existing pipeline)
 *
 * Modes:
 ...
 *   - Lock file at ~/.goldband/.sync-gbrain.lock (PID + start ts).
 ...
 * than building a goldband-side daemon.
 */
...
import { detectEngineTier, withErrorContext, canonicalizeRemote } from "../lib/goldband-memory-helpers";
...
const STATE_PATH = join(GOLDBAND_HOME, ".gbrain-sync-state.json");
const LOCK_PATH = join(GOLDBAND_HOME, ".sync-gbrain.lock");
const STALE_LOCK_MS = 5 * 60 * 1000;

// Default 35-minute timeout for code-walk + memory-ingest stages. Override via
// GOLDBAND_SYNC_CODE_TIMEOUT_MS / GOLDBAND_SYNC_MEMORY_TIMEOUT_MS. Bounds-checked
...
function printUsage(): void {
  console.error(`Usage: goldband-gbrain-sync [--incremental|--full|--dry-run] [options]
...
Options:
  ...
  --no-memory          Skip the goldband-memory-ingest stage (transcripts + artifacts).
  --no-brain-sync      Skip the goldband-brain-sync git pipeline stage.
 ...
 *
 * Pattern: `goldband-code-<slug>-<hostpathhash8>` where slug comes from origin
 ...
 *
 * `GOLDBAND_HOSTNAME` env override is honored for deterministic tests; in
...
function deriveCodeSourceId(repoPath: string): string {
  const host = process.env.GOLDBAND_HOSTNAME || hostname();
  ...
  if (remote) {
    ...
    const fullId = constrainSourceId("goldband-code", `${slugSource}-${hostPathHash}`);
    ...
    return constrainSourceId("goldband-code", `${repoOnly}-${hostPathHash}`);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("goldband-code", `${base}-${hostPathHash}`);
}

/**
 * Pre-pathhash source id, kept for orphan detection only.
 *
 * Earlier /sync-gbrain versions registered `goldband-code-<slug>` (no pathhash
...
function deriveLegacyCodeSourceId(repoPath: string): string {
  ...
  if (remote) {
    const segs = remote.split("/").filter(Boolean);
    const slugSource = segs.slice(-2).join("-");
    return constrainSourceId("goldband-code", slugSource);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("goldband-code", base);
}
 ...
 * repo path: `goldband-code-<slug>-<sha1(path).slice(0,8)>`. After #1468 the
...
export function derivePathOnlyHashLegacyId(repoPath: string): string {
  ...
  if (remote) {
    const segs = remote.split("/").filter(Boolean);
    const slugSource = segs.slice(-2).join("-");
    return constrainSourceId("goldband-code", `${slugSource}-${pathHash}`);
  }
  const base = repoPath.split("/").pop() || "repo";
  return constrainSourceId("goldband-code", `${base}-${pathHash}`);
}
...
function acquireLock(): boolean {
  mkdirSync(GOLDBAND_HOME, { recursive: true });
...
async function runCodeImport(args: CliArgs): Promise<StageResult> {
  ...
  // Earlier /sync-gbrain versions registered `goldband-code-<slug>` (no path
  ...
  const codeTimeoutMs = resolveStageTimeoutMs(
    process.env.GOLDBAND_SYNC_CODE_TIMEOUT_MS,
    "GOLDBAND_SYNC_CODE_TIMEOUT_MS",
  );
  ...
  // goldband's own .gitignore. Without the consumer-side entry, the pin gets
...
function runMemoryIngest(args: CliArgs): StageResult {
  ...
  if (args.mode === "dry-run") {
    return { name: "memory", ran: false, ok: true, duration_ms: 0, summary: "would: goldband-memory-ingest --probe" };
  }

  // Split-engine pre-flight (per plan D12). goldband-memory-ingest shells out
  ...
  if (resume.kind === "resume") {
    ...
    childEnv.GOLDBAND_INGEST_RESUME_DIR = resume.stagingDir;
  ...
  const ingestPath = join(import.meta.dir, "goldband-memory-ingest.ts");
  ...
  // .env.local footgun affects goldband-memory-ingest.ts too, not just the
  ...
  const memoryTimeoutMs = resolveStageTimeoutMs(
    process.env.GOLDBAND_SYNC_MEMORY_TIMEOUT_MS,
    "GOLDBAND_SYNC_MEMORY_TIMEOUT_MS",
  );
...
function runBrainSyncPush(args: CliArgs): StageResult {
  ...
  if (args.mode === "dry-run") {
    return { name: "brain-sync", ran: false, ok: true, duration_ms: 0, summary: "would: goldband-brain-sync --discover-new --once" };
  }

  const brainSyncPath = join(import.meta.dir, "goldband-brain-sync");
  if (!existsSync(brainSyncPath)) {
    return { name: "brain-sync", ran: false, ok: true, duration_ms: 0, summary: "skipped (goldband-brain-sync not installed)" };
  }
  ...
  return {
    ...
    ok: result.status === 0,
    duration_ms: Date.now() - t0,
    summary: result.status === 0 ? "curated artifacts pushed" : `goldband-brain-sync exited ${result.status}`,
  };
}
...
function loadSyncState(): SyncState {
  if (!existsSync(STATE_PATH)) {
    return { schema_version: 1, last_writer: "goldband-gbrain-sync" };
  }
  try {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!args.noCode) {
      stages.push(await withErrorContext("sync:code", () => runCodeImport(args), "goldband-gbrain-sync"));
    }
    if (!args.noMemory) {
      stages.push(await withErrorContext("sync:memory", () => runMemoryIngest(args), "goldband-gbrain-sync"));
    }
    if (!args.noBrainSync) {
      stages.push(await withErrorContext("sync:brain-sync", () => runBrainSyncPush(args), "goldband-gbrain-sync"));
    }
    ...
    if (!args.quiet || args.mode === "dry-run") {
      console.log(`\ngoldband-gbrain-sync (${args.mode}):`);
...
if (import.meta.main) {
  main().catch((err) => {
    console.error(`goldband-gbrain-sync fatal: ${err instanceof Error ? err.message : String(err)}`);
    releaseLock();
    process.exit(1);
  });
}
