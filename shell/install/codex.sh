...
emit_installed_codex_managed_config() {
    local path="$1"
    [ -f "$path" ] || return 1
    awk '
...
is_current_generated_codex_config() {
    ...
    is_generated_codex_config "$path" || return 1
    [ -f "$base_config" ] || return 1
    cmp -s \
        <(write_generated_codex_config "$base_config" "$local_config" | normalize_codex_managed_config) \
        <(emit_installed_codex_managed_config "$path" | normalize_codex_managed_config)
}
