# This file must be sourced by bash, not executed directly.

is_generated_codex_config() {
    local path="$1"
    [ -f "$path" ] || return 1
    head -n 1 "$path" 2>/dev/null | grep -q '^# Generated by goldband Codex config installer$'
}

emit_codex_profile_without_runtime_state() {
    local path="$1"
    [ -f "$path" ] || return 1
    awk '
        /^\[hooks\.state(\.|\])/ { in_runtime_state = 1; next }
        /^\[tui\.model_availability_nux\]/ { in_runtime_state = 1; next }
        in_runtime_state && /^\[/ { in_runtime_state = 0 }
        !in_runtime_state { print }
    ' "$path"
}

normalize_codex_managed_config() {
    awk '
        /^[[:space:]]*$/ { pending_blanks = pending_blanks "\n"; next }
        {
            if (pending_blanks != "") {
                printf "%s", pending_blanks
                pending_blanks = ""
            }
            print
        }
    '
}

emit_codex_config_owned_records() {
    awk '
        function trim(value) {
            sub(/^[[:space:]]+/, "", value)
            sub(/[[:space:]]+$/, "", value)
            return value
        }
        function runtime_owned(table_name, key_name) {
            return table_name ~ /^\[marketplaces\./ && key_name == "last_updated"
        }
        /^[[:space:]]*($|#)/ { next }
        /^[[:space:]]*\[/ {
            table_name = trim($0)
            next
        }
        {
            separator = index($0, "=")
            if (separator == 0) next
            key_name = trim(substr($0, 1, separator - 1))
            value = trim(substr($0, separator + 1))
            if (runtime_owned(table_name, key_name)) next
            print table_name "\034" key_name "\034" value
        }
    '
}

codex_config_projection_source_supported() {
    local path="$1"
    [ -f "$path" ] || return 1
    awk '
        function trim(value) {
            sub(/^[[:space:]]+/, "", value)
            sub(/[[:space:]]+$/, "", value)
            return value
        }
        /^[[:space:]]*($|#)/ { next }
        /^[[:space:]]*\[[^]]+\][[:space:]]*(#.*)?$/ { next }
        {
            separator = index($0, "=")
            if (separator == 0) exit 1
            value = trim(substr($0, separator + 1))
            if (substr(value, 1, 3) == "\"\"\"" || substr(value, 1, 3) == "\047\047\047") exit 1
            if (value ~ /^\[/ && value !~ /\][[:space:]]*(#.*)?$/) exit 1
            if (value ~ /^\{/ && value !~ /\}[[:space:]]*(#.*)?$/) exit 1
        }
    ' "$path"
}

codex_toml_syntax_valid() {
    local path="$1"
    local validator=()
    if command -v python3 >/dev/null 2>&1 && python3 -c 'import tomllib' >/dev/null 2>&1; then
        validator=(python3)
    elif command -v python >/dev/null 2>&1 && python -c 'import tomllib' >/dev/null 2>&1; then
        validator=(python)
    elif command -v py >/dev/null 2>&1 && py -3 -c 'import tomllib' >/dev/null 2>&1; then
        validator=(py -3)
    else
        CODEX_CONFIG_FRESHNESS_REASON="syntax-unverifiable"
        return 1
    fi

    if ! "${validator[@]}" - "$path" >/dev/null 2>&1 <<'PY'
import sys
import tomllib

with open(sys.argv[1], "rb") as config_file:
    tomllib.load(config_file)
PY
    then
        CODEX_CONFIG_FRESHNESS_REASON="invalid-toml"
        return 1
    fi
}

codex_config_owned_records_match() {
    local expected_records="$1"
    local installed_records="$2"
    awk '
        BEGIN { FS = sprintf("%c", 28) }
        NR == FNR {
            record_key = $1 FS $2
            if (record_key in expected_value) duplicate_expected = 1
            expected_value[record_key] = $3
            next
        }
        {
            record_key = $1 FS $2
            if (!(record_key in expected_value)) next
            seen[record_key]++
            if ($3 != expected_value[record_key]) mismatch[record_key] = 1
        }
        END {
            if (duplicate_expected) exit 1
            for (record_key in expected_value) {
                if (seen[record_key] != 1 || mismatch[record_key]) exit 1
            }
        }
    ' "$expected_records" "$installed_records"
}

is_current_generated_codex_config() {
    local path="$1"
    local base_config="$REPO_DIR/codex/config.toml"
    local local_config="$REPO_DIR/codex/local/config.toml"

    CODEX_CONFIG_FRESHNESS_REASON=""
    is_generated_codex_config "$path" || {
        CODEX_CONFIG_FRESHNESS_REASON="not-generated"
        return 1
    }
    codex_toml_syntax_valid "$path" || return 1
    [ -f "$base_config" ] || {
        CODEX_CONFIG_FRESHNESS_REASON="source-missing"
        return 1
    }
    codex_config_projection_source_supported "$base_config" || {
        CODEX_CONFIG_FRESHNESS_REASON="source-unsupported"
        return 1
    }
    if [ -f "$local_config" ]; then
        codex_config_projection_source_supported "$local_config" || {
            CODEX_CONFIG_FRESHNESS_REASON="source-unsupported"
            return 1
        }
    fi
    if ! codex_config_owned_records_match \
        <(write_generated_codex_config "$base_config" "$local_config" | emit_codex_config_owned_records) \
        <(emit_codex_config_owned_records < "$path"); then
        CODEX_CONFIG_FRESHNESS_REASON="managed-drift"
        return 1
    fi
    CODEX_CONFIG_FRESHNESS_REASON="current"
}

codex_profile_installed_from() {
    local src="$1"
    local dest="$2"

    repo_link_points_to "$dest" "$src" && return 0
    [ -f "$src" ] && [ -f "$dest" ] || return 1
    cmp -s \
        <(normalize_codex_managed_config < "$src") \
        <(emit_codex_profile_without_runtime_state "$dest" | normalize_codex_managed_config)
}

emit_toml_root_keys() {
    local path="$1"
    awk '
        /^[[:space:]]*\[/ { in_tables = 1 }
        !in_tables { print }
    ' "$path"
}

emit_toml_tables() {
    local path="$1"
    awk '
        /^[[:space:]]*\[/ { in_tables = 1 }
        in_tables { print }
    ' "$path"
}

install_generated_codex_config() {
    local base_config="$REPO_DIR/codex/config.toml"
    local local_config="$REPO_DIR/codex/local/config.toml"
    local dest="$CODEX_CONFIG_FILE"

    if [ ! -f "$base_config" ]; then
        echo -e "  ${YELLOW}[跳過] Codex config.toml — 來源不存在${NC}"
        return
    fi

    if [ -L "$dest" ]; then
        rm "$dest"
    elif [ -e "$dest" ] && ! is_generated_codex_config "$dest"; then
        backup_existing_path "$dest"
    fi

    mkdir -p "$(dirname "$dest")"

    local tmp="${dest}.tmp.$$"
    write_generated_codex_config "$base_config" "$local_config" > "$tmp"

    mv "$tmp" "$dest"

    if [ -f "$local_config" ]; then
        echo -e "  ${GREEN}[安裝] Codex config.toml (base + local overlay)${NC}"
    else
        echo -e "  ${GREEN}[安裝] Codex config.toml (base only)${NC}"
    fi
}

write_generated_codex_config() {
    local base_config="$1"
    local local_config="$2"

    echo "# Generated by goldband Codex config installer"
    echo "# Source: $base_config"
    if [ -f "$local_config" ]; then
        echo "# Local overlay: $local_config"
    else
        echo "# Local overlay: none"
    fi
    echo ""
    write_generated_codex_managed_config "$base_config" "$local_config"
}

write_generated_codex_managed_config() {
    local base_config="$1"
    local local_config="$2"

    echo "# --- goldband portable root keys ---"
    emit_toml_root_keys "$base_config"
    emit_local_codex_config_section "$local_config" "root keys" emit_toml_root_keys
    echo ""
    echo "# --- goldband portable tables ---"
    emit_toml_tables "$base_config"
    emit_local_codex_config_section "$local_config" "tables" emit_toml_tables
}

emit_local_codex_config_section() {
    local local_config="$1"
    local label="$2"
    local emitter="$3"
    [ -f "$local_config" ] || return 0
    echo ""
    echo "# --- goldband local $label ---"
    "$emitter" "$local_config"
}

install_codex_profile_configs() {
    local profile_dir="$REPO_DIR/codex/profiles"

    if [ ! -d "$profile_dir" ]; then
        echo -e "  ${YELLOW}[跳過] Codex profile configs — 來源不存在${NC}"
        return
    fi

    local profile_file
    for profile_file in "$profile_dir"/*.config.toml; do
        [ -f "$profile_file" ] || continue
        install_codex_profile_config_file "$profile_file" "$CODEX_DIR/$(basename "$profile_file")"
    done
}

install_codex_profile_config_file() {
    local src="$1"
    local dest="$2"
    local label="Codex profile $(basename "$src")"

    if [ ! -e "$src" ]; then
        echo -e "  ${YELLOW}[跳過] $label — 來源不存在${NC}"
        return
    fi

    if [ -L "$dest" ]; then
        rm "$dest"
    elif [ -e "$dest" ] && ! cmp -s "$src" "$dest" 2>/dev/null; then
        backup_existing_path "$dest"
    fi

    materialize_file_copy "$src" "$dest" || return 1
    chmod 0644 "$dest"
    echo -e "  ${GREEN}[安裝] $label (materialized copy)${NC}"
}

install_codex_requirements_file() {
    local src="$REPO_DIR/codex/requirements.toml"
    local dest="$CODEX_REQUIREMENTS_FILE"

    if [ ! -f "$src" ]; then
        echo -e "  ${YELLOW}[跳過] Codex requirements.toml — 來源不存在${NC}"
        return
    fi

    local dest_dir
    dest_dir="$(dirname "$dest")"
    local backup_suffix
    backup_suffix="$(date +%s)"

    local can_write_without_sudo=false
    if [ "$(id -u)" -eq 0 ]; then
        can_write_without_sudo=true
    elif mkdir -p "$dest_dir" 2>/dev/null && [ -w "$dest_dir" ]; then
        can_write_without_sudo=true
    fi

    if $can_write_without_sudo; then
        mkdir -p "$dest_dir"
        if [ -e "$dest" ] && ! cmp -s "$src" "$dest"; then
            cp "$dest" "${dest}.bak.${backup_suffix}"
            echo -e "  ${YELLOW}[備份] Codex requirements.toml -> ${dest}.bak.${backup_suffix}${NC}"
        fi
        cp "$src" "$dest"
        chmod 0644 "$dest"
    elif is_windows_host; then
        echo -e "  ${RED}[錯誤] Codex requirements 需要可寫入的 Windows managed path:${NC} $dest" >&2
        echo -e "  ${CYAN}  請用系統管理員權限建立此目錄並複製 codex/requirements.toml，或用 CODEX_REQUIREMENTS_FILE 指到可寫入的測試路徑。${NC}" >&2
        return 1
    else
        sudo mkdir -p "$dest_dir"
        if sudo test -e "$dest" && ! sudo cmp -s "$src" "$dest"; then
            sudo cp "$dest" "${dest}.bak.${backup_suffix}"
            echo -e "  ${YELLOW}[備份] Codex requirements.toml -> ${dest}.bak.${backup_suffix}${NC}"
        fi
        sudo cp "$src" "$dest"
        sudo chmod 0644 "$dest"
    fi

    echo -e "  ${GREEN}[安裝] Codex managed requirements -> $dest${NC}"
}

is_repo_codex_rule_link() {
    local link_path="$1"
    local base_rules="$REPO_DIR/codex/rules"
    local local_rules="$REPO_DIR/codex/local/rules"

    [ -L "$link_path" ] || return 1

    local target
    target=$(readlink "$link_path")
    case "$target" in
        "$base_rules"/*|"$local_rules"/*)
            return 0
            ;;
        *)
            return 1
            ;;
    esac
}

link_codex_rule_file() {
    local src="$1"
    local dest="$2"
    local label="$3"

    if repo_path_installed_from "$src" "$dest"; then
        if [ -L "$dest" ]; then
            echo -e "  ${GREEN}[已安裝] $label${NC}"
        else
            echo -e "  ${GREEN}[已安裝 (copy fallback)] $label${NC}"
        fi
        return
    fi

    if [ -L "$dest" ]; then
        if is_repo_codex_rule_link "$dest"; then
            rm "$dest"
        else
            backup_existing_path "$dest"
        fi
    elif [ -e "$dest" ]; then
        echo -e "  ${YELLOW}[保留] $label — 目標已有非 goldband 檔案: $dest${NC}"
        return
    fi

    create_repo_link "$src" "$dest" || return 1
    if [ "$CREATE_REPO_LINK_MODE" = "copy" ]; then
        echo -e "  ${YELLOW}[安裝 (copy fallback)] $label — 此環境無法建立檔案 symlink${NC}"
    else
        echo -e "  ${GREEN}[安裝 (repo-linked)] $label${NC}"
    fi
}

ensure_codex_local_default_rules() {
    local local_default="$REPO_DIR/codex/local/rules/default.rules"

    if [ -f "$local_default" ]; then
        return
    fi

    mkdir -p "$(dirname "$local_default")"
    {
        echo "# Machine-local Codex execpolicy rules."
        echo "# This ignored file is the writable default target for one-off approvals."
        echo ""
    } > "$local_default"
}

prune_stale_codex_rule_links() {
    local entry
    for entry in "$CODEX_RULES_DIR"/*.rules; do
        if [ ! -e "$entry" ] && [ ! -L "$entry" ]; then
            continue
        fi
        if is_repo_codex_rule_link "$entry"; then
            local target
            target=$(readlink "$entry")
            if [ ! -e "$target" ]; then
                rm "$entry"
            fi
        fi
    done
}

install_codex_rules_dir() {
    local base_rules="$REPO_DIR/codex/rules"
    local local_rules="$REPO_DIR/codex/local/rules"

    if [ ! -d "$base_rules" ]; then
        echo -e "  ${YELLOW}[跳過] Codex Rules — 來源不存在${NC}"
        return
    fi

    if [ -L "$CODEX_RULES_DIR" ]; then
        rm "$CODEX_RULES_DIR"
    elif [ -e "$CODEX_RULES_DIR" ] && [ ! -d "$CODEX_RULES_DIR" ]; then
        backup_existing_path "$CODEX_RULES_DIR"
    fi

    mkdir -p "$CODEX_RULES_DIR"
    ensure_codex_local_default_rules
    prune_stale_codex_rule_links

    local rule_file
    for rule_file in "$base_rules"/*.rules; do
        [ -f "$rule_file" ] || continue
        local dest_name
        dest_name="$(basename "$rule_file")"
        if [ "$dest_name" = "default.rules" ]; then
            dest_name="goldband.rules"
        fi
        link_codex_rule_file "$rule_file" "$CODEX_RULES_DIR/$dest_name" "Codex rule $dest_name"
    done

    if [ -d "$local_rules" ]; then
        for rule_file in "$local_rules"/*.rules; do
            [ -f "$rule_file" ] || continue
            link_codex_rule_file "$rule_file" "$CODEX_RULES_DIR/$(basename "$rule_file")" "Codex local rule $(basename "$rule_file")"
        done
    fi
}

codex_portable_rules_have_local_state() {
    local base_rules="$REPO_DIR/codex/rules"
    [ -d "$base_rules" ] || return 1
    grep -R -q -F '/Users/' "$base_rules" ||
        grep -R -q -F '/private/tmp' "$base_rules" ||
        grep -R -q -F '.studio_os_write_token' "$base_rules" ||
        grep -R -q -F 'LaunchAgents' "$base_rules"
}

warn_codex_portable_rules_local_state() {
    if ! codex_portable_rules_have_local_state; then
        return 0
    fi

    echo -e "  ${YELLOW}[警告] Codex portable rules 含有本機 approvals，會阻止 self-update${NC}"
    echo -e "  ${CYAN}  建議執行:${NC} ./install.sh repair-codex-rules"
    echo -e "  ${CYAN}  這會把本機 approvals 從 codex/rules/default.rules 移到 ignored overlay:${NC}"
    echo -e "  ${CYAN}  codex/local/rules/default.rules${NC}"
}

repair_codex_local_rules() {
    local migrator="$REPO_DIR/scripts/migrate-codex-local-rules.py"
    local base_rules="$REPO_DIR/codex/rules/default.rules"
    local local_rules="$REPO_DIR/codex/local/rules/default.rules"

    if [ ! -f "$migrator" ]; then
        echo -e "  ${RED}[錯誤] 找不到 repair script: $migrator${NC}"
        return 1
    fi
    if ! command -v python3 >/dev/null 2>&1; then
        echo -e "  ${RED}[錯誤] repair-codex-rules 需要 python3${NC}"
        return 1
    fi

    python3 "$migrator" --base-rules "$base_rules" --local-rules "$local_rules"
    bash "$REPO_DIR/scripts/check-codex-portability.sh"
    install_codex_rules_dir
}
