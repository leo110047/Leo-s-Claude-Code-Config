...
function writeGeneratedCodexConfig(context) {
  ...
  const localConfig = path.join(context.repoDir, 'codex', 'local', 'config.toml');
  const destPath = context.paths.codexConfigFile;

  if (!fs.existsSync(baseConfig)) {
    console.log(`  ${colorize(context.colorsEnabled, 'yellow', '[skip]')} Codex config.toml — source missing`);
    return;
  }
  ...
  const lines = [
    ...
    `# Source: ${baseConfig}`,
    fs.existsSync(localConfig) ? `# Local overlay: ${localConfig}` : '# Local overlay: none',
    '',
...
function isRepoCodexRuleLink(context, linkPath) {
  ...
  return target.startsWith(`${baseRules}${path.sep}`) || target.startsWith(`${localRules}${path.sep}`);
}
...
function installCodexRuleFile(context, sourcePath, destPath, label) {
  ...
  if (stat) {
    if (stat.isSymbolicLink()) {
      const currentTarget = path.resolve(path.dirname(destPath), fs.readlinkSync(destPath));
...
function installCodexRulesDirectory(context) {
  ...
  if (!fs.existsSync(baseRules)) {
    console.log(`  ${colorize(context.colorsEnabled, 'yellow', '[skip]')} Codex Rules — source missing`);
    return;
  }
