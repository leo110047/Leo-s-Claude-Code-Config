...
test -L "$TMP_HOME/.codex/readonly.config.toml"
test -L "$TMP_HOME/.codex/release.config.toml"
test -L "$TMP_HOME/.codex/auto_review_experiment.config.toml"
test -f "$TMP_HOME/.codex/requirements.toml"
test -L "$TMP_HOME/.claude/CLAUDE.md"
test "$(readlink "$TMP_HOME/.claude/CLAUDE.md")" = "$TMP_ROOT/claude/CLAUDE.md"
test -L "$TMP_HOME/.codex/AGENTS.md"
test -d "$TMP_HOME/.codex/rules"
test -L "$TMP_HOME/.codex/rules/default.rules"
test -L "$TMP_HOME/.codex/rules/goldband.rules"
test "$(readlink "$TMP_HOME/.codex/rules/default.rules")" = "$TMP_ROOT/codex/local/rules/default.rules"
test "$(readlink "$TMP_HOME/.codex/rules/goldband.rules")" = "$TMP_ROOT/codex/rules/default.rules"
...
test "$(HOME="$TMP_HOME" git config --global --get core.hooksPath)" = "$TMP_ROOT/git-hooks"
...
echo "$STATUS_OUTPUT" | grep -q "claude CLAUDE.md"
echo "$STATUS_OUTPUT" | grep -q "codex custom agents"
echo "$STATUS_OUTPUT" | grep -q "codex hooks.json"
echo "$STATUS_OUTPUT" | grep -q "codex hook scripts"
echo "$STATUS_OUTPUT" | grep -q "codex-rules"
echo "$STATUS_OUTPUT" | grep -q "codex profiles"
echo "$STATUS_OUTPUT" | grep -q "codex requirements"
echo "$STATUS_OUTPUT" | grep -q "token-backed MCP env"
echo "$STATUS_OUTPUT" | grep -q "Git style gate 狀態"
echo "$STATUS_OUTPUT" | grep -q "global core.hooksPath -> $TMP_ROOT/git-hooks"
