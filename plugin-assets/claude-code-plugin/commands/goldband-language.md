...
5. 如果有合法參數：
   - 先找 helper script，依序檢查：
     - `${CLAUDE_PLUGIN_ROOT}/commands/scripts/set-goldband-language.sh`
     - `~/.claude/plugins/cache/goldband/goldband/*/commands/scripts/set-goldband-language.sh`（若有多版，選版本排序最後一個）
     - `~/.claude/commands/scripts/set-goldband-language.sh`
   - 找不到 helper script 時，明確說 goldband language helper 尚未安裝，提示使用者重裝 `goldband@goldband` plugin 或跑 `./install.sh commands`
   - 用 `bash "$helper" set <normalized>` 執行，不要假設 helper 有 executable bit
   - 這一步會更新 `goldband_language` config
   - 再執行一次 `bash "$helper" get` 驗證
   - 根據新語言回報切換結果
