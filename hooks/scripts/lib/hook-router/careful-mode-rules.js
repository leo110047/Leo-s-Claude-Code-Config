const CAREFUL_MODE_GUARDS = [
  ...
  },
  {
    ...
    detail:
      'destructive SQL via psql/mysql/sqlite3/sqlcmd/pscale/prisma db execute',
    matches(command) {
      const usesDbClient =
        /\b(psql|mysql|sqlite3|sqlcmd|pscale|prisma\s+db\s+execute)\b/i.test(
          command,
        );
